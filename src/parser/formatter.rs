	...
	fn format_compact(&self) -> String {
		...
		if is_listing {
			...
			if self.dependencies.len() > MAX_DEPS_LISTING {
				lines.push(format!("  ... +{} more", self.dependencies.len() - MAX_DEPS_LISTING));
			}
			return lines.join("\n");
		}
		...
		for dep in self.dependencies.iter().take(10) {
			if let Some(latest) = &dep.latest_version {
				if &dep.current_version != latest {
					lines.push(format!("{}: {} → {}", dep.name, dep.current_version, latest));
				}
			}
		}
			...
			for dep in &self.dependencies {
				if let Some(latest) = &dep.latest_version {
					if &dep.current_version != latest {
						...
						lines.push(format!("  {}: {} → {}{}", dep.name, dep.current_version, latest, dev_marker));
...
mod tests {
	...
	fn test_dependency_state_plain_listing_shows_packages() {
		let state = DependencyState {
			total_packages: 2,
			outdated_count: 0,
			dependencies: vec![make_dep("react", "18.0.0", None), make_dep("typescript", "5.0.0", None)],
		};
