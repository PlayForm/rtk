...
impl PeriodEconomics {
	...
	fn set_rtk_from_month(&mut self, stats: &MonthStats) {
		...
		self.rtk_savings_pct = Some(if stats.input_tokens + stats.output_tokens > 0 {
			stats.saved_tokens as f64 / (stats.saved_tokens + stats.input_tokens + stats.output_tokens) as f64 * 100.0
		} else {
			0.0
		});
	}
...
pub fn run(daily: bool, weekly: bool, monthly: bool, all: bool, format: &str, verbose: u8) -> Result<()> {
...
fn display_text(tracker: &Tracker, daily: bool, weekly: bool, monthly: bool, all: bool, verbose: u8) -> Result<()> {
...
fn display_summary(tracker: &Tracker, verbose: u8) -> Result<()> {
	let cc_monthly = ccusage::fetch(Granularity::Monthly).context("Failed to fetch ccusage monthly data")?;
	...
	println!("  Spent (ccusage):              {}", format_usd(totals.cc_cost));
	...
	if let Some(weighted_savings) = totals.savings_weighted {
		...
		if let Some(input_cpt) = totals.weighted_input_cpt {
			println!("  │ Derived input CPT:     {}               │", format_cpt(input_cpt));
		}
	} else {
...
fn display_daily(tracker: &Tracker, verbose: u8) -> Result<()> {
	let cc_daily = ccusage::fetch(Granularity::Daily).context("Failed to fetch ccusage daily data")?;
...
fn display_weekly(tracker: &Tracker, verbose: u8) -> Result<()> {
	let cc_weekly = ccusage::fetch(Granularity::Weekly).context("Failed to fetch ccusage weekly data")?;
...
fn print_period_table(periods: &[PeriodEconomics], verbose: u8) {
	...
	if verbose > 0 {
		...
		for p in periods {
			...
			let saved = p.rtk_saved_tokens.map(format_tokens).unwrap_or_else(|| "—".to_string());
			let weighted = p.savings_weighted.map(format_usd).unwrap_or_else(|| "—".to_string());
			let active = p.savings_active.map(format_usd).unwrap_or_else(|| "—".to_string());
			let blended = p.savings_blended.map(format_usd).unwrap_or_else(|| "—".to_string());
			let cmds = p.rtk_commands.map(|c| c.to_string()).unwrap_or_else(|| "—".to_string());
		...
		for p in periods {
			...
			println!("{:<12} {:>10} {:>10} {:>10} {:>12}", p.label, spent, saved, weighted, cmds);
		}
	}
	println!();
}
...
fn export_json(tracker: &Tracker, daily: bool, weekly: bool, monthly: bool, all: bool) -> Result<()> {
	...
	let mut export = Export { daily: None, weekly: None, monthly: None, totals: None };

	if all || daily {
		let cc = ccusage::fetch(Granularity::Daily).context("Failed to fetch ccusage daily data for JSON export")?;
		let rtk = tracker
			.get_all_days()
	...
	if all || weekly {
		let cc = ccusage::fetch(Granularity::Weekly).context("Failed to fetch ccusage weekly data for export")?;
		let rtk = tracker
			.get_by_week()
	...
	if all || monthly {
		let cc = ccusage::fetch(Granularity::Monthly).context("Failed to fetch ccusage monthly data for export")?;
		let rtk = tracker
			.get_by_month()
	...
	println!(
		"{}",
		serde_json::to_string_pretty(&export).context("Failed to serialize economics data to JSON")?
	);
	Ok(())
}
...
fn print_csv_row(p: &PeriodEconomics) {
	...
	let output_tokens = p.cc_output_tokens.map(|t| t.to_string()).unwrap_or_default();
	let cache_create = p.cc_cache_create_tokens.map(|t| t.to_string()).unwrap_or_default();
	let cache_read = p.cc_cache_read_tokens.map(|t| t.to_string()).unwrap_or_default();
	let active_tokens = p.cc_active_tokens.map(|t| t.to_string()).unwrap_or_default();
	let total_tokens = p.cc_total_tokens.map(|t| t.to_string()).unwrap_or_default();
	let saved_tokens = p.rtk_saved_tokens.map(|t| t.to_string()).unwrap_or_default();
	let weighted_savings = p.savings_weighted.map(|s| format!("{:.4}", s)).unwrap_or_default();
	let active_savings = p.savings_active.map(|s| format!("{:.4}", s)).unwrap_or_default();
	let blended_savings = p.savings_blended.map(|s| format!("{:.4}", s)).unwrap_or_default();
...
mod tests {
	...
	fn test_convert_saturday_to_monday() {
		...
		assert_eq!(convert_saturday_to_monday("2026-01-18"), Some("2026-01-20".to_string()));
