...
pub fn fetch(granularity: Granularity) -> Result<Option<Vec<CcusagePeriod>>> {
	...
	cmd.arg(subcommand).arg("--json").arg("--since").arg("20250101"); // 90 days back approx
	...
	if !result.success() {
		eprintln!("[warn] ccusage exited with {}: {}", result.exit_code, result.stderr.trim());
		return Ok(None);
	}

	let periods = parse_json(&result.stdout, granularity).context("Failed to parse ccusage JSON output")?;
...
fn parse_json(json: &str, granularity: Granularity) -> Result<Vec<CcusagePeriod>> {
	match granularity {
		Granularity::Daily => {
			let resp: DailyResponse = serde_json::from_str(json).context("Invalid JSON structure for daily data")?;
			Ok(resp
				.daily
				.into_iter()
				.map(|e| CcusagePeriod { key: e.date, metrics: e.metrics })
				.collect())
		},
		Granularity::Weekly => {
			let resp: WeeklyResponse = serde_json::from_str(json).context("Invalid JSON structure for weekly data")?;
			Ok(resp
				.weekly
				.into_iter()
				.map(|e| CcusagePeriod { key: e.week, metrics: e.metrics })
				.collect())
		},
		Granularity::Monthly => {
			...
			Ok(resp
				.monthly
				.into_iter()
				.map(|e| CcusagePeriod { key: e.month, metrics: e.metrics })
				.collect())
		},
	}
}
