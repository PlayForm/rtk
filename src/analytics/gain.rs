...
pub fn run(
	...
	if reset {
		...
		tracker.reset_all().context("Failed to reset token savings")?;
	...
	if !daily && !weekly && !monthly && !all {
		...
		print_kpi(
			"Tokens saved",
			format!("{} ({:.1}%)", format_tokens(summary.total_saved), summary.avg_savings_pct),
		);
		print_kpi(
			"Total exec time",
			format!(
		...
		match hook_check::status() {
			...
			hook_check::HookStatus::Outdated => {
				eprintln!("{}", "[warn] Hook outdated — run `rtk init -g` to update".yellow());
				eprintln!();
			},
		...
		if !summary.by_command.is_empty() {
			...
			let max_saved = summary.by_command.iter().map(|(_, _, saved, _, _)| *saved).max().unwrap_or(1);
			...
			for (idx, (cmd, count, saved, pct, avg_time)) in summary.by_command.iter().enumerate() {
				...
				let saved_cell = format!("{:>saved_width$}", format_tokens(*saved), saved_width = saved_width);
				let pct_plain = format!("{:>6}", format!("{pct:.1}%"));
				let pct_cell = colorize_pct_cell(*pct, &pct_plain); // added: color-coded percentage
				let time_cell = format!("{:>time_width$}", format_duration(*avg_time), time_width = time_width);
		...
		if quota {
			...
			print_kpi("Tokens saved (lifetime)", format_tokens(summary.total_saved));
...
fn styled(text: &str, strong: bool) -> String {
	...
	if strong { text.bold().green().to_string() } else { text.to_string() }
}
...
fn mini_bar(value: usize, max: usize, width: usize) -> String {
	...
	if std::io::stdout().is_terminal() { bar.cyan().to_string() } else { bar }
}
...
fn shorten_path(path: &str) -> String {
	...
	if root == "/" || root.is_empty() {
		...
		format!("{}/.../{}/{}", root, comps[comps.len() - 2], comps[comps.len() - 1])
	}
}
...
fn confirm_reset() -> Result<bool> {
	...
	stdin.lock().read_line(&mut line).context("Failed to read confirmation")?;
