...
impl ClaudeProvider {
	...
	fn discover_sessions_in_projects_dir(
		...
		let entries =
			fs::read_dir(projects_dir).with_context(|| format!("failed to read {}", projects_dir.display()))?;
		...
		for entry in entries.flatten() {
			...
			for walk_entry in WalkDir::new(&path).follow_links(false).into_iter().filter_map(|e| e.ok()) {
	...
	pub fn encode_project_path(path: &str) -> String {
		...
		path.chars()
			.map(|c| if !c.is_ascii() || SANITIZED_CHARS.contains(&c) { '-' } else { c })
			.collect()
	}
}
						...
						for block in content {
							if block.get("type").and_then(|t| t.as_str()) == Some("tool_result") {
								if let Some(id) = block.get("tool_use_id").and_then(|i| i.as_str()) {
									...
									tool_results.insert(id.to_string(), (output_len, content_preview, is_error));
								}
							}
						}
					}
...
mod tests {
	...
	fn test_extract_assistant_bash() {
		...
		assert_eq!(cmds[0].output_len.unwrap(), "On branch master\nnothing to commit".len());
	}
	...
	fn test_extract_non_message_ignored() {
		let jsonl = make_jsonl(&[r#"{"type":"file-history-snapshot","messageId":"abc","snapshot":{}}"#]);
	...
	fn test_encode_project_path() {
		assert_eq!(ClaudeProvider::encode_project_path("/Users/foo/bar"), "-Users-foo-bar");
	}
	...
	fn test_encode_project_path_multiple_dots() {
		assert_eq!(ClaudeProvider::encode_project_path("/Users/a.b.c/proj"), "-Users-a-b-c-proj");
	}
	...
	fn test_encode_project_path_windows() {
		...
		assert_eq!(ClaudeProvider::encode_project_path(r"C:\Users\foo\bar"), "C:-Users-foo-bar");
	}
	...
	fn test_encode_path_with_spaces() {
		...
		assert_eq!(
			ClaudeProvider::encode_project_path(r"/home/user/projects/[QZX-7K42] - Análise Genérica de Exemplo"),
	...
	fn test_discover_sessions_missing_projects_dir_returns_empty() {
		...
		let missing_projects_dir = temp_home.path().join(crate::hooks::constants::CLAUDE_DIR).join("projects");

		let sessions =
			ClaudeProvider::discover_sessions_in_projects_dir(&missing_projects_dir, None, Some(30)).unwrap();
	...
	fn test_discover_sessions_applies_project_filter() {
		...
		let sessions =
			ClaudeProvider::discover_sessions_in_projects_dir(projects_dir.path(), Some("rtk"), None).unwrap();

		assert_eq!(sessions.len(), 1);
		assert_eq!(sessions[0].file_name().and_then(|name| name.to_str()), Some("matching.jsonl"));
	}
	...
	fn test_discover_sessions_existing_non_directory_returns_error() {
		...
		let err = ClaudeProvider::discover_sessions_in_projects_dir(projects_file.path(), None, None).unwrap_err();
