...
use crate::hooks::constants::{
	COPILOT_HOOK_FILE, CURSOR_DIR, GITHUB_DIR, HERMES_DIR, HERMES_PLUGINS_SUBDIR, HERMES_PLUGIN_MANIFEST_FILE,
	HERMES_PLUGIN_NAME, HOOKS_SUBDIR, REWRITE_HOOK_FILE,
};
...
impl AgentIntegrationStatus {
	pub fn detect() -> Self {
		let mut status = dirs::home_dir().map(|home| Self::detect_from_home(&home)).unwrap_or_default();
	...
	fn detect_from_home(home: &Path) -> Self {
		Self {
			cursor_hook_installed: home.join(CURSOR_DIR).join(HOOKS_SUBDIR).join(REWRITE_HOOK_FILE).exists(),
	...
	fn copilot_hook_installed_in(dir: &Path) -> bool {
		dir.join(GITHUB_DIR).join(HOOKS_SUBDIR).join(COPILOT_HOOK_FILE).exists()
	}
}
...
impl DiscoverReport {
	pub fn total_saveable_tokens(&self) -> usize {
		self.supported.iter().map(|s| s.estimated_savings_tokens).sum()
	}
...
mod tests {
	...
	fn test_already_rtk_percent_shows_decimal() {
		...
		assert!(output.contains("0.3%"), "Expected '0.3%' in output but got:\n{}", output);
		assert!(
			!output.contains("(0%)"),
	...
	fn test_format_text_reports_hermes_plugin_detected() {
		...
		report.agent_status =
			AgentIntegrationStatus { hermes_plugin_installed: true, ..AgentIntegrationStatus::default() };
	...
	fn test_agent_status_detects_copilot_hook_in_project() {
		...
		let hook = temp.path().join(GITHUB_DIR).join(HOOKS_SUBDIR).join(COPILOT_HOOK_FILE);
		std::fs::create_dir_all(hook.parent().unwrap()).unwrap();
		std::fs::write(&hook, "{}").unwrap();

		assert!(AgentIntegrationStatus::copilot_hook_installed_in(temp.path()));
	...
	fn test_format_text_reports_copilot_detected() {
		let mut report = make_report(0, 0);
		report.agent_status =
			AgentIntegrationStatus { copilot_hook_installed: true, ..AgentIntegrationStatus::default() };
