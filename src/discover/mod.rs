...
use registry::{category_avg_tokens, classify_command, split_command_chain, strip_disabled_prefix, Classification};
...
pub fn run(project: Option<&str>, all: bool, since_days: u64, limit: usize, format: &str, verbose: u8) -> Result<()> {
		...
		for ext_cmd in &extracted {
			...
			for part in parts {
				...
				match classify_command(part) {
					Classification::Supported { rtk_equivalent, category, estimated_savings_pct, status } => {
						let bucket = supported_map.entry(rtk_equivalent).or_insert_with(|| SupportedBucket {
							rtk_equivalent,
							category,
							count: 0,
					...
					Classification::Unsupported { base_command } => {
						let bucket = unsupported_map
							.entry(base_command)
							.or_insert_with(|| UnsupportedBucket { count: 0, example: part.to_string() });
						bucket.count += 1;
					},
	...
	let mut unsupported: Vec<UnsupportedEntry> = unsupported_map
		.into_iter()
		.map(|(base, bucket)| UnsupportedEntry { base_command: base, count: bucket.count, example: bucket.example })
		.collect();
