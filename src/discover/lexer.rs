	...
	while let Some(c) = chars.next() {
		...
		match c {
			'$' => {
				...
				let start = byte_pos;
				byte_pos += char_len;
				if chars.peek().is_some_and(|&nc| nc.is_ascii_alphabetic() || nc == '_') {
					...
					tokens.push(ParsedToken { kind: TokenKind::Arg, value: name, offset: start });
				} else {
					tokens.push(ParsedToken { kind: TokenKind::Shellism, value: "$".into(), offset: start });
				}
				current_start = byte_pos;
			},
			'*' | '?' | '`' | '(' | ')' | '{' | '}' | '!' => {
				flush_arg(&mut tokens, &mut current, current_start);
				tokens.push(ParsedToken { kind: TokenKind::Shellism, value: c.to_string(), offset: byte_pos });
				byte_pos += char_len;
				current_start = byte_pos;
			},
			'|' => {
				...
				if chars.peek() == Some(&'|') {
					chars.next();
					byte_pos += 1;
					tokens.push(ParsedToken { kind: TokenKind::Operator, value: "||".into(), offset: start });
				} else {
					tokens.push(ParsedToken { kind: TokenKind::Pipe, value: "|".into(), offset: start });
				}
				current_start = byte_pos;
			},
			';' => {
				flush_arg(&mut tokens, &mut current, current_start);
				tokens.push(ParsedToken { kind: TokenKind::Operator, value: ";".into(), offset: byte_pos });
				byte_pos += char_len;
				current_start = byte_pos;
			},
			'&' => {
				...
				if chars.peek() == Some(&'&') {
					...
					tokens.push(ParsedToken { kind: TokenKind::Redirect, value: val, offset: start });
				} else {
					tokens.push(ParsedToken { kind: TokenKind::Shellism, value: "&".into(), offset: start });
				}
				current_start = byte_pos;
			...
			'>' => {
				let fd_prefix = if !current.is_empty() && current.chars().all(|ch| ch.is_ascii_digit()) {
					Some(std::mem::take(&mut current))
				} else {
					flush_arg(&mut tokens, &mut current, current_start);
					None
				};
				let redir_start = if fd_prefix.is_some() { current_start } else { byte_pos };
				...
				tokens.push(ParsedToken { kind: TokenKind::Redirect, value: val, offset: redir_start });
				current_start = byte_pos;
			},
			'<' => {
				flush_arg(&mut tokens, &mut current, current_start);
			...
			'\n' | '\r' if emit_newline => {
				flush_arg(&mut tokens, &mut current, current_start);
				tokens.push(ParsedToken { kind: TokenKind::Operator, value: "\n".into(), offset: byte_pos });
				byte_pos += char_len;
				current_start = byte_pos;
			},
...
fn flush_arg(tokens: &mut Vec<ParsedToken>, current: &mut String, offset: usize) {
	if !current.is_empty() {
		tokens.push(ParsedToken { kind: TokenKind::Arg, value: std::mem::take(current), offset });
	}
}
...
fn contains_substitution(cmd: &str) -> bool {
	...
	while i < bytes.len() {
		match bytes[i] {
			...
			b'<' | b'>' if !in_single && !in_double && bytes.get(i + 1) == Some(&b'(') => return true,
			_ => {},
		}
		i += 1;
	}
...
pub fn strip_quotes(s: &str) -> String {
	...
	if chars.len() >= 2
		&& ((chars[0] == '"' && chars[chars.len() - 1] == '"') || (chars[0] == '\'' && chars[chars.len() - 1] == '\''))
	{
...
mod tests {
	...
	fn test_quoted_operator_not_split() {
		...
		assert!(!tokens.iter().any(|t| matches!(t.kind, TokenKind::Operator) && t.value == "&&"));
	...
	fn test_multiple_pipes() {
		let tokens = tokenize("a | b | c");
		let pipes: Vec<_> = tokens.iter().filter(|t| t.kind == TokenKind::Pipe).collect();
	...
	fn test_simple_var_is_arg() {
		...
		assert!(
			tokens.iter().any(|t| t.kind == TokenKind::Arg && t.value == "$HOME"),
	...
	fn test_subshell_detection() {
		let tokens = tokenize("echo $(date)");
		let shellisms: Vec<_> = tokens.iter().filter(|t| t.kind == TokenKind::Shellism).collect();
	...
	fn test_escaped_glob() {
		let tokens = tokenize("echo \\*.txt");
		assert!(!tokens.iter().any(|t| t.kind == TokenKind::Shellism && t.value == "*"));
	}
	...
	fn test_redirect_append() {
		let tokens = tokenize("cmd >> file");
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Redirect && t.value == ">>"));
	}
	...
	fn test_redirect_stderr_no_space() {
		...
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Redirect && t.value == "2>"));
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Arg && t.value == "/dev/null"));
	}
	...
	fn test_redirect_1_to_2_single_token() {
		...
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Redirect && t.value == "1>&2"));
	}
	...
	fn test_combined_redirect_chain() {
		...
		let redirects: Vec<_> = tokens.iter().filter(|t| t.kind == TokenKind::Redirect).collect();
	...
	fn test_redirect_2_to_1_with_pipe() {
		...
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Redirect && t.value == "2>&1"));
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Pipe));
	}
	...
	fn test_semicolon_no_space() {
		...
		assert_eq!(tokens.iter().filter(|t| t.kind == TokenKind::Arg).count(), 4);
	}
	...
	fn test_find_pipe_xargs() {
		...
		let pipe_idx = tokens.iter().position(|t| t.kind == TokenKind::Pipe).unwrap();
		assert!(pipe_idx > 0);
		let before_pipe: Vec<_> = tokens[..pipe_idx].iter().filter(|t| t.kind == TokenKind::Arg).collect();
	...
	fn test_fd_redirect_needs_adjacent_digit() {
		...
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Arg && t.value == "2"));
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Redirect && t.value == ">"));
	}
	...
	fn test_fd_redirect_no_space() {
		let tokens = tokenize("echo 2>file");
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Redirect && t.value == "2>"));
		assert!(tokens.iter().any(|t| t.kind == TokenKind::Arg && t.value == "file"));
	}
	...
	fn test_shell_split_mixed_quotes() {
		assert_eq!(shell_split(r#"echo "it's" 'a "test"'"#), vec!["echo", "it's", "a \"test\""]);
	}
	...
	fn test_split_on_operators_through_pipes() {
		...
		assert_eq!(split_on_operators("a && b | c ; d", false), vec!["a", "b", "c", "d"]);
	}
	...
	fn test_unattestable_command_substitution() {
		assert!(contains_unattestable_construct("git log --pretty=$(rm -rf ~)"));
	}
	...
	fn test_unattestable_substitution_inside_double_quotes() {
		assert!(contains_unattestable_construct(r#"git log --pretty="$(rm -rf ~)""#));
		assert!(contains_unattestable_construct(r#"git log --pretty="`rm -rf ~`""#));
		assert!(contains_unattestable_construct(r#"git -c x="$(whoami)" status"#));
	}
	...
	fn test_split_perms_operators() {
		...
		assert_eq!(split_for_permissions("git log | head"), vec!["git log", "head"]);
	}
	...
	fn test_split_perms_background_ampersand() {
		assert_eq!(split_for_permissions("git status & rm -rf ~"), vec!["git status", "rm -rf ~"]);
	...
	fn test_split_perms_truncates_at_redirect() {
		...
		assert_eq!(split_for_permissions("git push --force 2>&1"), vec!["git push --force"]);
	}
