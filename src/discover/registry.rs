...
pub fn classify_command(cmd: &str) -> Classification {
	...
	if cmd_clean.starts_with("cat ") || cmd_clean.starts_with("head ") || cmd_clean.starts_with("tail ") {
		...
		if has_redirect {
			return Classification::Unsupported {
				base_command: cmd_clean.split_whitespace().next().unwrap_or("cat").to_string(),
			};
		}
	}
	...
	if let Some(&idx) = matches.last() {
		...
		if base.is_empty() {
			...
			Classification::Unsupported { base_command: base.to_string() }
		}
	}
}
			...
			if !second.starts_with('-') && !second.contains('/') && !second.contains('.') {
				...
				let end = cmd
					...
					.and_then(|i| {
						...
						trimmed.find(char::is_whitespace).map(|j| i + (rest.len() - trimmed.len()) + j)
					})
					.unwrap_or(cmd.len());
				&cmd[..end]
			} else {
...
fn parse_golangci_run_parts(cmd: &str) -> Option<GolangciRunParts<'_>> {
	...
	while i < tokens.len() {
		...
		if !token.starts_with('-') {
			if token == "run" {
				let global_segment = if i > 1 { cmd[tokens[1].1..tokens[i].1].trim() } else { "" };
				let run_segment = cmd[tokens[i].1..].trim();
				return Some(GolangciRunParts { global_segment, run_segment });
			}
			return None;
		}
...
fn rewrite_compound(cmd: &str, excluded: &[ExcludePattern], transparent_prefixes: &[String]) -> Option<String> {
	let tokens = tokenize(cmd);
	...
	for tok in &tokens {
		...
		match tok.kind {
			TokenKind::Operator => {
				...
				let rewritten = rewrite_segment(seg, excluded, transparent_prefixes).unwrap_or_else(|| seg.to_string());
				if rewritten != seg {
					any_changed = true;
				}
			...
			TokenKind::Pipe => {
				let seg = cmd[seg_start..tok.offset].trim();
				let is_pipe_incompatible =
					seg.starts_with("find ") || seg == "find" || seg.starts_with("fd ") || seg == "fd";
				...
				let pipe_group_end = tokens.iter().find(|t| {
					t.offset > tok.offset
						&& (t.kind == TokenKind::Operator || (t.kind == TokenKind::Shellism && t.value == "&"))
				});
...
fn rewrite_line_range(cmd: &str) -> Option<String> {
	...
	for re in [&*TAIL_N, &*TAIL_N_SPACE, &*TAIL_LINES_EQ, &*TAIL_LINES_SPACE] {
		if let Some(caps) = re.captures(cmd) {
			let n = caps.get(1)?.as_str();
			let file = caps.get(2)?.as_str();
	...
	patterns
		...
		.filter_map(|pattern| {
			...
			if trimmed.is_empty() || trimmed == "^" {
				eprintln!("rtk: warning: ignoring trivial exclude_commands pattern '{}'", pattern);
				return None;
			}
			...
			Some(match Regex::new(&anchored) {
				...
				Err(e) => {
					eprintln!("rtk: warning: invalid exclude_commands pattern '{}': {}", pattern, e);
...
fn rewrite_segment(seg: &str, excluded: &[ExcludePattern], transparent_prefixes: &[String]) -> Option<String> {
...
fn rewrite_segment_inner(
	...
	if !env_prefix.is_empty() {
		...
		let rewritten = rewrite_segment_inner(rest_after_env, excluded, transparent_prefixes, depth + 1)?;
	...
	if let Some(parts) = parse_golangci_run_parts(cmd_part) {
		let rewritten = if parts.global_segment.is_empty() {
			...
			format!("rtk golangci-lint {} {}", parts.global_segment, parts.run_segment)
		};
	...
	if rule.rtk_cmd == "rtk gh" {
		...
		if args_lower.contains("--json") || args_lower.contains("--jq") || args_lower.contains("--template") {
			return None;
		}
	}
...
fn strip_word_prefix<'a>(cmd: &'a str, prefix: &str) -> Option<&'a str> {
	if cmd == prefix {
		Some("")
	} else if cmd.len() > prefix.len() && cmd.starts_with(prefix) && cmd.as_bytes()[prefix.len()] == b' ' {
...
mod tests {
	...
	fn test_classify_echo_ignored() {
		assert_eq!(classify_command("echo hello world"), Classification::Ignored);
	}
	...
	fn test_registry_covers_all_git_subcommands() {
		...
		for subcmd in [
			"status", "log", "diff", "show", "add", "commit", "push", "pull", "branch", "fetch", "stash", "worktree",
		] {
	...
	fn test_split_quoted_and() {
		assert_eq!(split_command_chain(r#"echo "a && b""#), vec![r#"echo "a && b""#]);
	}
	...
	fn test_rewrite_git_status() {
		assert_eq!(rewrite_command_no_prefixes("git status", &[]), Some("rtk git status".into()));
	}

	#[test]
	fn test_rewrite_git_log() {
		assert_eq!(rewrite_command_no_prefixes("git log -10", &[]), Some("rtk git log -10".into()));
	}
	...
	fn test_classify_git_dash_c() {
		...
		assert!(
			matches!(result, Classification::Supported { rtk_equivalent: "rtk git", .. }),
	...
	fn test_rewrite_cargo_test() {
		assert_eq!(rewrite_command_no_prefixes("cargo test", &[]), Some("rtk cargo test".into()));
	}
	...
	fn test_rewrite_compound_three_segments() {
		assert_eq!(
			rewrite_command_no_prefixes("cargo fmt --all && cargo clippy --all-targets && cargo test", &[]),
	...
	fn test_rewrite_cat_with_incompatible_flags_skipped() {
		...
		assert_eq!(rewrite_command_no_prefixes("cat --show-all file.txt", &[]), None);
	}
	...
	fn test_rewrite_find_pipe_xargs_wc() {
		assert_eq!(rewrite_command_no_prefixes("find src -type f | wc -l", &[]), None);
	}
	...
	fn test_rewrite_rtk_disabled_subprocess_warns() {
		...
		let rtk_mtime = std::fs::metadata(&rtk_bin).ok().and_then(|m| m.modified().ok());
		...
		assert!(!output.status.success(), "Should exit non-zero (no rewrite)");
	...
	fn test_rewrite_env_quoted_value_with_spaces() {
		assert_eq!(
			rewrite_command_no_prefixes(r#"GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" git push"#, &[]),
	...
	fn test_rewrite_redirect_quotes_not_stripped() {
		...
		assert!(result.is_some(), "Should still rewrite even with apostrophe");
	}
	...
	fn test_rewrite_head_other_flag_skipped() {
		...
		assert_eq!(rewrite_command_no_prefixes("head -c 100 src/main.rs", &[]), None);
	}
	...
	fn test_rewrite_tail_other_flag_skipped() {
		assert_eq!(rewrite_command_no_prefixes("tail -c 100 src/main.rs", &[]), None);
	}
	...
	fn test_rewrite_head_numeric_flag_multi_file_skipped() {
		assert_eq!(rewrite_command_no_prefixes("head -3 /tmp/a /tmp/b /tmp/c", &[]), None);
	}

	#[test]
	fn test_rewrite_head_lines_long_flag_multi_file_skipped() {
		assert_eq!(rewrite_command_no_prefixes("head --lines=50 src/main.rs src/lib.rs", &[]), None);
	}

	#[test]
	fn test_rewrite_tail_numeric_flag_multi_file_skipped() {
		assert_eq!(rewrite_command_no_prefixes("tail -20 a.log b.log", &[]), None);
	}

	#[test]
	fn test_rewrite_tail_n_space_flag_multi_file_skipped() {
		assert_eq!(rewrite_command_no_prefixes("tail -n 12 a.log b.log c.log", &[]), None);
	}

	#[test]
	fn test_rewrite_tail_lines_eq_multi_file_skipped() {
		assert_eq!(rewrite_command_no_prefixes("tail --lines=7 a.log b.log", &[]), None);
	}
	...
	fn test_classify_gh_release() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk gh", .. }
		));
	}
	...
	fn test_classify_glab_mr() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk glab", .. }
		));
	}
	...
	fn test_classify_cargo_install() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk cargo", .. }
		));
	}
	...
	fn test_classify_docker_run() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk docker", .. }
		));
	}
	...
	fn test_classify_kubectl_describe() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk kubectl", .. }
		));
	}
	...
	fn test_classify_tree() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk tree", .. }
		));
	}
	...
	fn test_classify_diff() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk diff", .. }
		));
	}

	#[test]
	fn test_rewrite_tree() {
		assert_eq!(rewrite_command_no_prefixes("tree src/", &[]), Some("rtk tree src/".into()));
	}
	...
	fn test_rewrite_docker_compose_up_skipped() {
		assert_eq!(rewrite_command_no_prefixes("docker compose up -d", &[]), None);
	}

	#[test]
	fn test_rewrite_docker_compose_down_skipped() {
		assert_eq!(rewrite_command_no_prefixes("docker compose down", &[]), None);
	}
	...
	fn test_classify_aws() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk aws", .. }
		));
	}
	...
	fn test_classify_psql() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk psql", .. }
		));
	}
	...
	#[test]
	fn test_rewrite_aws() {
		assert_eq!(rewrite_command_no_prefixes("aws s3 ls", &[]), Some("rtk aws s3 ls".into()));
	}
	...
	fn test_classify_ruff_check() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk ruff", .. }
		));
	}
	...
	fn test_classify_pytest() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk pytest", .. }
		));
	}
	...
	fn test_classify_pip_list() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk pip", .. }
		));
	}
	...
	fn test_rewrite_pip_list() {
		assert_eq!(rewrite_command_no_prefixes("pip list", &[]), Some("rtk pip list".into()));
	}
	...
	fn test_rewrite_uv_pip_list() {
		assert_eq!(rewrite_command_no_prefixes("uv pip list", &[]), Some("rtk pip list".into()));
	}
	...
	fn test_classify_go_test() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk go", .. }
		));
	}
	...
	fn test_classify_golangci_lint() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk golangci-lint run", .. }
		));
	}
	...
	fn test_rewrite_other_golangci_lint_subcommand_skips_compact_wrapper() {
		assert_eq!(rewrite_command_no_prefixes("golangci-lint version", &[]), None);
	}
		...
		for command in commands {
			assert!(
				matches!(
					...
					Classification::Supported { rtk_equivalent: "rtk lint", .. }
				),
				"Failed for command: {}",
				command
			);
	...
	fn test_classify_jest() {
		...
		for command in commands {
			assert!(
				matches!(
					classify_command(command),
					Classification::Supported { rtk_equivalent: "rtk jest", .. }
				),
				"Failed for command: {}",
				command
			);
	...
	fn test_classify_vitest() {
		...
		for command in commands {
			assert!(
				matches!(
					classify_command(command),
					Classification::Supported { rtk_equivalent: "rtk vitest", .. }
				),
				"Failed for command: {}",
				command
			);
		...
		for command in commands {
			assert!(
				matches!(
					...
					Classification::Supported { rtk_equivalent: "rtk prisma", .. }
				),
				"Failed for command: {}",
				command
			);
	...
	fn test_rewrite_pnpm_command() {
		let commands = vec!["exec", "i", "install", "list", "ls", "outdated", "run", "run-script"];
		for command in commands {
			assert_eq!(
	...
	#[test]
	fn test_rewrite_npx() {
		assert_eq!(rewrite_command_no_prefixes("npx svgo", &[]), Some("rtk npx svgo".to_string()),);
	}
	...
	fn test_classify_gradlew() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk gradlew", .. }
		));
	}
	...
	fn test_classify_mvn_test() {
		assert!(matches!(
			...
			Classification::Supported { rtk_equivalent: "rtk mvn", .. }
		));
	}
	...
	fn test_rewrite_mvnw_test() {
		assert_eq!(rewrite_command_no_prefixes("./mvnw test", &[]), Some("rtk mvn test".into()));
	}
	...
	fn test_rewrite_compound_four_segments() {
		assert_eq!(
			rewrite_command_no_prefixes("cargo fmt --all && cargo clippy && cargo test && git status", &[]),
			Some("rtk cargo fmt --all && rtk cargo clippy && rtk cargo test && rtk git status".into())
		);
	}
	...
	fn test_all_rules_are_complete() {
		for rule in RULES {
			assert!(!rule.pattern.is_empty(), "Rule '{}' has empty pattern", rule.rtk_cmd);
	...
	fn test_exclude_subcommand_pattern() {
		...
		assert_eq!(rewrite_command_no_prefixes("git push origin main", &excluded), None);
	}

	#[test]
	fn test_exclude_regex_pattern() {
		let excluded = vec!["^curl".to_string()];
		assert_eq!(rewrite_command_no_prefixes("curl http://example.com", &excluded), None);
	}
	...
	fn test_rewrite_gh_template_skipped() {
		assert_eq!(rewrite_command_no_prefixes("gh pr view 42 --template '{{.title}}'", &[]), None);
	}

	#[test]
	fn test_rewrite_gh_api_json_skipped() {
		assert_eq!(rewrite_command_no_prefixes("gh api repos/owner/repo --jq '.name'", &[]), None);
	}

	#[test]
	fn test_rewrite_gh_without_json_still_works() {
		assert_eq!(rewrite_command_no_prefixes("gh pr list", &[]), Some("rtk gh pr list".into()));
	}
	...
	fn test_cmd_has_rtk_disabled_prefix() {
		...
		assert!(cmd_has_rtk_disabled_prefix("FOO=1 RTK_DISABLED=1 cargo test"));
		assert!(cmd_has_rtk_disabled_prefix("RTK_DISABLED=true git log --oneline"));
	...
	fn test_strip_golangci_global_opts_helper() {
		...
		assert_eq!(strip_golangci_global_opts("golangci-lint version"), "golangci-lint version");
	...
	fn test_rewrite_wc_multi_file() {
		assert_eq!(rewrite_command_no_prefixes("wc src/*.rs", &[]), Some("rtk wc src/*.rs".into()));
	}
	...
	fn test_shell_prefix_unknown_inner() {
		assert_eq!(rewrite_command_no_prefixes("noglob unknown_cmd --flag", &[]), None);
	}
	...
	fn test_transparent_prefix_unknown_inner_returns_none() {
		let prefixes = vec!["shadowenv exec --".to_string()];
		assert_eq!(super::rewrite_command("shadowenv exec -- htop", &[], &prefixes), None);
	}

	#[test]
	fn test_transparent_prefix_not_matched_is_passthrough() {
		// Without the prefix configured, the wrapper breaks routing.
		assert_eq!(super::rewrite_command("shadowenv exec -- git status", &[], &[]), None);
	}
	...
	fn test_transparent_prefix_whole_word_matching() {
		...
		assert_eq!(super::rewrite_command("foobar git status", &[], &prefixes), None);
	}

	#[test]
	fn test_transparent_prefix_empty_rest_returns_none() {
		let prefixes = vec!["shadowenv exec --".to_string()];
		assert_eq!(super::rewrite_command("shadowenv exec --", &[], &prefixes), None);
	}
	...
	fn test_transparent_prefix_inside_compound() {
		...
		let prefixes = vec!["shadowenv exec --".to_string()];
		assert_eq!(
			super::rewrite_command("shadowenv exec -- git status && shadowenv exec -- cargo test", &[], &prefixes),
	...
	#[test]
	fn test_gt_graphite() {
		assert_eq!(rewrite_command_no_prefixes("gt log", &[]), Some("rtk gt log".into()));
	}

	#[test]
	fn test_command_no_longer_ignored() {
		assert_ne!(classify_command("command git status"), Classification::Ignored);
	}
	...
	fn test_rewrite_env_pipe_then_and() {
		assert_eq!(
			rewrite_command_no_prefixes("RUST_BACKTRACE=1 cargo test 2>&1 | grep FAILED && git stash", &[]),
