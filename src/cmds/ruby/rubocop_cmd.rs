...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	let has_format = args.iter().any(|a| a.starts_with("--format") || a.starts_with("-f"));
...
fn filter_rubocop_json(output: &str) -> String {
	...
	let correctable_count = if s.correctable_offense_count > 0 {
		...
		rubocop.files.iter().flat_map(|f| &f.offenses).filter(|o| o.correctable).count()
	};

	let mut result = format!("rubocop: {} offenses ({} files)\n", s.offense_count, s.inspected_file_count);

	// Build list of files with offenses, sorted by worst severity then file path
	let mut files_with_offenses: Vec<&RubocopFile> = rubocop.files.iter().filter(|f| !f.offenses.is_empty()).collect();

	// Sort files: worst severity first, then alphabetically
	files_with_offenses.sort_by(|a, b| {
		let a_worst = a.offenses.iter().map(|o| severity_rank(&o.severity)).min().unwrap_or(3);
		let b_worst = b.offenses.iter().map(|o| severity_rank(&o.severity)).min().unwrap_or(3);
	...
	if files_with_offenses.len() > max_files {
		...
		let all_files = files_with_offenses
			...
			.collect::<Vec<_>>()
			.join("\n");
		if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_files, "rubocop-files", max_files + 1) {
			result.push_str(&format!("  {}\n", hint));
		}
	}

	if correctable_count > 0 {
		result.push_str(&format!("\n({} correctable, run `rubocop -A`)", correctable_count));
	}
...
fn filter_rubocop_text(output: &str) -> String {
	...
	for line in output.lines() {
			...
			|| t.starts_with("rubocop: No such file")
		{
			...
			if total_lines > 20 {
				return format!("RuboCop error:\n{}\n... ({} more lines)", truncated, total_lines - 20);
			}
			return format!("RuboCop error:\n{}", truncated);
		}
	}
	...
	for line in output.lines().rev() {
		...
		if t.contains("inspected") && t.contains("autocorrected") {
			...
			if files > 0 && corrected > 0 {
				return format!("ok ✓ rubocop -A ({} files, {} autocorrected)", files, corrected);
			}
...
mod tests {
	...
	fn test_filter_rubocop_severity_ordering() {
		...
		assert!(ctrl_pos < model_pos, "Error-file should appear before convention-file");
		...
		assert!(error_pos < conv_pos, "Error offense should appear before convention");
	}
...
15 files inspected, 3 offenses detected, 3 offenses autocorrected"#;
	...
	fn test_compact_ruby_path() {
		assert_eq!(compact_ruby_path("/home/user/project/app/models/user.rb"), "app/models/user.rb");
		assert_eq!(
	...
	fn test_filter_rubocop_text_bundler_error() {
		...
		let result = filter_rubocop_text(text);
		assert!(result.starts_with("RuboCop error:"), "should detect Bundler error: {}", result);
		assert!(result.contains("GemNotFound"));
	}

	#[test]
	fn test_filter_rubocop_text_load_error() {
		let text = "/usr/lib/ruby/3.2.0/rubygems.rb:250: cannot load such file -- rubocop (LoadError)";
		let result = filter_rubocop_text(text);
		assert!(result.starts_with("RuboCop error:"), "should detect load error: {}", result);
	}
