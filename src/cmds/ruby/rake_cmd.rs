...
fn select_runner(args: &[String]) -> (&'static str, Vec<String>) {
	...
	if needs_rails { ("rails", args.to_vec()) } else { ("rake", args.to_vec()) }
}
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	if verbose > 0 {
		eprintln!("Running: {} {}", cmd.get_program().to_string_lossy(), effective_args.join(" "));
	}
...
fn filter_minitest_output(output: &str) -> String {
	...
	for line in clean.lines() {
		...
		if (trimmed.contains(" runs,") || trimmed.contains(" tests,")) && trimmed.contains(" assertions,") {
...
6 runs, 5 assertions, 2 failures, 1 errors, 0 skips"#;
	...
	fn test_select_runner_multiple_files_uses_rails() {
		let (tool, a) = select_runner(&args("test test/models/post_test.rb test/models/user_test.rb"));
		assert_eq!(tool, "rails");
		assert_eq!(a, args("test test/models/post_test.rb test/models/user_test.rb"));
	}
	...
	fn test_select_runner_multiple_with_line_numbers() {
		let (tool, _) = select_runner(&args("test test/models/post_test.rb:15 test/models/user_test.rb:30"));
		assert_eq!(tool, "rails");
	}
