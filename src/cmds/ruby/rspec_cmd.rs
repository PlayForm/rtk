...
fn filter_rspec_output(output: &str) -> String {
	...
	match serde_json::from_str::<RspecOutput>(&stripped) {
		...
		Err(e) => {
			eprintln!("[rtk] rspec: JSON parse failed ({}), using text fallback", e);
		},
	}
...
fn build_rspec_summary(rspec: &RspecOutput) -> String {
	...
	let passed = s.example_count.saturating_sub(s.failure_count + s.pending_count);
	...
	let failures: Vec<&RspecExample> = rspec.examples.iter().filter(|e| e.status == "failed").collect();
...
"#;
	...
	fn test_filter_rspec_many_failures_caps_at_five() {
		...
		assert!(result.contains("+1 more"), "should show overflow count: {}", result);
	}
	...
	fn test_strip_noise_deprecation() {
		let input =
			"DEPRECATION WARNING: Using `return` in before callbacks is deprecated.\n...\n3 examples, 0 failures";
		let result = strip_noise(input);
...
"#;
		...
		assert!(result.contains("5. ✗"), "should show fifth failure");
		assert!(!result.contains("6. ✗"), "should not show sixth inline");
		assert!(result.contains("+2 more"), "should show overflow count: {}", result);
	}
...
"#;
		let result = filter_rspec_text(text);
		assert!(result.contains("5 examples, 1 failure"), "should contain summary: {}", result);
		assert!(result.contains("RSpec:"), "should have RSpec prefix: {}", result);
	}
	...
	fn test_has_format_flag_short_combined() {
		...
		for flag in &["-fjson", "-fj", "-fdocumentation"] {
			...
			assert!(
				args.iter().any(|a| a.starts_with("-f") && a.len() > 2 && !a.starts_with("--")),
				"should detect {}",
				flag
			);
		}
