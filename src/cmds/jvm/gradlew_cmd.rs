...
lazy_static! {
	...
	static ref TRY_SECTION: Regex = Regex::new(r"^\* Try:|^> Run with --|^> Get more help at").unwrap();
...
fn detect_task(args: &[String]) -> GradlewTask {
	...
	if task.contains("connected") {
		...
		GradlewTask::Test
	} else if task.contains("assemble") || task.contains("build") || task.contains("bundle") || task.contains("install")
	{
		GradlewTask::Build
...
impl StreamFilter for BuildLineFilter {
	fn feed_line(&mut self, line: &str) -> Option<String> {
		if filter_build_line(line) { Some(format!("{}\n", line)) } else { None }
	}
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	match detect_task(args) {
		...
		GradlewTask::Test => {
			runner::run_filtered(cmd, tool, &args_display, filter_test, RunOptions::with_tee("gradlew_test"))
		},
		...
		GradlewTask::Lint => {
			runner::run_filtered(cmd, tool, &args_display, filter_lint, RunOptions::with_tee("gradlew_lint"))
		},
...
fn filter_build_line(line: &str) -> bool {
	...
	// Always strip these
	if TASK_LINE.is_match(line) || DAEMON_LINE.is_match(line) || PROGRESS.is_match(line) || TRY_SECTION.is_match(line) {
		return false;
	}
...
fn filter_test(output: &str) -> String {
	lazy_static! {
		...
		static ref SUMMARY_LINE: Regex =
			Regex::new(r"\d+ tests? completed|\d+ tests? failed|There were failing tests|See the report at").unwrap();
	}
...
fn filter_connected(output: &str) -> String {
	lazy_static! {
		static ref INSTRUMENTATION_STATUS: Regex = Regex::new(r"^INSTRUMENTATION_STATUS[_CODE]*:").unwrap();
...
fn filter_dependencies(output: &str) -> String {
	...
	for line in output.lines() {
		...
		if (line.starts_with("+---") || line.starts_with("\\---")) && !current_config.is_empty() {
			let dep = trimmed.trim_start_matches("+--- ").trim_start_matches("\\--- ").to_string();
...
mod tests {
	...
	fn test_detect_flag_value_does_not_trigger_test() {
		...
		let args = vec!["assembleRelease".to_string(), "-Pflavor=testRelease".to_string()];
		assert_eq!(detect_task(&args), GradlewTask::Build);
	}
...
42 actionable tasks: 42 executed"#;
		...
		let savings = 100.0 - (count_tokens(&filtered.join("\n")) as f64 / count_tokens(input) as f64 * 100.0);
...
1 actionable tasks: 1 up-to-date"#;
		let filtered: Vec<&str> = input.lines().filter(|l| filter_build_line(l)).collect();
		assert!(!filtered.is_empty(), "Filter must not produce empty output on success");
	}
...
10 tests completed, 1 failed"#;
		...
		assert!(out.contains("testBar FAILED"), "FAILED test must be preserved");
		assert!(out.contains("AssertionError"), "Exception class must be preserved");
		assert!(out.contains("FooTest.testBar"), "User code frame must be preserved");
		assert!(!out.contains("org.junit.Assert.fail"), "Framework frames must be skipped");
		assert!(!out.contains("PASSED"), "PASSED tests must be stripped");
		assert!(out.contains("10 tests completed, 1 failed"), "Summary must be preserved");
    ...
    java.lang.AssertionError: expected:<5> but was:<3>
		...
		let out = filter_test(input);
		assert!(out.contains("com.example.CalcTest.testAdd"), "User code frame must be shown");
		assert!(!out.contains("org.junit.Assert"), "JUnit frames must be skipped");
		assert!(!out.contains("java.lang.reflect"), "Reflection frames must be skipped");
	}
...
Tests run: 3, Failures: 1, Errors: 0, Skipped: 0"#;
		...
		assert!(!out.contains("Starting 3 tests"), "Starting tests line must be stripped");
	}
	...
	fn test_connected_no_device_error() {
		...
		let out = filter_connected(input);
		assert!(out.contains("No connected devices"), "Must show actionable error");
	}
...
0 errors, 4 warnings"#;
		...
		assert!(out.contains("StringFormatInvalid"), "Lint violation must be preserved");
		assert!(out.contains("0 errors, 4 warnings"), "Summary must be preserved");
		assert!(!out.contains("Wrote HTML report"), "Report path must be stripped");
	}
...
Ran lint on variant debug: 2 warnings"#;
		let out = filter_lint(input);
		assert!(out.contains("HardcodedText"), "Warning violation must be preserved");
		assert!(out.contains("ContentDescription"), "Warning violation must be preserved");
...
3 actionable tasks: 1 executed, 2 up-to-date"#;
	...
	fn test_build_fixture_token_savings() {
		...
		let filtered: Vec<&str> = input.lines().filter(|l| filter_build_line(l)).collect();
		let savings = 100.0 - (count_tokens(&filtered.join("\n")) as f64 / count_tokens(input) as f64 * 100.0);
		assert!(savings >= 70.0, "Build fixture: expected ≥70% savings, got {:.1}%", savings);
		assert!(!filtered.iter().any(|l| l.starts_with("> Task :")));
	}
	...
	fn test_test_fixture_preserves_failures() {
		...
		assert!(!out.contains("PASSED"), "PASSED tests must be stripped");
		let savings = 100.0 - (count_tokens(&out) as f64 / count_tokens(input) as f64 * 100.0);
		assert!(savings >= 60.0, "Test fixture: expected ≥60% savings, got {:.1}%", savings);
	}
	...
	fn test_test_failed_fixture_shows_user_code() {
		...
		assert!(
			out.contains("CalculatorTest.testSubtraction") || out.contains("MainViewModelTest.loadDataError"),
			"User code frame must be shown"
		);
		assert!(out.contains("5 tests completed, 2 failed"), "Summary must be preserved");
	}
	...
	fn test_lint_fixture_token_savings() {
		...
		let out = filter_lint(input);
		assert!(!out.contains("Wrote HTML report"), "Report lines must be stripped");
		let savings = 100.0 - (count_tokens(&out) as f64 / count_tokens(input) as f64 * 100.0);
		assert!(savings >= 60.0, "Lint fixture: expected ≥60% savings, got {:.1}%", savings);
	}
	...
	fn test_build_success_output_format() {
		let input = include_str!("../../../tests/fixtures/gradlew_build_raw.txt");
		let output: String = input.lines().filter(|l| filter_build_line(l)).collect::<Vec<_>>().join("\n");
...
BUILD FAILED in 3s"#;
		...
		// Must keep
		assert!(out.contains("BUILD FAILED"), "BUILD FAILED must be preserved");
		assert!(out.contains("FAILURE:"), "FAILURE line must be preserved");
		assert!(out.contains("Execution failed"), "Execution failed must be preserved");
		assert!(out.contains("Lint found 8 error"), "Lint summary must be preserved");
		assert!(out.contains("Error: Field requires"), "Lint error must be preserved");

		// Must strip
		assert!(!out.contains("Configure project"), "Configure lines must be stripped");
		assert!(!out.contains("dokka"), "Dokka warnings must be stripped");
		assert!(!out.contains("android-junit5"), "Plugin warnings must be stripped");
		assert!(!out.contains("> Task :"), "Task lines must be stripped");
		assert!(!out.contains("Incubating"), "Incubating must be stripped");
		assert!(!out.contains("Deprecated Gradle"), "Deprecated must be stripped");
		assert!(!out.contains("Run with --stacktrace"), "Try section must be stripped");
...
1 actionable tasks: 1 executed"#;
		...
		assert!(out.contains("implementation (5):"), "Must show config with count: {}", out);
		assert!(out.contains("testImplementation (2):"), "Must show test config: {}", out);
		assert!(out.contains("kotlin-stdlib"), "Must show top-level dep: {}", out);
		// Should NOT contain transitive deps
		assert!(!out.contains("cursoradapter"), "Transitive deps must be stripped: {}", out);
...
BUILD SUCCESSFUL in 1s"#;
	...
	fn test_build_filter_empty_line_preserved() {
		...
		assert!(filter_build_line("   "), "whitespace-only line must pass through");
	}
...
18 actionable tasks: 18 up-to-date"#;
		...
		let filtered: Vec<&str> = input.lines().filter(|l| filter_build_line(l)).collect();
		let token_savings = 100.0 - (count_tokens(&filtered.join("\n")) as f64 / count_tokens(input) as f64 * 100.0);
		assert!(token_savings >= 70.0, "Expected ≥70% token savings, got {:.1}%", token_savings);
	}

	#[test]
	fn test_is_framework_frame() {
		assert!(is_framework_frame("at org.junit.Assert.fail(Assert.java:89)"));
		assert!(is_framework_frame("at junit.framework.Assert.fail(Assert.java:50)"));
		assert!(is_framework_frame("at java.lang.reflect.Method.invoke(Method.java:498)"));
		assert!(is_framework_frame(
			"at org.gradle.api.internal.tasks.testing.SuiteTestClassProcessor.processTestClass(SuiteTestClassProcessor.java:51)"
		));
		assert!(!is_framework_frame("at com.example.FooTest.testBar(FooTest.kt:25)"));
		assert!(!is_framework_frame("at com.example.MyApp.doSomething(MyApp.java:100)"));
	}
}
