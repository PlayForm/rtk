...
fn is_framework_frame(trimmed: &str) -> bool {
	FRAMEWORK_FRAME_PREFIXES.iter().any(|p| trimmed.starts_with(p))
}
...
impl<'a> SurefireBlock<'a> {
	...
	fn commit_failing(&mut self, out: &mut String, running: Option<&str>, lines: &[&str], close: &str) {
...
fn mvn_binary() -> &'static str {
	if cfg!(windows) {
		if Path::new(".\\mvnw.cmd").exists() { ".\\mvnw.cmd" } else { "mvn" }
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	if args.iter().any(|a| matches!(a.as_str(), "-X" | "--debug" | "-e" | "--errors")) {
...
mod tests {
	...
	fn phase_flags_before_goal() {
		assert_eq!(detect_phase(&s(["-B", "-DskipTests", "test"])), MvnPhase::Test);
	}
	#[test]
	...
	fn filter_surefire_pass_output_compact() {
		...
		assert!(savings >= 50.0, "pass-fixture savings >=50%, got {:.1}%", savings);
	}
	...
	fn close_line_matches_error_marker() {
		...
		let caps = CLOSE.captures(line).expect("CLOSE must match an ERROR!-marked close line");
	...
	fn surefire_handles_crlf_line_endings() {
		let i_lf = include_str!("../../../tests/fixtures/mvn_test_pass_slice_raw.txt").replace("\r\n", "\n");
	...
	fn package_handles_crlf_line_endings() {
		let i_lf = include_str!("../../../tests/fixtures/mvn_install_slice_raw.txt").replace("\r\n", "\n");
	...
	fn filter_package_install_compact() {
		...
		let o = filter_package(i);
		let savings = 100.0 - (count_tokens(&o) as f64 / count_tokens(i) as f64 * 100.0);
		assert!(savings >= 50.0, "install-slice savings >=50%, got {:.1}%", savings);
	}
