...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	runner::run_filtered(cmd, "wc", &args.join(" "), |stdout| filter_wc_output(stdout, &mode), opts)
}
	...
	match mode {
		...
		WcMode::Mixed => {
			...
			if parts.len() >= 2 {
				...
				if last_is_path { parts[..parts.len() - 1].join(" ") } else { parts.join(" ") }
			} else {
				line.trim().to_string()
			}
		},
		...
		match mode {
			...
			WcMode::Full => {
				if is_total {
					...
					result.push(format!("{}L {}W {}B {}", parts[0], parts[1], parts[2], name));
				} else {
