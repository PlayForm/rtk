...
pub fn run(file: &Path, max_depth: usize, schema_only: bool, verbose: u8) -> Result<()> {
	...
	println!("{}", output);
	timer.track(&format!("cat {}", file.display()), "rtk json", &content, &output);
	Ok(())
}
	...
	match value {
		...
		Value::Array(arr) => {
			if arr.is_empty() {
				...
				let items: Vec<String> = arr.iter().map(|v| compact_json(v, depth + 1, max_depth)).collect();
				let all_simple = arr
					.iter()
...
mod tests {
	...
	fn assert_value_truncated(payload: &str) {
		...
		let output = filter_json_compact(&json, 5).expect("filter_json_compact must not error on valid JSON");

		assert!(output.contains("key"));
		assert!(output.contains("..."), "long string should be truncated, got: {output}");

		let value = output.split('"').nth(1).expect("output should contain a quoted string value");
		assert!(value.len() <= 80, "truncated value is {} bytes: {value}", value.len());
	}
