...
pub fn auto_detect_filter(input: &str) -> fn(&str) -> String {
	...
	if first_1k.lines().take(5).filter(|l| !l.trim().is_empty()).any(|l| {
	...
	let path_like_lines: usize = first_1k
		...
		.filter(|l| {
			let t = l.trim();
			!t.is_empty() && !t.contains(':') && (t.starts_with('.') || t.starts_with('/') || t.contains('/'))
		})
		.count();
...
fn apply_filter(filter_fn: fn(&str) -> String, input: &str) -> String {
	std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| filter_fn(input))).unwrap_or_else(|_| {
...
mod tests {
	...
	fn test_resolve_filter_grep() {
		...
		let out = f(input);
		assert!(out.contains("main.rs") || out.contains("matches"), "out={}", out);
	}
...
"#;
	...
	fn test_auto_detect_find_not_triggered_for_grep_output() {
		...
		let f = auto_detect_filter(input);
		let out = f(input);
		assert!(!out.contains("files"), "should not trigger find filter: out={}", out);
	}
	...
	fn test_find_wrapper_token_savings() {
		...
		for dir in 1..=30 {
			for file in 1..=17 {
				input.push_str(&format!("./src/components/feature{}/sub_{}/component_{}.tsx\n", dir, dir, file));
			}
		}
