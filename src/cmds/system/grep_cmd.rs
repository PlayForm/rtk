...
fn parse_cluster(rest: &str) -> ClusterResult {
	...
	while j < bytes.len() {
		...
		if is_e || VALUE_FLAGS_SHORT.contains(&ch) {
			...
			let inline = std::str::from_utf8(&bytes[j + 1..]).unwrap_or("").to_string();
			return ClusterResult::ValueTaking { prefix, flag: ch as char, inline };
		}
...
fn strip_r(flag_letters: &str) -> Option<String> {
	let s: String = flag_letters.chars().filter(|&c| c != 'r' && c != 'R').collect();
	...
	while i < args.len() {
		...
		match arg.strip_prefix('-') {
			Some(rest) if !rest.is_empty() => match parse_cluster(rest) {
				...
				ClusterResult::ValueTaking { prefix, flag, inline } => {
					if let Some(s) = prefix {
						flags.push(format!("-{}", s));
					}
					if flag == 'e' {
...
pub fn run(
	...
	let pattern_display = if patterns.len() == 1 { patterns[0].clone() } else { patterns.join("|") };
	...
	if has_format_flag(&extra_args) {
		...
		let args_display = if extra_args.is_empty() {
			...
			format!("{} '{}' {}", extra_args.join(" "), pattern_display, path_display)
		};
	...
	let context_re = if context_only {
		Regex::new(&format!("(?i).{{0,20}}{}.*", regex::escape(&pattern_display))).ok()
	} else {
		None
	};
	...
	rtk_output.push_str(&format!("{} matches in {} files:\n\n", total_matches, by_file.len()));
...
fn clean_line(line: &str, max_len: usize, context_re: Option<&Regex>, pattern: &str) -> String {
	...
	if trimmed.len() <= max_len {
		...
		if let Some(pos) = lower.find(&pattern_lower) {
			...
			let start = if end == char_len { end.saturating_sub(max_len) } else { start };
...
fn compact_path(path: &str) -> String {
	...
	format!("{}/.../{}/{}", parts[0], parts[parts.len() - 2], parts[parts.len() - 1])
}
...
mod tests {
	...
	fn vt(prefix: Option<&str>, flag: char, inline: &str) -> ClusterResult {
		ClusterResult::ValueTaking { prefix: prefix.map(|s| s.to_string()), flag, inline: inline.to_string() }
	}
	...
	fn test_extract_ending_e() {
		let (patterns, paths, flags) = extract_pattern_path(&["-e", "foo", "-e", "bar", "src", "-e"]);
		assert_eq!(patterns, vec!["foo", "bar"]);
		assert_eq!(paths, vec!["src"]);
	...
	fn test_extract_long_ignore_file() {
		let (patterns, paths, flags) = extract_pattern_path(&["--ignore-file", ".myignore", "foo", "src"]);
		assert_eq!(patterns, vec!["foo"]);
		assert_eq!(paths, vec!["src"]);
	...
	fn test_extract_long_type_clear() {
		let (patterns, paths, flags) = extract_pattern_path(&["--type-clear", "rust", "foo", "src"]);
		assert_eq!(patterns, vec!["foo"]);
		assert_eq!(paths, vec!["src"]);
	...
	fn test_extract_long_path_separator() {
		let (patterns, paths, flags) = extract_pattern_path(&["--path-separator", "/", "foo", "src"]);
		assert_eq!(patterns, vec!["foo"]);
		assert_eq!(paths, vec!["src"]);
	...
	fn test_grep_overflow_uses_uncapped_total() {
		...
		assert_ne!(wrong_overflow, overflow, "capping before subtraction gives wrong overflow");
	}
	...
	fn test_rg_no_ignore_vcs_flag_accepted() {
		...
		let mut cmd = resolved_command("rg");
		cmd.args(["-n", "--no-heading", "--no-ignore-vcs", "NONEXISTENT_PATTERN_12345", "."]);
		if let Ok(output) = cmd.output() {
			assert!(
				output.status.code() == Some(1) || output.status.success(),
