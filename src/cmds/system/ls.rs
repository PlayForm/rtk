...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	for flag in &flags {
		if flag.starts_with("--") {
			...
			let extra: String = stripped.chars().filter(|c| *c != 'l' && *c != 'a' && *c != 'h').collect();
			if !extra.is_empty() {
	...
	let target_display = if paths.is_empty() { ".".to_string() } else { paths.join(" ") };
	...
	runner::run_filtered(
		...
		|raw| {
			...
			let has_real_content = raw.lines().any(|l| !l.starts_with("total ") && !l.is_empty() && !is_dotdir(l));
			...
			let filtered = if is_tty { format!("{}{}", entries, summary) } else { entries };
			...
			if verbose > 0 {
				eprintln!(
					...
					raw.len(),
					filtered.len(),
					if !raw.is_empty() { 100 - (filtered.len() * 100 / raw.len()) } else { 0 }
				);
			}
			filtered
		},
...
fn compact_ls(raw: &str, show_all: bool, show_long: bool) -> (String, String, usize) {
	...
	for line in raw.lines() {
		...
		let octal = if show_long { perms_to_octal(&perms) } else { None };
...
mod tests {
	...
	fn test_compact_multiline_group() {
		...
		let (entries, _summary, _parsed) = compact_ls(input, false, false);
		assert!(entries.contains("empty.txt"), "should contain 'empty.txt', got: {entries}");
		assert!(entries.contains("data.json"), "should contain 'data.json', got: {entries}");
		assert!(!entries.contains("16:18"), "time should not leak into filename, got: {entries}");
		assert!(entries.contains("0B"), "empty.txt should show 0B, got: {entries}");
		assert!(
	...
	fn test_compact_year_format_date() {
		...
		let (entries, _summary, _parsed) = compact_ls(input, false, false);
		assert!(entries.contains("archive.tar"), "should contain filename, got: {entries}");
		assert!(entries.contains("5.5K"), "should show 5.5K, got: {entries}");
	}

	#[test]
	fn test_parse_ls_line_basic() {
		let (ft, perms, size, name) = parse_ls_line("-rw-r--r--  1 user staff 1234 Jan  1 12:00 file.txt").unwrap();
		assert_eq!(ft, '-');
	...
	fn test_parse_ls_line_multiline_group() {
		let (ft, perms, size, name) =
			parse_ls_line("-rw-r--r--  1 fjeanne utilisa. du domaine 0 Mar 31 16:18 empty.txt").unwrap();
		assert_eq!(ft, '-');
		assert_eq!(perms, "-rw-r--r--");
		assert_eq!(size, 0);
	...
	fn test_parse_ls_line_dir_with_space_in_group() {
		let (ft, perms, size, name) =
			parse_ls_line("drwxr-xr-x  2 fjeanne utilisa. du domaine 64 Mar 31 16:18 my dir").unwrap();
		assert_eq!(ft, 'd');
	...
	fn test_parse_ls_line_symlink() {
		let (ft, perms, size, name) = parse_ls_line("lrwxr-xr-x  1 user staff 10 Jan  1 12:00 link -> target").unwrap();
		assert_eq!(ft, 'l');
	...
	fn test_compact_device_files() {
		...
		let (entries, _summary, _parsed) = compact_ls(input, false, false);
		assert!(entries.contains("/dev/ttyACM0"), "should contain device file, got: {entries}");
	...
	fn test_compact_device_files_macos_hex_size() {
		...
		let (entries, _summary, _parsed) = compact_ls(input, false, false);
		assert!(entries.contains("/dev/tty"), "should contain device file, got: {entries}");
	}
	...
	fn test_compact_block_device() {
		...
		let (entries, _summary, _parsed) = compact_ls(input, false, false);
		assert!(entries.contains("/dev/sda"), "should contain block device, got: {entries}");
	}
	...
	fn test_parse_ls_line_year_format() {
		let (ft, perms, size, name) = parse_ls_line("-rw-r--r--  1 user staff 5678 Dec 25  2024 old.tar.gz").unwrap();
		assert_eq!(ft, '-');
		assert_eq!(perms, "-rw-r--r--");
		assert_eq!(size, 5678);
