...
fn glob_match_inner(pat: &[u8], name: &[u8]) -> bool {
	match (pat.first(), name.first()) {
		...
		(Some(b'*'), _) => {
			...
			glob_match_inner(&pat[1..], name) || (!name.is_empty() && glob_match_inner(pat, &name[1..]))
		},
...
const UNSUPPORTED_FIND_FLAGS: &[&str] = &[
	"-not", "!", "-or", "-o", "-and", "-a", "-exec", "-execdir", "-delete", "-print0", "-newer", "-perm", "-size",
	"-mtime", "-mmin", "-atime", "-amin", "-ctime", "-cmin", "-empty", "-link", "-regex", "-iregex",
];

fn has_unsupported_find_flags(args: &[String]) -> bool {
	args.iter().any(|a| UNSUPPORTED_FIND_FLAGS.contains(&a.as_str()))
}
...
fn parse_rtk_find_args(args: &[String]) -> Result<FindArgs> {
	let mut parsed = FindArgs { pattern: args[0].clone(), ..FindArgs::default() };
	let mut i = 1;
