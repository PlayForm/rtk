...
pub(crate) fn detect_major_version() -> u32 {
	...
	match exec_capture(&mut cmd) {
		Ok(r) => {
			let version_text = if r.stdout.trim().is_empty() { &r.stderr } else { &r.stdout };
...
pub(crate) fn filter_golangci_json(output: &str, version: u32) -> String {
	...
	// Count unique files
	let unique_files: std::collections::HashSet<_> = issues.iter().map(|i| &i.pos.filename).collect();
	...
	result.push_str(&format!("golangci-lint: {} issues in {} files\n", total_issues, total_files));
	...
	for (file, count) in file_counts.iter().take(MAX_GOLANGCI_FILES) {
		...
		for issue in issues.iter().filter(|i| i.pos.filename.as_str() == **file) {
			file_linters.entry(issue.from_linter.clone()).or_default().push(issue);
		}
...
mod tests {
  ...
  "Issues": [
	...
	fn test_compact_path() {
		...
		assert_eq!(compact_path("/home/user/app/cmd/main/main.go"), "cmd/main/main.go");
		assert_eq!(compact_path("/project/internal/config/loader.go"), "internal/config/loader.go");
	...
	fn test_classify_invocation_run_uses_filtered_path() {
		assert_eq!(
			...
			Invocation::FilteredRun(RunInvocation { global_args: vec![], run_args: vec!["./...".into()] })
		);
	}
	...
	fn test_classify_invocation_with_global_flag_value_uses_filtered_path() {
		assert_eq!(
			classify_invocation(&["--color".into(), "never".into(), "run".into(), "./...".into(),]),
	...
	fn test_classify_invocation_with_short_global_flag_uses_filtered_path() {
		assert_eq!(
			...
			Invocation::FilteredRun(RunInvocation { global_args: vec!["-v".into()], run_args: vec!["./...".into()] })
		);
	}
	...
	fn test_classify_invocation_version_flag_is_passthrough() {
		assert_eq!(classify_invocation(&["--version".into()]), Invocation::Passthrough);
	}
	...
	fn test_build_filtered_args_does_not_duplicate_run() {
		let invocation = RunInvocation { global_args: vec![], run_args: vec!["./...".into()] };
  ...
  "Issues": [
    {
      ...
      "Pos": {"Filename": "main.go", "Line": 42, "Column": 5, "Offset": 0}
		...
		let result = filter_golangci_json(output, 2);
		assert!(result.contains("→"), "v2 should show source line with → prefix");
  ...
  "Issues": [
    {
      ...
      "Pos": {"Filename": "main.go", "Line": 42, "Column": 5, "Offset": 0}
		...
		let result = filter_golangci_json(output, 2);
		assert!(result.contains("errcheck"));
		assert!(!result.contains("→"), "no source line to show, should degrade gracefully");
	}
      ...
      "Pos": {{"Filename": "main.go", "Line": 1, "Column": 1, "Offset": 0}}
		...
		for line in result.lines() {
			if line.trim_start().starts_with('→') {
				...
				assert!(content.chars().count() <= 80, "content chars: {}", content.chars().count());
			}
		}
	}
