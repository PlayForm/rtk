...
pub fn run_test(args: &[String], verbose: u8) -> Result<i32> {
	...
	if verbose > 0 {
		eprintln!("Running: go test {}{}", if !skip_json { "-json " } else { "" }, args.join(" "));
	}

	let filter: fn(&str) -> String = if skip_json { |s: &str| s.to_string() } else { filter_go_test_json };
...
pub fn run_other(args: &[OsString], verbose: u8) -> Result<i32> {
	...
	let output = cmd.output().with_context(|| format!("Failed to run go {}", subcommand))?;
...
fn run_go_tool_golangci_lint(args: &[OsString], verbose: u8) -> Result<i32> {
	...
	let output = cmd.output().context("Failed to run go tool golangci-lint")?;
	...
	let json_output = if version >= 2 { stdout.lines().next().unwrap_or("") } else { &*stdout };
		...
		match event.action.as_str() {
			"build-output" => {
				if let (Some(import_path), Some(output_text)) = (&event.import_path, &event.output) {
					...
					if !text.is_empty() {
						build_output.entry(import_path.clone()).or_default().push(text);
					}
				}
				continue;
			},
...
fn is_go_build_error_line(line: &str) -> bool {
	...
	non_file_error_prefixes.iter().any(|prefix| lower.starts_with(prefix))
...
{"Time":"2024-01-01T10:01:03Z","Action":"fail","Package":"example.com/foo","Elapsed":63.003}"#;
		...
		let result = filter_go_test_json(output);
		assert!(result.contains("1 failed"), "Expected '1 failed' in output, got: {}", result);
		assert!(
			!result.contains("No tests found"),
			"Should not say 'No tests found' on timeout, got: {}",
			result
		);
		assert!(result.contains("FAIL"), "Expected failure output in summary, got: {}", result);
	}
...
go: downloading golang.org/x/xerrors v0.0.0-20220907171357-04be3eba64a2"#;
	...
	fn test_is_go_build_error_line_recognizes_real_compiler_errors() {
		...
		assert!(is_go_build_error_line("imports example.com/cycle/a: import cycle not allowed"));
		assert!(is_go_build_error_line(
		...
		assert!(is_go_build_error_line("main.go:10:5: undefined: missingFunc"));
		assert!(is_go_build_error_line("error: failed to load module"));
		assert!(!is_go_build_error_line("go: downloading github.com/pkg/errors v0.9.1"));
		assert!(!is_go_build_error_line("go: finding module for package example.com/foo"));
		assert!(!is_go_build_error_line("go: extracting github.com/pkg/errors v0.9.1"));
...
utils.go:15:5: unreachable code"#;
	...
	fn test_has_golangci_format_flag_v2() {
		assert!(has_golangci_format_flag(&os(&["--output.json.path", "stdout"])));
		assert!(has_golangci_format_flag(&os(&["--output.json.path=stdout"])));
	}
