...
pub fn run(file1: &Path, file2: &Path, verbose: u8) -> Result<i32> {
	...
	print!("{}", rtk);
	timer.track(&format!("diff {} {}", file1.display(), file2.display()), "rtk diff", &raw, &rtk);
	Ok(exit_code)
}
...
fn format_diff_changes(diff: &DiffResult) -> String {
	...
	for change in &diff.changes {
		match change {
			...
			DiffChange::Modified(ln, old, new) => out.push_str(&format!("~{:4} {} → {}\n", ln, old, new)),
		}
	}
	out
}
...
mod tests {
	...
	fn test_render_modified_only_yaml_not_identical() {
		...
		let (out, code) = render_file_diff(Path::new("one.yaml"), Path::new("two.yaml"), "a: 1\n", "a: 2\n");
	...
	fn test_render_modified_only_json_not_identical() {
		let (out, code) = render_file_diff(Path::new("j1.json"), Path::new("j2.json"), "{\"a\": 1}\n", "{\"a\": 2}\n");
		assert!(!out.contains("identical"), "modified-only diff reported as identical:\n{}", out);
		assert_eq!(code, 1);
	}
	...
	fn test_render_identical_files_exit_zero() {
		let (out, code) = render_file_diff(Path::new("a.yaml"), Path::new("b.yaml"), "a: 1\nb: 2\n", "a: 1\nb: 2\n");
...
"#;
	...
	fn test_condense_unified_diff_overflow_count_accuracy() {
		...
		let result = condense_unified_diff(&diff);
		assert!(result.contains("+190 more"), "Expected '+190 more' but got:\n{}", result);
		assert!(
