...
fn extract_mr_number(text: &str) -> Option<String> {
	MR_URL_RE.captures(text).and_then(|c| c.get(1)).map(|m| m.as_str().to_string())
}
...
fn extract_identifier_and_extra_args(args: &[String]) -> Option<(String, Vec<String>)> {
	...
	let flags_with_value = ["-R", "--repo", "-g", "--group", "-F", "--output", "-m", "--message"];
...
fn has_output_flag(args: &[String]) -> bool {
	args.iter().any(|a| a == "--output" || a == "-F" || a == "--json")
}
...
fn format_mr_list(json: &Value, ultra_compact: bool) -> String {
	...
	filtered.push_str(if ultra_compact { "MRs\n" } else { "Merge Requests\n" });
...
fn issue_list(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<i32> {
	...
	run_glab_json(cmd, "issue list", |json| format_issue_list(json, ultra_compact))
}
...
fn format_issue_view(json: &Value) -> String {
	...
	let icon = if state == "opened" { "[open]" } else { "[closed]" };
...
fn format_ci_list(json: &Value, ultra_compact: bool) -> String {
	...
	if all_lines.len() > MAX_CI_LIST {
		...
		let all_text = all_lines.join("\n");
		if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_text, "glab-pipelines", MAX_CI_LIST + 1) {
			filtered.push_str(&format!("  {}\n", hint));
		}
	}
	filtered
...
mod tests {
	...
	fn test_ok_confirmation_mr_create() {
		let result = ok_confirmation("created", "!42 https://gitlab.example.com/-/merge_requests/42");
