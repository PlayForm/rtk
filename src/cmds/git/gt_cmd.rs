...
lazy_static! {
	static ref EMAIL_RE: Regex = Regex::new(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b").unwrap();
	static ref BRANCH_NAME_RE: Regex =
		Regex::new(r#"(?:Created|Pushed|pushed|Deleted|deleted)\s+branch\s+[`"']?([a-zA-Z0-9/_.\-+@]+)"#).unwrap();
	static ref PR_LINE_RE: Regex =
		Regex::new(r"(Created|Updated)\s+pull\s+request\s+#(\d+)\s+for\s+([^\s:]+)(?::\s*(\S+))?").unwrap();
}
...
fn run_gt_filtered(
	...
	let cmd_output = exec_capture(&mut cmd)
		.with_context(|| format!("Failed to run gt {}. Is gt (Graphite) installed?", subcmd_str))?;

	let raw = format!("{}\n{}", cmd_output.stdout, cmd_output.stderr);

	let clean = strip_ansi(cmd_output.stdout.trim());
	let output = if verbose > 0 { clean.clone() } else { filter_fn(&clean) };
...
pub fn run_log(args: &[String], verbose: u8) -> Result<i32> {
	match args.first().map(|s| s.as_str()) {
		Some("short") => run_gt_filtered(&["log", "short"], &args[1..], verbose, "gt_log_short", filter_identity),
		Some("long") => run_gt_filtered(&["log", "long"], &args[1..], verbose, "gt_log_long", filter_gt_log_entries),
...
pub fn run_other(args: &[OsString], verbose: u8) -> Result<i32> {
	...
	let rest: Vec<String> = args[1..].iter().map(|a| a.to_string_lossy().into()).collect();
	...
	match subcommand.as_ref() {
		...
		"stash" => {
			...
			crate::git::run(
				crate::git::GitCommand::Stash { subcommand: stash_sub },
				stash_args,
				None,
				verbose,
				&[],
...
fn filter_gt_submit(input: &str) -> String {
	...
	for line in trimmed.lines() {
		...
		if line.contains("pushed") || line.contains("Pushed") {
			...
			if let Some(url) = caps.get(4) {
				prs.push(format!("{} PR #{} {} {}", action, num, branch, url.as_str()));
			} else {
	...
	if !pushed.is_empty() {
		let branch_names: Vec<&str> = pushed.iter().map(|s| s.as_str()).filter(|s| !s.is_empty()).collect();
...
fn filter_gt_sync(input: &str) -> String {
	...
	for line in trimmed.lines() {
		...
		if (line.contains("Synced") && line.contains("branch")) || line.starts_with("Synced with remote") {
			synced += 1;
		}
...
fn is_graph_node(line: &str) -> bool {
	let stripped = line.trim_start_matches('│').trim_start_matches('|').trim_start();
...
"#;
		...
		assert_eq!(output, "ok sync: 1 synced, 2 deleted (feat/merged-feature, fix/old-hotfix)");
	}
...
"#;
	...
	fn test_extract_branch_name() {
		assert_eq!(extract_branch_name("Created branch feat/new-feature"), "feat/new-feature");
		assert_eq!(extract_branch_name("Pushed branch fix/bug-123"), "fix/bug-123");
		assert_eq!(extract_branch_name("Pushed branch feat/auth+session"), "feat/auth+session");
