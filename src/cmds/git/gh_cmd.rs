...
lazy_static! {
	...
	static ref HORIZONTAL_RULE_RE: Regex = Regex::new(r"(?m)^\s*(?:---+|\*\*\*+|___+)\s*$").unwrap();
...
fn filter_markdown_body(body: &str) -> String {
	...
	loop {
		...
		let fence_pos = remaining.find("```").or_else(|| remaining.find("~~~")).map(|pos| {
			let fence = if remaining[pos..].starts_with("```") { "```" } else { "~~~" };
			(pos, fence)
		});
		...
		match fence_pos {
			Some((start, fence)) => {
				...
				let close_pos = remaining[code_start..].find(fence).map(|p| code_start + p + fence.len());
				...
				match close_pos {
					Some(end) => {
						...
						let after_close = remaining[end..].find('\n').map(|p| end + p + 1).unwrap_or(remaining.len());
...
fn extract_identifier_and_extra_args(args: &[String]) -> Option<(String, Vec<String>)> {
	...
	let flags_with_value = ["-R", "--repo", "-q", "--jq", "-t", "--template", "--job", "--attempt"];
	...
	F: Fn(&Value) -> String,
{
	runner::run_filtered(
		...
		RunOptions::stdout_only().early_exit_on_failure().no_trailing_newline(),
	)
}
...
fn format_pr_list(json: &Value, ultra_compact: bool) -> String {
	...
	out.push_str(if ultra_compact { "PRs\n" } else { "Pull Requests\n" });
...
fn should_passthrough_pr_status(args: &[String]) -> bool {
	args.iter()
		.any(|a| matches!(a.as_str(), "--help" | "-h" | "--web" | "--jq" | "--template"))
}
...
fn format_pr_view(json: &Value, ultra_compact: bool) -> String {
	...
	if let Some(reviews) = json["reviews"]["nodes"].as_array() {
		...
		if approved > 0 || changes > 0 {
			out.push_str(&format!("  Reviews: {} approved, {} changes requested\n", approved, changes));
		}
	}
	...
	if let Some(checks) = json["statusCheckRollup"].as_array() {
		...
		let passed = checks
			.iter()
			.filter(|c| c["conclusion"].as_str() == Some("SUCCESS") || c["state"].as_str() == Some("SUCCESS"))
			.count();
		let failed = checks
			.iter()
			.filter(|c| c["conclusion"].as_str() == Some("FAILURE") || c["state"].as_str() == Some("FAILURE"))
			.count();
		if ultra_compact {
			if failed > 0 {
...
fn list_issues(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<i32> {
	...
	run_gh_json(cmd, "issue list", |json| format_issue_list(json, ultra_compact))
}
...
fn format_run_list(json: &Value, ultra_compact: bool) -> String {
	...
	let mut out = String::new();
	out.push_str(if ultra_compact { "Runs\n" } else { "Workflow Runs\n" });
	for run in runs {
...
fn should_passthrough_run_view(extra_args: &[String]) -> bool {
	extra_args.iter().any(|a| a == "--log-failed" || a == "--log" || a == "--json")
}
...
fn run_repo(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<i32> {
	let (subcommand, rest_args) = if args.is_empty() { ("view", args) } else { (args[0].as_str(), &args[1..]) };
	if subcommand != "view" {
...
fn has_non_diff_format_flag(args: &[String]) -> bool {
	args.iter()
		.any(|a| a == "--name-only" || a == "--name-status" || a == "--stat" || a == "--numstat" || a == "--shortstat")
}

fn pr_diff(args: &[String], _verbose: u8) -> Result<i32> {
	let no_compact = args.iter().any(|a| a == "--no-compact");
	let gh_args: Vec<String> = args.iter().filter(|a| *a != "--no-compact").cloned().collect();
...
fn run_passthrough_with_extra(cmd: &str, base_args: &[&str], extra_args: &[String]) -> Result<i32> {
	let mut os_args: Vec<std::ffi::OsString> = base_args.iter().map(std::ffi::OsString::from).collect();
...
mod tests {
	...
	fn test_has_json_flag_present() {
		assert!(has_json_flag(&["view".into(), "--json".into(), "number,url".into()]));
	}
	...
	fn test_run_view_passthrough_json() {
		assert!(should_passthrough_run_view(&["--json".into(), "jobs".into()]));
	}
	...
	fn test_extract_identifier_with_job_and_log_failed() {
		...
		let args: Vec<String> = vec!["--log-failed".into(), "--job".into(), "67890".into(), "12345".into()];
		let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
		assert_eq!(id, "12345");
	...
	fn test_should_passthrough_pr_view_json() {
		assert!(should_passthrough_pr_view(&["--json".into(), "body,comments".into()]));
	}
	...
	fn test_should_passthrough_pr_status_output_transform_flags() {
		...
		assert!(should_passthrough_pr_status(&["--jq".into(), ".currentBranch".into()]));
	...
	fn test_should_passthrough_pr_status_repo_flag_stays_filtered() {
		assert!(!should_passthrough_pr_status(&["-R".into(), "owner/repo".into()]));
	}
	...
	fn test_should_passthrough_issue_view_json() {
		assert!(should_passthrough_issue_view(&["--json".into(), "body,comments".into()]));
	}

	#[test]
	fn test_should_passthrough_issue_view_jq() {
		assert!(should_passthrough_issue_view(&["--jq".into(), ".body".into()]));
	}
	...
	fn test_non_diff_format_flag_regular_args() {
		assert!(!has_non_diff_format_flag(&["123".into(), "--color=always".into()]));
	}
