...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	if is_binary(&output.stdout) {
		...
		timer.track_passthrough(&format!("curl {}", args.join(" ")), &format!("rtk curl {}", args.join(" ")));
		return Ok(exit_code);
	}
...
fn filter_curl_output(raw: &str, is_tty: bool) -> FilterResult<'_> {
	...
	if !is_tty || looks_like_json || trimmed.len() < MAX_RESPONSE_SIZE {
		return FilterResult { content: Cow::Borrowed(trimmed), tee_hint: None };
	}
	...
	FilterResult {
		content: Cow::Owned(format!("{}... ({} bytes total)", &trimmed[..end], trimmed.len())),
		tee_hint: Some(hint),
	}
}
...
mod tests {
	...
	fn test_filter_curl_large_json_array_passthrough() {
		...
		assert!(json.len() >= MAX_RESPONSE_SIZE, "fixture must exceed cap, got {}", json.len());
		let result = filter_curl_output(&json, true);
		assert!(!result.content.contains("bytes total"));
		assert!(result.content.starts_with('['));
		assert!(result.content.ends_with(']'));
