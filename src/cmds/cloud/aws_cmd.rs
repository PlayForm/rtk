...
use crate::core::utils::{
	exit_code_from_output, exit_code_from_status, human_bytes, join_with_overflow, resolved_command, shorten_arn,
	truncate_iso_date,
};
use crate::json_cmd;
...
pub fn run(subcommand: &str, args: &[String], verbose: u8) -> Result<i32> {
	...
	match subcommand {
		"sts" if !args.is_empty() && args[0] == "get-caller-identity" => {
			run_aws_filtered(&["sts", "get-caller-identity"], &args[1..], verbose, filter_sts_identity)
		},
		"s3" if !args.is_empty() && args[0] == "ls" => run_s3_ls(&args[1..], verbose),
		"ec2" if !args.is_empty() && args[0] == "describe-instances" => {
			run_aws_filtered(&["ec2", "describe-instances"], &args[1..], verbose, filter_ec2_instances)
		},
		"ecs" if !args.is_empty() && args[0] == "list-services" => {
			run_aws_filtered(&["ecs", "list-services"], &args[1..], verbose, filter_ecs_list_services)
		},
		"ecs" if !args.is_empty() && args[0] == "describe-services" => {
			run_aws_filtered(&["ecs", "describe-services"], &args[1..], verbose, filter_ecs_describe_services)
		},
		"rds" if !args.is_empty() && args[0] == "describe-db-instances" => {
			run_aws_filtered(&["rds", "describe-db-instances"], &args[1..], verbose, filter_rds_instances)
		},
		"cloudformation" if !args.is_empty() && args[0] == "list-stacks" => {
			run_aws_filtered(&["cloudformation", "list-stacks"], &args[1..], verbose, filter_cfn_list_stacks)
		},
		...
		"cloudformation" if !args.is_empty() && args[0] == "describe-stack-events" => run_aws_filtered(
			&["cloudformation", "describe-stack-events"],
			&args[1..],
			verbose,
			filter_cfn_events,
		),
		"logs" if !args.is_empty() && (args[0] == "get-log-events" || args[0] == "filter-log-events") => {
			run_aws_filtered(&["logs", &args[0]], &args[1..], verbose, filter_logs_events)
		},
		"lambda" if !args.is_empty() && args[0] == "list-functions" => {
			run_aws_filtered(&["lambda", "list-functions"], &args[1..], verbose, filter_lambda_list)
		},
		"lambda" if !args.is_empty() && args[0] == "get-function" => {
			run_aws_filtered(&["lambda", "get-function"], &args[1..], verbose, filter_lambda_get)
		},
		"iam" if !args.is_empty() && args[0] == "list-roles" => {
			run_aws_filtered(&["iam", "list-roles"], &args[1..], verbose, filter_iam_roles)
		},
		"iam" if !args.is_empty() && args[0] == "list-users" => {
			run_aws_filtered(&["iam", "list-users"], &args[1..], verbose, filter_iam_users)
		},
		"dynamodb" if !args.is_empty() && (args[0] == "scan" || args[0] == "query") => {
			run_aws_filtered(&["dynamodb", &args[0]], &args[1..], verbose, filter_dynamodb_items)
		},
		"ecs" if !args.is_empty() && args[0] == "describe-tasks" => {
			run_aws_filtered(&["ecs", "describe-tasks"], &args[1..], verbose, filter_ecs_tasks)
		},
		...
		"s3api" if !args.is_empty() && args[0] == "list-objects-v2" => {
			run_aws_filtered(&["s3api", "list-objects-v2"], &args[1..], verbose, filter_s3_objects)
		},
		"eks" if !args.is_empty() && args[0] == "describe-cluster" => {
			run_aws_filtered(&["eks", "describe-cluster"], &args[1..], verbose, filter_eks_cluster)
		},
		"sqs" if !args.is_empty() && args[0] == "receive-message" => {
			run_aws_filtered(&["sqs", "receive-message"], &args[1..], verbose, filter_sqs_messages)
		},
		"dynamodb" if !args.is_empty() && args[0] == "get-item" => {
			run_aws_filtered(&["dynamodb", "get-item"], &args[1..], verbose, filter_dynamodb_get_item)
		},
		"logs" if !args.is_empty() && args[0] == "get-query-results" => {
			run_aws_filtered(&["logs", "get-query-results"], &args[1..], verbose, filter_logs_query_results)
		},
		...
		"secretsmanager" if !args.is_empty() && args[0] == "get-secret-value" => {
			run_aws_filtered(&["secretsmanager", "get-secret-value"], &args[1..], verbose, filter_secrets_get)
		},
...
fn run_generic(subcommand: &str, args: &[String], verbose: u8, full_sub: &str) -> Result<i32> {
	...
	if !output.status.success() {
		timer.track(&format!("aws {}", full_sub), &format!("rtk aws {}", full_sub), &stderr, &stderr);
	...
	timer.track(&format!("aws {}", full_sub), &format!("rtk aws {}", full_sub), &raw, &filtered);
...
fn run_s3_transfer(operation: &str, extra_args: &[String], verbose: u8) -> Result<i32> {
	...
	let output = cmd.output().context(format!("Failed to run {}", cmd_label))?;
	let stdout = String::from_utf8_lossy(&output.stdout).to_string();
	let stderr = String::from_utf8_lossy(&output.stderr).to_string();
	let raw = if stderr.is_empty() {
		stdout.clone()
		...
		if let Some(insts) = res["Instances"].as_array() {
			for inst in insts {
				...
				let sgs: Vec<&str> = inst["SecurityGroups"]
					...
					.unwrap_or_default();
				let sg_str = if sgs.is_empty() { "-".to_string() } else { sgs.join(",") };
...
fn filter_cfn_events(json_str: &str) -> Option<FilterResult> {
	...
	for event in events {
		...
		let resource_type = resource_type_raw.strip_prefix("AWS::").unwrap_or(resource_type_raw);
		let ts = event["Timestamp"].as_str().map(truncate_iso_date).unwrap_or("?");
...
fn filter_lambda_list(json_str: &str) -> Option<FilterResult> {
	...
	for func in functions.iter().take(MAX_ITEMS) {
		...
		result.push(format!("{} {} {}MB {}s {}", name, runtime, memory, timeout, state));
	}
...
fn filter_lambda_get(json_str: &str) -> Option<FilterResult> {
	...
	let last_modified = config["LastModified"].as_str().map(truncate_iso_date).unwrap_or("?");
...
fn filter_iam_roles(json_str: &str) -> Option<FilterResult> {
	...
	for role in roles.iter().take(MAX_ITEMS) {
		...
		let date = role["CreateDate"].as_str().map(truncate_iso_date).unwrap_or("?");
...
fn filter_iam_users(json_str: &str) -> Option<FilterResult> {
	...
	for user in users.iter().take(MAX_ITEMS) {
		...
		let date = user["CreateDate"].as_str().map(truncate_iso_date).unwrap_or("?");
				...
				match key.as_str() {
					...
					"L" => {
						if let Some(arr) = inner.as_array() {
							return Value::Array(arr.iter().map(|v| unwrap_dynamodb_value(v, depth + 1)).collect());
						}
					},
					"M" => {
...
fn filter_security_groups(json_str: &str) -> Option<FilterResult> {
	...
	for sg in groups.iter().take(MAX_ITEMS) {
		...
		let ingress_str = if ingress.is_empty() { "none".to_string() } else { ingress.join(", ") };
		let egress_str = if egress.is_empty() { "none".to_string() } else { egress.join(", ") };

		result.push(format!("{} ({}) ingress: {} | egress: {}", name, id, ingress_str, egress_str));
	}
...
fn filter_s3_objects(json_str: &str) -> Option<FilterResult> {
	...
	for obj in contents.iter().take(MAX_ITEMS) {
		...
		let modified = obj["LastModified"].as_str().map(truncate_iso_date).unwrap_or("?");
...
fn filter_s3_transfer(output: &str) -> FilterResult {
	...
	if !summary_parts.is_empty() {
		result_lines.push(format!("S3 transfer: {}, {} errors", summary_parts.join(", "), errors.len()));
	}
...
fn filter_secrets_get(json_str: &str) -> Option<FilterResult> {
	...
	if let Some(secret_str) = v["SecretString"].as_str() {
		...
		if let Ok(secret_json) = serde_json::from_str::<Value>(secret_str) {
			let compact = serde_json::to_string(&secret_json).unwrap_or_else(|_| secret_str.to_string());
...
mod tests {
    ...
    "Arn": "arn:aws:iam::123456789012:user/dev-user"
		...
		assert_eq!(result.text, "AWS: 123456789012 arn:aws:iam::123456789012:user/dev-user");
	...
	fn test_snapshot_ec2_instances() {
		...
		assert!(result.text.contains("i-0a1b2c3d4e5f00002 stopped t3.large 10.0.2.20"));
		assert!(!result.truncated);
	}
	...
	fn test_filter_sts_identity() {
		...
		let result = filter_sts_identity(json).unwrap();
		assert_eq!(result.text, "AWS: 123456789012 arn:aws:iam::123456789012:user/dev");
	}
	...
	fn test_filter_cfn_list_stacks() {
		...
		assert!(result.text.contains("other-stack UPDATE_COMPLETE 2024-02-20"));
	}
    ...
    "Reservations": [{
		...
		assert!(savings >= 60.0, "EC2 filter: expected >=60% savings, got {:.1}%", savings);
	}
	...
	fn test_filter_logs_events_token_savings() {
		...
		assert!(savings >= 15.0, "Logs filter: expected >=15% savings, got {:.1}%", savings);
	}
	...
	fn test_filter_lambda_list() {
		...
		assert!(result.text.contains("my-worker nodejs20.x 256MB 60s Active"));
	...
	fn test_filter_iam_roles() {
		...
		assert!(result.text.contains("lambda-exec 2024-02-20 assume:[lambda.amazonaws.com]"));
	...
	fn test_filter_iam_roles_token_savings() {
		...
		let output_tokens = count_tokens(&result.text);
		let savings = 100.0 - (output_tokens as f64 / input_tokens as f64 * 100.0);
		assert!(savings >= 60.0, "IAM roles filter: expected >=60% savings, got {:.1}%", savings);
	}
	...
	fn test_filter_dynamodb_token_savings() {
		...
		let output_tokens = count_tokens(&result.text);
		let savings = 100.0 - (output_tokens as f64 / input_tokens as f64 * 100.0);
		assert!(savings >= 30.0, "DynamoDB filter: expected >=30% savings, got {:.1}%", savings);
	}
	...
	fn test_filter_ecs_tasks() {
		...
		assert!(result.text.contains("def789ghi012 STOPPED containers:[worker:STOPPED(exit:1)]"));
		assert!(result.text.contains("reason:Essential container in task exited"));
	...
	fn test_filter_security_groups_token_savings() {
		...
		let output_tokens = count_tokens(&result.text);
		let savings = 100.0 - (output_tokens as f64 / input_tokens as f64 * 100.0);
		assert!(savings >= 60.0, "SG filter: expected >=60% savings, got {:.1}%", savings);
	}
...
";
	...
	fn test_filter_secrets_get() {
		...
		assert!(result.text.contains(r#"{"username":"admin","password":"secret123"}"#));
	...
	fn test_snapshot_security_groups_format() {
		...
		let result = filter_security_groups(json).unwrap();
		assert_eq!(result.text, "web (sg-1) ingress: tcp/443<-0.0.0.0/0 | egress: all<-0.0.0.0/0");
	}
	...
	fn test_snapshot_cfn_events_format() {
		...
		let result = filter_cfn_events(json).unwrap();
		assert!(result.text.starts_with("CloudFormation: 2 events (1 failed, 1 successful)"));
		assert!(result.text.contains("--- FAILURES ---"));
		assert!(result.text.contains("Bucket S3::Bucket CREATE_FAILED REASON: Already exists"));
	}
	...
	fn test_filter_cfn_events_empty() {
		let json = r#"{"StackEvents": []}"#;
		let result = filter_cfn_events(json).unwrap();
		assert_eq!(result.text, "CloudFormation: 0 events (0 failed, 0 successful)");
	}
	...
	fn test_aws_unsupported_subcommand_json_preserves_values() {
		...
		assert!(output.contains("isMpaEnabled"), "object keys must be preserved, got:\n{output}");
	}
}
