...
fn format_container_line_from_parts(parts: &[&str], with_ports: bool) -> Option<String> {
	...
	let port_suffix = if with_ports {
		...
		if ports == "-" { String::new() } else { format!(" [{}]", ports) }
	} else {
		String::new()
	};
...
fn docker_images(_verbose: u8) -> Result<i32> {
	...
	let result =
		exec_capture(resolved_command("docker").args(["images", "--format", "{{.Repository}}:{{.Tag}}\t{{.Size}}"]))
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<i32> {
	...
	runner::run_filtered(
		...
		"docker",
		&label,
		|raw| format!("[docker] Logs for {}:\n{}", container, crate::log_cmd::run_stdin_str(raw)),
...
fn format_kubectl_pods(json: &Value) -> String {
	...
	if !issues.is_empty() {
		...
		if issues.len() > MAX_PODS_ISSUES {
			...
			if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_issues, "kubectl-pods", MAX_PODS_ISSUES + 1)
			{
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<i32> {
	...
	runner::run_filtered(
		...
		"kubectl",
		&label,
		|stdout| format!("Logs for {}:\n{}", pod, crate::log_cmd::run_stdin_str(stdout)),
	...
	let all_formatted: Vec<String> = lines
		...
		.filter_map(|line| {
			...
			let port_str = if ports.trim().is_empty() {
				...
				if compact == "-" { String::new() } else { format!(" [{}]", compact) }
			};
...
pub fn format_compose_build(raw: &str) -> String {
	...
	let step_count = raw.lines().filter(|l| l.trim_start().starts_with("=> ")).count();
	if step_count > 0 {
...
pub fn run_compose_ps(all: bool, verbose: u8) -> Result<i32> {
	...
	let raw_result =
		exec_capture(resolved_command("docker").args(&raw_args)).context("Failed to run docker compose ps")?;
...
mod tests {
	...
	fn test_format_compose_ps_no_ports() {
		...
		assert!(!redis_line.contains("] ["), "should not show port brackets when empty");
	}
	...
	fn test_format_compose_ps_long_image_path() {
		...
		let out = format_compose_ps(raw);
		assert!(out.contains("myapp:latest"), "should shorten image to last segment");
		assert!(!out.contains("ghcr.io"), "should not show full registry path");
	}
...
[+] Building 12.3s (8/8) FINISHED
 ...
 => => naming to docker.io/library/myapp-web                       0.0s";
	...
	fn test_format_compose_build_empty() {
		...
		assert!(!out.is_empty(), "should produce output even for empty input");
	}
	...
	fn test_compact_ports_many() {
		let result =
			compact_ports("0.0.0.0:80->80/tcp, 0.0.0.0:443->443/tcp, 0.0.0.0:8080->8080/tcp, 0.0.0.0:9090->9090/tcp");
	...
	fn test_kubectl_get_target_pods_aliases() {
		for resource in ["po", "pod", "pods"] {
			...
			assert_eq!(kubectl_get_target(&args), Some(("pods", &args[1..])), "failed for {resource}");
		}
	}
	...
	fn test_kubectl_get_target_respects_output_flags() {
		for output_flag in ["-o", "-owide", "--output", "--output=json"] {
			let args = vec!["pods".to_string(), output_flag.to_string(), "wide".to_string()];

			assert_eq!(kubectl_get_target(&args), None, "should pass through {output_flag}");
		}
	}
}
