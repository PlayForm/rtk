...
pub fn run(url: &str, args: &[String], verbose: u8) -> Result<i32> {
	...
	if result.success() {
		...
		let msg = format!("{} ok | {} | {}", compact_url(url), filename, format_size(size));
		println!("{}", msg);
...
pub fn run_stdout(url: &str, args: &[String], verbose: u8) -> Result<i32> {
	...
	if result.success() {
		...
		print!("{}", rtk_output);
		timer.track(&format!("wget -O - {}", url), "rtk wget -o", &result.stdout, &rtk_output);
	} else {
...
mod tests {
	...
	fn test_parse_error_dns() {
		assert_eq!(parse_error("unable to resolve host example.com", ""), "DNS lookup failed");
	}

	#[test]
	fn test_parse_error_ssl() {
		assert_eq!(parse_error("SSL certificate verification failed", ""), "SSL/TLS error");
	}
	...
	fn test_extract_filename_from_output_flag() {
		...
		assert_eq!(extract_filename_from_output("", "https://example.com/x", &args), "myfile.zip");
	}
