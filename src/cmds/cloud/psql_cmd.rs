...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	runner::run_filtered(
		...
		&args.join(" "),
		filter_psql_output,
		RunOptions::stdout_only().tee("psql").early_exit_on_failure(),
	)
}
...
fn filter_expanded(output: &str) -> String {
	...
	if record_count > MAX_EXPANDED_RECORDS {
		result.push(format!("... +{} more records", record_count - MAX_EXPANDED_RECORDS));
	}
...
";
	...
	fn test_table_token_savings() {
		...
		assert!(savings >= 40.0, "Table filter: expected >=40% savings, got {:.1}%", savings);
	}
	...
	fn test_expanded_token_savings() {
		...
		let output_tokens = count_tokens(&result);
		let savings = 100.0 - (output_tokens as f64 / input_tokens as f64 * 100.0);
		assert!(savings >= 60.0, "Expanded filter: expected >=60% savings, got {:.1}%", savings);
	}
}
