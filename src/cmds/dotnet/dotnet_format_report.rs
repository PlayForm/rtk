...
pub fn parse_format_report(path: &Path) -> Result<FormatSummary> {
	let file =
		File::open(path).with_context(|| format!("Failed to read dotnet format report at {}", path.display()))?;
	let reader = BufReader::new(file);

	let entries: Vec<FormatReportEntry> = serde_json::from_reader(reader)
		.with_context(|| format!("Failed to parse dotnet format report JSON at {}", path.display()))?;
	...
	let files_with_changes: Vec<FileWithChanges> = entries
		...
		.filter_map(|entry| {
			...
			Some(FileWithChanges { path: entry.file_path, changes })
		})
		.collect();

	let files_unchanged = total_files.saturating_sub(files_with_changes.len());

	Ok(FormatSummary { files_with_changes, files_unchanged, total_files })
}
