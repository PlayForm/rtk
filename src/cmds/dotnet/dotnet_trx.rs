...
fn extract_attr_value(reader: &Reader<&[u8]>, start: &BytesStart<'_>, key: &[u8]) -> Option<String> {
...
fn format_duration_between(start_dt: DateTime<FixedOffset>, finish_dt: DateTime<FixedOffset>) -> Option<String> {
...
pub fn parse_trx_files_in_dir_since(dir: &Path, since: Option<SystemTime>) -> Option<TestSummary> {
	...
	for entry in entries.flatten() {
		let path = entry.path();
		if path.extension().is_none_or(|e| !e.eq_ignore_ascii_case("trx")) {
			continue;
		}
...
fn find_recent_trx_in_dir(dir: &Path) -> Option<PathBuf> {
	...
	std::fs::read_dir(dir)
		...
		.filter_map(|entry| {
			let path = entry.path();
			let is_trx = path.extension().is_some_and(|ext| ext.eq_ignore_ascii_case("trx"));
			if !is_trx {
				return None;
			}
...
fn parse_trx_content(content: &str) -> Option<TestSummary> {
	...
	loop {
		match reader.read_event_into(&mut buf) {
			Ok(Event::Start(e)) => match local_name(e.name().as_ref()) {
				...
				b"UnitTestResult" => {
					let outcome = extract_attr_value(&reader, &e, b"outcome").unwrap_or_else(|| "Unknown".to_string());
					...
					if outcome == "Failed" {
						...
						stack_buf.clear();
						failed_test_name =
							extract_attr_value(&reader, &e, b"testName").unwrap_or_else(|| "unknown".to_string());
					}
				},
			...
			Ok(Event::Empty(e)) => match local_name(e.name().as_ref()) {
				...
				b"UnitTestResult" => {
					...
					if outcome == "Failed" {
						let name =
							extract_attr_value(&reader, &e, b"testName").unwrap_or_else(|| "unknown".to_string());
						summary.failed_tests.push(FailedTest { name, details: Vec::new() });
					}
				},
				_ => {},
			},
			...
			Ok(Event::End(e)) => match local_name(e.name().as_ref()) {
				...
				b"UnitTestResult" if in_failed_result => {
					...
					summary
						.failed_tests
						.push(FailedTest { name: failed_test_name.clone(), details });
	...
	if summary.total > 0 {
		summary.skipped = summary.total.saturating_sub(summary.passed + summary.failed);
	}
...
<TestRun>
  ...
  <ResultSummary><Counters total="1" executed="1" passed="0" failed="1" /></ResultSummary>
		...
		assert_eq!(summary.failed_tests[0].name, "MyTests.Calculator.Add_ShouldFail");
...
<TestRun>
  ...
  <ResultSummary><Counters failed="1" passed="0" executed="1" total="1" /></ResultSummary>
		...
		assert_eq!(summary.failed_tests.len(), 1);
		assert_eq!(summary.failed_tests[0].name, "MyTests.Ordering.ShouldStillParse");
	}
