...
pub fn run_format(args: &[String], verbose: u8) -> Result<i32> {
	...
	let filtered = format_report_summary_or_raw(report_path.as_deref(), check_mode, &raw, command_started_at);
	println!("{}", filtered);
...
pub fn run_passthrough(args: &[OsString], verbose: u8) -> Result<i32> {
	...
	let result = exec_capture(&mut cmd).with_context(|| format!("Failed to run dotnet {}", subcommand))?;
...
fn run_dotnet_with_binlog(subcommand: &str, args: &[String], verbose: u8) -> Result<i32> {
	...
	for arg in build_effective_dotnet_args(subcommand, args, &binlog_path, trx_results_dir.as_deref()) {
		cmd.arg(arg);
	}
	...
	let filtered = match subcommand {
		"build" => {
			let binlog_summary = if should_expect_binlog && binlog_path.exists() {
				normalize_build_summary(binlog::parse_build(&binlog_path).unwrap_or_default(), command_success)
			} else {
				binlog::BuildSummary::default()
			};
			let raw_summary = normalize_build_summary(binlog::parse_build_from_text(&raw), command_success);
		...
		"test" => {
			...
			let raw_diagnostics = normalize_build_summary(binlog::parse_build_from_text(&raw), command_success);
			let test_build_summary = merge_build_summaries(binlog_diagnostics, raw_diagnostics);
			format_test_output(&summary, &test_build_summary.errors, &test_build_summary.warnings, &binlog_path)
		},
		"restore" => {
			let binlog_summary = if should_expect_binlog && binlog_path.exists() {
				normalize_restore_summary(binlog::parse_restore(&binlog_path).unwrap_or_default(), command_success)
			} else {
				binlog::RestoreSummary::default()
			};
			let raw_summary = normalize_restore_summary(binlog::parse_restore_from_text(&raw), command_success);
...
fn build_binlog_path(subcommand: &str) -> PathBuf {
	std::env::temp_dir().join(format!("rtk_dotnet_{}_{}.binlog", subcommand, unique_temp_suffix()))
}
...
fn format_dotnet_format_output(summary: &dotnet_format_report::FormatSummary, check_mode: bool) -> String {
	let changed_count = summary.files_with_changes.len();

	if changed_count == 0 {
		return format!("ok dotnet format: {} files formatted correctly", summary.total_files);
	}
	...
	for (index, file) in summary.files_with_changes.iter().take(MAX_FORMAT_FILES).enumerate() {
	...
	if changed_count > MAX_FORMAT_FILES {
		...
		let all_files = summary
			...
			.join("\n");
		if let Some(hint) =
			crate::core::tee::force_tee_tail_hint(&all_files, "dotnet-format-files", MAX_FORMAT_FILES + 1)
		{
...
fn detect_test_runner_mode(args: &[String]) -> TestRunnerMode {
	...
	let explicit_projects: Vec<&str> = args
		...
		.filter(|a| {
			...
			project_extensions.iter().any(|ext| lower.ends_with(&format!(".{ext}")))
		})
		.collect();
	...
	if !explicit_projects.is_empty() {
		...
		if let Ok(entries) = std::fs::read_dir(".") {
			for entry in entries.flatten() {
				...
				if project_extensions.iter().any(|ext| name_str.ends_with(&format!(".{ext}")))
...
fn extract_report_arg(args: &[String]) -> Option<PathBuf> {
	...
	while let Some(arg) = iter.next() {
		...
		if let Some((_, value)) = arg.split_once('=') {
			if arg.split('=').next().is_some_and(|key| key.eq_ignore_ascii_case("--report")) {
...
fn normalize_build_summary(mut summary: binlog::BuildSummary, command_success: bool) -> binlog::BuildSummary {
	if command_success {
...
fn normalize_test_summary(mut summary: binlog::TestSummary, command_success: bool) -> binlog::TestSummary {
...
fn normalize_restore_summary(mut summary: binlog::RestoreSummary, command_success: bool) -> binlog::RestoreSummary {
	...
	if !summary.errors.is_empty() {
		...
		if summary.errors.len() > MAX_BUILD_ERRORS {
			...
			let all_errors = summary
				...
				.join("\n");
			if let Some(hint) =
				crate::core::tee::force_tee_tail_hint(&all_errors, "dotnet-build-errors", MAX_BUILD_ERRORS + 1)
			{
				errors.push_str(&format!("  {}\n", hint));
			}
		}
	...
	if !summary.warnings.is_empty() {
		...
		if summary.warnings.len() > MAX_BUILD_WARNINGS {
			...
			let all_warnings = summary
				...
				.join("\n");
			if let Some(hint) =
				crate::core::tee::force_tee_tail_hint(&all_warnings, "dotnet-build-warnings", MAX_BUILD_WARNINGS + 1)
			{
				warnings.push_str(&format!("  {}\n", hint));
			}
		}
...
fn format_test_output(
	...
	if has_failures && !summary.failed_tests.is_empty() {
		...
		if summary.failed_tests.len() > MAX_DOTNET_FAILURES {
			...
			let all_failed = summary
				...
				.collect::<Vec<_>>()
				.join("\n\n");
			if let Some(hint) = crate::core::tee::force_tee_hint(&all_failed, "dotnet-test-failures") {
				failed_tests_section.push_str(&format!("  {}\n", hint));
			}
		}
	}
	...
	if !errors.is_empty() {
		...
		if errors.len() > MAX_TEST_ERRORS {
			...
			let all_errors = errors.iter().map(|e| format_issue(e, "error")).collect::<Vec<_>>().join("\n");
			if let Some(hint) =
				crate::core::tee::force_tee_tail_hint(&all_errors, "dotnet-test-errors", MAX_TEST_ERRORS + 1)
			{
				errors_section.push_str(&format!("  {}\n", hint));
			}
		}
	...
	if !warnings.is_empty() {
		...
		if warnings.len() > MAX_TEST_WARNINGS {
			...
			let all_warnings = warnings
				...
				.join("\n");
			if let Some(hint) =
				crate::core::tee::force_tee_tail_hint(&all_warnings, "dotnet-test-warnings", MAX_TEST_WARNINGS + 1)
			{
				warnings_section.push_str(&format!("  {}\n", hint));
			}
		}
...
fn format_restore_output(
	...
	if !errors.is_empty() {
		...
		if errors.len() > MAX_FORMAT_ERRORS {
			...
			let all_errors = errors.iter().map(|e| format_issue(e, "error")).collect::<Vec<_>>().join("\n");
			if let Some(hint) =
				crate::core::tee::force_tee_tail_hint(&all_errors, "dotnet-format-errors", MAX_FORMAT_ERRORS + 1)
			{
				errors_section.push_str(&format!("  {}\n", hint));
			}
		}
	...
	if !warnings.is_empty() {
		...
		if warnings.len() > MAX_FORMAT_WARNINGS {
			...
			let all_warnings = warnings
				...
				.join("\n");
			if let Some(hint) =
				crate::core::tee::force_tee_tail_hint(&all_warnings, "dotnet-format-warnings", MAX_FORMAT_WARNINGS + 1)
			{
				warnings_section.push_str(&format!("  {}\n", hint));
			}
		}
...
mod tests {
	...
	fn build_dotnet_args_for_test(subcommand: &str, args: &[String], with_trx: bool) -> Vec<String> {
		let binlog_path = Path::new("/tmp/test.binlog");
		let trx_results_dir = if with_trx { Some(Path::new("/tmp/test results")) } else { None };
...
<TestRun xmlns="http://microsoft.com/schemas/VisualStudio/TeamTest/2010">
  <ResultSummary outcome="Completed">
	...
	fn test_format_test_output_surfaces_errors() {
		...
		let errors = vec![binlog::BinlogIssue {
			...
			line: 135,
			column: 0,
			message: "CreateInstance_should_initialize_interface_message_type_on_demand".to_string(),
		}];

		let output = format_test_output(&summary, &errors, &[], Path::new("/tmp/test.binlog"));
		assert!(output.contains("Errors:"));
		assert!(output.contains("error TESTERROR"));
		assert!(output.contains("CreateInstance_should_initialize_interface_message_type_on_demand"));
	}
	...
	fn test_format_restore_output_includes_error_details() {
		...
		let output = format_restore_output(&summary, &issues, &[], Path::new("/tmp/restore.binlog"));
		assert!(output.contains("Errors:"));
	...
	fn test_merge_test_summaries_keeps_structured_counts_and_fills_failed_tests() {
		...
		assert!(merged.failed_tests[0].name.contains("CreateInstance_should_initialize"));
	}

	#[test]
	fn test_normalize_restore_summary_sets_error_floor_on_failed_command() {
		let summary = binlog::RestoreSummary { restored_projects: 2, warnings: 0, errors: 0, duration_text: None };
	...
	fn test_forwarding_project_file() {
		let args = vec!["--project".to_string(), "src/My App.Tests/My App.Tests.csproj".to_string()];
	...
	fn test_user_logger_override() {
		let args = vec!["--logger".to_string(), "console;verbosity=detailed".to_string()];
	...
	fn test_user_results_directory_prevents_extra_injection() {
		let args = vec!["--results-directory".to_string(), "/custom/results".to_string()];
  ...
  <PropertyGroup>
    <UseMicrosoftTestingPlatformRunner>true</UseMicrosoftTestingPlatformRunner>
		...
		assert_eq!(detect_test_runner_mode(&args), TestRunnerMode::MtpVsTestBridge);
  ...
  <PropertyGroup>
    <UseMicrosoftTestingPlatformRunner>true</UseMicrosoftTestingPlatformRunner>
		...
		let args = vec![csproj.display().to_string(), "--".to_string(), "--parallel".to_string()];
		let binlog_path = Path::new("/tmp/test.binlog");
		let injected = build_effective_dotnet_args("test", &args, binlog_path, None);
  ...
  <PropertyGroup>
    <UseMicrosoftTestingPlatformRunner>true</UseMicrosoftTestingPlatformRunner>
		...
		let args = vec![csproj.display().to_string(), "--".to_string(), "--report-trx".to_string()];
		let binlog_path = Path::new("/tmp/test.binlog");
		let injected = build_effective_dotnet_args("test", &args, binlog_path, None);
  ...
  <PropertyGroup>
	...
	fn test_is_global_json_mtp_mode_returns_false_for_vstest_runner() {
		let temp_dir = tempfile::tempdir().expect("create temp dir");
		let global_json = temp_dir.path().join("global.json");
		fs::write(&global_json, r#"{ "sdk": { "version": "9.0.100" } }"#).expect("write global.json");
	...
	fn test_merge_test_summary_from_trx_uses_primary_and_cleans_file() {
		...
		let filled =
			merge_test_summary_from_trx(binlog::TestSummary::default(), Some(temp_dir.path()), None, SystemTime::now());
	...
	fn test_merge_test_summary_from_trx_returns_default_when_no_trx() {
		...
		let filled =
			merge_test_summary_from_trx(binlog::TestSummary::default(), Some(&missing), None, SystemTime::now());
	...
	fn test_merge_test_summary_from_trx_keeps_larger_existing_counts() {
		...
		let merged = merge_test_summary_from_trx(existing, Some(temp_dir.path()), None, SystemTime::now());
	...
	fn test_extract_results_directory_arg_detects_variants() {
		...
		assert_eq!(extract_results_directory_arg(&args), Some(PathBuf::from("/tmp/r1")));

		let args = vec!["--results-directory=/tmp/r2".to_string()];
		assert_eq!(extract_results_directory_arg(&args), Some(PathBuf::from("/tmp/r2")));
	}
	...
	fn test_format_all_formatted() {
		let summary = dotnet_format_report::parse_format_report(&format_fixture("format_success.json"))
	...
	fn test_format_needs_formatting() {
		let summary = dotnet_format_report::parse_format_report(&format_fixture("format_changes.json"))
			.expect("parse format report");
	...
	fn test_format_user_report_arg_no_cleanup() {
		let args = vec!["--report".to_string(), "/tmp/user-format-report.json".to_string()];

		let (report_path, cleanup) = resolve_format_report_path(&args);
		assert_eq!(report_path, Some(PathBuf::from("/tmp/user-format-report.json")));
		assert!(!cleanup);
	}
	...
	fn test_format_preserves_positional_project_argument_order() {
		...
		let effective = build_effective_dotnet_format_args(&args, Some(Path::new("/tmp/report.json")));
		assert_eq!(effective.first().map(String::as_str), Some("src/App/App.csproj"));
	}
	...
	fn test_format_report_summary_ignores_stale_report_file() {
		...
		let command_started_at = SystemTime::now().checked_add(Duration::from_secs(2)).expect("future timestamp");
		let raw = "RAW OUTPUT";
