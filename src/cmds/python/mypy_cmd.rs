...
pub fn filter_mypy_output(output: &str) -> String {
	...
	while i < lines.len() {
		...
		if let Some(caps) = MYPY_DIAG.captures(line) {
			...
			let code = caps.get(5).map(|m| m.as_str().to_string()).unwrap_or_default();
			...
			let mut err = MypyError { file, line: line_num, code, message, context_lines: Vec::new() };
	...
	if !errors.is_empty() {
		result.push_str(&format!("mypy: {} errors in {} files\n", errors.len(), by_file.len()));
...
";
	...
	fn test_filter_mypy_no_file_limit() {
		...
		for i in 1..=15 {
			output.push_str(&format!("src/file{}.py:{}: error: Error in file {}.  [assignment]\n", i, i, i));
		}
		output.push_str("Found 15 errors in 15 files\n");
		let result = filter_mypy_output(&output);
		assert!(result.contains("15 errors in 15 files"));
		for i in 1..=15 {
			assert!(result.contains(&format!("file{}.py", i)), "file{}.py missing from output", i);
		}
	}
}
