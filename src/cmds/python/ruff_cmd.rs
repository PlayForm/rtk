...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	if is_check {
		...
		let start_idx = if !args.is_empty() && args[0] == "check" { 1 } else { 0 };
		for arg in &args[start_idx..] {
			cmd.arg(arg);
		}

		if args.iter().skip(start_idx).all(|a| a.starts_with('-') || a.contains('=')) {
			cmd.arg(".");
		}
	} else {
		for arg in args {
...
pub fn filter_ruff_check_json(output: &str) -> String {
	...
	// Count unique files
	let unique_files: std::collections::HashSet<_> = diagnostics.iter().map(|d| &d.filename).collect();
	...
	result.push_str(&format!("Ruff: {} issues in {} files", total_issues, total_files));
	...
	if violation_lines.len() > MAX_VIOLATIONS {
		...
		if let Some(hint) = crate::core::tee::force_tee_tail_hint(&full, "ruff-check", MAX_VIOLATIONS + 1) {
			result.push_str(&format!("  {}\n", hint));
		}
	}
...
pub fn filter_ruff_format(output: &str) -> String {
	...
	if output_lower.contains("would reformat") {
		...
		if files_to_format.is_empty() {
			...
			result.push_str(&format!("Ruff format: {} files need formatting\n", files_to_format.len()));

			const MAX_RUFF_FORMAT_FILES: usize = CAP_WARNINGS;
			for (i, file) in files_to_format.iter().take(MAX_RUFF_FORMAT_FILES).enumerate() {
...
2 files would be reformatted, 3 files left unchanged"#;
	...
	fn test_filter_ruff_check_caps_violations_and_emits_hint() {
		...
		let listed = in_section.lines().filter(|l| l.trim().starts_with("src/")).count();
		assert!(listed <= 50, "violations cap not enforced: got {listed}");
		assert!(result.contains("… +150 more"), "missing '+N more' indicator");
		...
		assert!(savings >= 60.0, "token savings dropped below 60%: {savings:.1}%");
	}

	#[test]
	fn test_compact_path() {
		assert_eq!(compact_path("/Users/foo/project/src/main.py"), "src/main.py");
		assert_eq!(compact_path("/home/user/app/lib/utils.py"), "lib/utils.py");
		assert_eq!(compact_path("C:\\Users\\foo\\project\\tests\\test.py"), "tests/test.py");
