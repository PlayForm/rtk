...
fn run_list(base_cmd: &str, args: &[String], verbose: u8) -> Result<(String, String, i32)> {
	...
	let result = exec_capture(&mut cmd).with_context(|| format!("Failed to run {} pip list", base_cmd))?;
...
fn run_outdated(base_cmd: &str, args: &[String], verbose: u8) -> Result<(String, String, i32)> {
	...
	let result = exec_capture(&mut cmd).with_context(|| format!("Failed to run {} pip list --outdated", base_cmd))?;
...
fn run_passthrough(base_cmd: &str, args: &[String], verbose: u8) -> Result<(String, String, i32)> {
	...
	let result =
		exec_capture(&mut cmd).with_context(|| format!("Failed to run {} pip {}", base_cmd, args.join(" ")))?;
...
fn filter_pip_list(output: &str) -> String {
	...
	let mut by_letter: std::collections::HashMap<char, Vec<&Package>> = std::collections::HashMap::new();
...
fn filter_pip_outdated(output: &str) -> String {
	...
	for (i, pkg) in packages.iter().take(MAX_PIP_PACKAGES).enumerate() {
		...
		result.push_str(&format!("{}. {} ({} → {})\n", i + 1, pkg.name, pkg.version, latest));
	}
