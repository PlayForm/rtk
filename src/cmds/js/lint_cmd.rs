...
fn detect_linter(args: &[String]) -> (&str, bool) {
	let is_path_or_flag = args.is_empty() || args[0].starts_with('-') || args[0].contains('/') || args[0].contains('.');

	if is_path_or_flag { ("eslint", false) } else { (&args[0], true) }
}
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	let start_idx = if !explicit {
		...
		if effective_args.len() > 1 && effective_args[1] == "check" { 2 } else { 1 }
	} else {
		1
	};
	...
	if !result.success() && result.exit_code > 128 {
		...
		if !result.stderr.is_empty() {
			eprintln!("stderr: {}", result.stderr.lines().take(5).collect::<Vec<_>>().join("\n"));
		}
...
fn filter_eslint_json(output: &str) -> String {
	...
	if by_file.len() > MAX_FILES {
		...
		let all_file_lines = by_file
			...
			.collect::<Vec<_>>()
			.join("\n");
		if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_file_lines, "eslint-files", MAX_FILES + 1) {
			result.push_str(&format!("  {}\n", hint));
		}
	}
...
fn filter_pylint_json(output: &str) -> String {
	...
	// Build output
	let mut result = String::new();
	result.push_str(&format!("Pylint: {} issues in {} files\n", diagnostics.len(), total_files));

	if errors > 0 || warnings > 0 {
		result.push_str(&format!("  {} errors, {} warnings", errors, warnings));
		if conventions > 0 || refactors > 0 {
			result.push_str(&format!(", {} conventions, {} refactors", conventions, refactors));
		}
		result.push('\n');
	}
	...
	if file_counts.len() > MAX_FILES {
		...
		let all_file_lines = file_counts
			...
			.collect::<Vec<_>>()
			.join("\n");
		if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_file_lines, "pylint-files", MAX_FILES + 1) {
			result.push_str(&format!("  {}\n", hint));
		}
	}
...
fn filter_generic_lint(output: &str) -> String {
	...
	if issues.len() > MAX_ISSUES {
		...
		if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_issues, "lint-issues", MAX_ISSUES + 1) {
			result.push_str(&format!("  {}\n", hint));
		}
	}
...
mod tests {
	...
	fn test_compact_path() {
		assert_eq!(compact_path("/Users/foo/project/src/utils.ts"), "src/utils.ts");
		assert_eq!(compact_path("C:\\Users\\project\\src\\api.ts"), "src/api.ts");
	...
	fn test_detect_linter_after_pnpm_exec_strip() {
		let full_args: Vec<String> = vec!["pnpm".into(), "exec".into(), "biome".into(), "check".into()];
		let skip = strip_pm_prefix(&full_args);
		let effective = &full_args[skip..];
		let (linter, _) = detect_linter(effective);
