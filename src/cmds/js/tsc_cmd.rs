...
lazy_static! {
	static ref TSC_ERROR: Regex = Regex::new(r"^(.+?)\((\d+),(\d+)\):\s+(error|warning)\s+(TS\d+):\s+(.+)$").unwrap();
}
...
impl TscHandler {
	fn new() -> Self {
		Self { error_count: 0, files: HashSet::new(), code_counts: HashMap::new() }
	}
}
...
impl BlockHandler for TscHandler {
	...
	fn format_summary(&self, _exit_code: i32, _raw: &str) -> Option<String> {
		...
		let mut result = format!("TypeScript: {} errors in {} files\n", self.error_count, self.files.len());
...
pub(crate) fn filter_tsc_output(output: &str) -> String {
	...
	while i < lines.len() {
		...
		if let Some(caps) = TSC_ERROR.captures(line) {
			...
			while i < lines.len() {
				let next = lines[i];
				if !next.is_empty() && (next.starts_with("  ") || next.starts_with('\t')) && !TSC_ERROR.is_match(next) {
	...
	result.push_str(&format!("TypeScript: {} errors in {} files\n", errors.len(), by_file.len()));
...
";
	...
	fn test_no_file_limit() {
		...
		for i in 1..=15 {
			output.push_str(&format!("src/file{}.ts({},1): error TS2322: Error in file {}.\n", i, i, i));
		}
		let result = filter_tsc_output(&output);
		assert!(result.contains("15 errors in 15 files"));
		for i in 1..=15 {
			assert!(result.contains(&format!("file{}.ts", i)), "file{}.ts missing from output", i);
		}
	}
...
";
		let mut f = BlockStreamFilter::new(TscHandler::new());
		let result = run_block_filter(&mut f, input, 1);
		assert!(result.contains("Property 'children' does not exist"), "got: {}", result);
		assert!(result.contains("TS2322"), "got: {}", result);
		assert!(result.contains("TS2345"), "got: {}", result);
	}
}
