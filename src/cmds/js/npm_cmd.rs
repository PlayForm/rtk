...
fn run_filtered(name: &str, args: &[String], verbose: u8, skip_env: bool) -> Result<i32> {
	...
	runner::run_filtered(cmd, name, &args_display, filter_npm_output, runner::RunOptions::default())
}
...
"#;
	...
	fn test_npm_subcommand_routing() {
		...
		for subcmd in NPM_SUBCOMMANDS {
			assert!(!needs_run_injection(&[subcmd]), "'npm {}' should NOT inject 'run'", subcmd);
		}

		// Script names SHOULD get "run" injected
		for script in &["build", "dev", "lint", "typecheck", "deploy"] {
			assert!(needs_run_injection(&[script]), "'npm {}' SHOULD inject 'run'", script);
		}
