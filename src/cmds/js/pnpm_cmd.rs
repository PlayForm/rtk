...
use crate::parser::{
	emit_degradation_warning, emit_passthrough_warning, truncate_passthrough, Dependency, DependencyState, FormatMode,
	OutputParser, ParseResult, TokenFormatter,
};
	...
	fn parse(input: &str) -> ParseResult<DependencyState> {
		...
		match serde_json::from_str::<Vec<PnpmListOutput>>(input) {
			Ok(json) => {
				...
				for pkg in &json {
					collect_dependencies(pkg.name.as_str(), &pkg.package, false, &mut dependencies, &mut total_count);
				}
			...
			Err(e) => {
				...
				match extract_list_text(input) {
					Some(result) => ParseResult::Degraded(result, vec![format!("JSON parse failed: {}", e)]),
					None => {
						// Tier 3: Passthrough
...
fn extract_list_text(output: &str) -> Option<DependencyState> {
	...
	if count > 0 {
		Some(DependencyState { total_packages: count, outdated_count: 0, dependencies })
	} else {
		None
	}
}
	...
	fn parse(input: &str) -> ParseResult<DependencyState> {
		...
		match serde_json::from_str::<PnpmOutdatedOutput>(input) {
			Ok(json) => {
				...
				let result = DependencyState { total_packages: dependencies.len(), outdated_count, dependencies };
...
fn extract_outdated_text(output: &str) -> Option<DependencyState> {
	...
	if !dependencies.is_empty() {
		Some(DependencyState { total_packages: dependencies.len(), outdated_count, dependencies })
	} else {
		None
	}
}
...
fn format_dependency_listing(state: &DependencyState, cap: bool) -> String {
	...
	let mut lines = vec![format!("{} packages ({} prod / {} dev)", total, prod.len(), dev.len())];
	...
	if !prod.is_empty() {
		...
		if cap && prod.len() > MAX_LISTING {
			...
			let all_prod = prod
				...
				.collect::<Vec<_>>()
				.join("\n");
			if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_prod, "pnpm-prod", MAX_LISTING + 1) {
	...
	if !dev.is_empty() {
		...
		if cap && dev.len() > MAX_LISTING {
			...
			let all_dev = dev
				...
				.collect::<Vec<_>>()
				.join("\n");
			if let Some(hint) = crate::core::tee::force_tee_tail_hint(&all_dev, "pnpm-dev", MAX_LISTING + 1) {
				lines.push(format!("  {}", hint));
			}
		}
	}
...
fn run_list(depth: usize, args: &[String], verbose: u8) -> Result<i32> {
	...
	let is_filtered = args.iter().any(|a| matches!(a.as_str(), "--prod" | "-P" | "--dev" | "-D"));
...
mod tests {
	...
	fn make_state(prod: &[&str], dev: &[&str]) -> DependencyState {
		...
		DependencyState { total_packages: deps.len(), outdated_count: 0, dependencies: deps }
	}
