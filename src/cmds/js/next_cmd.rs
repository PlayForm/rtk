...
fn filter_next_build(output: &str) -> String {
	...
	for line in clean_output.lines() {
		...
		if let Some(caps) = BUNDLE_PATTERN.captures(line) {
			...
			let pct_change = if total > 0.0 { Some(((total - size) / size) * 100.0) } else { None };
	...
	if !bundles.is_empty() {
		...
		for (route, size, pct_change) in bundles.iter().take(MAX_BUNDLES) {
			let warning_marker = if let Some(pct) = pct_change {
				if *pct > 10.0 { format!(" [warn] (+{:.0}%)", pct) } else { String::new() }
			} else {
				String::new()
			};

			result.push_str(&format!("  {:<30} {:>6.0} kB{}\n", truncate(route, 30), size, warning_marker));
		}
...
"#;
	...
	fn test_extract_time() {
		...
		assert_eq!(extract_time("Compiled in 1250ms"), Some("1250ms".to_string()));
