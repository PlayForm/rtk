...
use crate::parser::{
	emit_degradation_warning, emit_passthrough_warning, extract_json_object, truncate_passthrough, FormatMode,
...
fn extract_stats_regex(output: &str) -> Option<TestResult> {
	...
	let duration_ms = DURATION_RE.captures(&clean_output).and_then(|caps| {
		...
		let unit = &caps[2];
		Some(if unit == "ms" { value as u64 } else { (value * 1000.0) as u64 })
	});
...
pub fn run_test(command: &Commands, args: &[String], verbose: u8) -> Result<i32> {
	...
	for arg in args {
		if arg == "run" || arg.starts_with("--json") || arg.starts_with("--reporter") || arg.starts_with("--watch") {
			continue;
		}
		cmd.arg(arg);
	}
