...
use crate::parser::{
	emit_degradation_warning, emit_passthrough_warning, truncate_passthrough, FormatMode, OutputParser, ParseResult,
...
fn collect_test_results(suites: &[PlaywrightSuite], total: &mut usize, failures: &mut Vec<TestFailure>) {
	for suite in suites {
			...
			*total += 1;
			...
			if !spec.ok {
				...
				let error_msg = spec
					...
					.and_then(|t| t.results.iter().find(|r| r.status == "failed" || r.status == "timedOut"))
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
	...
	let result = exec_capture(&mut cmd).context("Failed to run playwright (try: npm install -g playwright)")?;
