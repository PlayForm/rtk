...
fn run_generate(args: &[String], verbose: u8) -> Result<i32> {
	...
	let result = exec_capture(&mut cmd).context("Failed to run prisma generate (try: npm install -g prisma)")?;
...
fn filter_prisma_generate(output: &str) -> String {
	...
	if models > 0 || enums > 0 || types > 0 {
		result.push_str(&format!("  • {} models, {} enums, {} types\n", models, enums, types));
	}
...
fn filter_migrate_status(output: &str) -> String {
	...
	let mut result = String::new();
	result.push_str(&format!("Migrations: {} applied, {} pending\n", applied_count, pending_count));
...
fn extract_number(line: &str) -> Option<usize> {
	line.split_whitespace().find_map(|word| word.parse::<usize>().ok())
}
