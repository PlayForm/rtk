...
where
	F: Fn(&str, i32) -> String,
{
	let stdin_mode = if opts.inherit_stdin { StdinMode::Inherit } else { StdinMode::Null };
	...
	let text_to_filter = if opts.filter_stdout_only { raw_stdout } else { raw };
	...
	let raw_for_tracking = if opts.filter_stdout_only { raw_stdout } else { raw };
	timer.track(cmd_label, &format!("rtk {}", cmd_label), raw_for_tracking, &filtered);
	Ok(exit_code)
}
...
pub fn run(
	...
	match mode {
		RunMode::Filtered(filter_fn) => {
			run_captured_filter(cmd, tool_name, &cmd_label, move |text, _| filter_fn(text), opts, timer)
		},
		...
		RunMode::Streamed(filter) => {
			let result = stream::run_streaming(&mut cmd, StdinMode::Null, FilterMode::Streaming(filter))
				.with_context(|| format!("Failed to run {}", tool_name))?;

			if let Some(label) = opts.tee_label {
				if let Some(hint) = crate::core::tee::tee_and_hint(&result.raw, label, result.exit_code) {
					println!("{}", hint);
				}
			}
...
where
	F: Fn(&str) -> String,
{
	run(cmd, tool_name, args_display, RunMode::Filtered(Box::new(filter_fn)), opts)
}
...
pub fn run_passthrough(tool: &str, args: &[std::ffi::OsString], verbose: u8) -> Result<i32> {
	...
	run(cmd, tool, &args_str, RunMode::Passthrough, RunOptions::default())
}
...
pub fn run_streamed(
	...
	run(cmd, tool_name, args_display, RunMode::Streamed(filter), opts)
}
