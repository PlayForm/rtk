...
fn sanitize_slug(slug: &str) -> String {
	let sanitized: String = slug
		...
		.collect();
	if sanitized.len() > 40 { sanitized[..40].to_string() } else { sanitized }
}
...
pub fn tee_raw(raw: &str, command_slug: &str, exit_code: i32) -> Option<PathBuf> {
	...
	write_tee_file(raw, command_slug, &tee_dir, config.tee.max_file_size, config.tee.max_files)
}
...
fn force_tee_path(content: &str, command_slug: &str) -> Option<PathBuf> {
	...
	write_tee_file(content, command_slug, &tee_dir, config.tee.max_file_size, config.tee.max_files)
}
...
pub fn force_tee_tail_hint(content: &str, command_slug: &str, line_offset: usize) -> Option<String> {
	let path = force_tee_path(content, command_slug)?;
	Some(format!("[see remaining: tail -n +{} {}]", line_offset, display_path(&path)))
}
...
mod tests {
	...
	fn test_should_tee_disabled() {
		let config = TeeConfig { enabled: false, ..TeeConfig::default() };
	...
	fn test_should_tee_never_mode() {
		let config = TeeConfig { mode: TeeMode::Never, ..TeeConfig::default() };
		let dir = PathBuf::from("/tmp/tee");
		assert!(should_tee(&config, 1000, 1, Some(dir)).is_none());
	}
	...
	fn test_should_tee_always_mode_success() {
		let config = TeeConfig { mode: TeeMode::Always, ..TeeConfig::default() };
		let dir = PathBuf::from("/tmp/tee");
	...
	fn test_write_tee_file_creates_file() {
		...
		let result = write_tee_file(&content, "cargo_test", tmpdir.path(), DEFAULT_MAX_FILE_SIZE, 20);
