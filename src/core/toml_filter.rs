...
fn compile_filter(name: String, def: TomlFilterDef) -> Result<CompiledFilter, String> {
	...
	let match_regex = Regex::new(&def.match_command).map_err(|e| format!("invalid match_command regex: {}", e))?;
	...
	let match_output = def
		...
		.map(|r| -> Result<CompiledMatchOutputRule, String> {
			let pat = r.pattern.clone();
			let pattern =
				Regex::new(&r.pattern).map_err(|e| format!("invalid match_output pattern '{}': {}", pat, e))?;
			let unless = r
				.unless
				.as_deref()
				.map(|u| Regex::new(u).map_err(|e| format!("invalid match_output unless pattern '{}': {}", u, e)))
				.transpose()?;
	...
	let line_filter = if !def.strip_lines_matching.is_empty() {
		...
		let set =
			RegexSet::new(&def.keep_lines_matching).map_err(|e| format!("invalid keep_lines_matching regex: {}", e))?;
		LineFilter::Keep(set)
	} else {
		LineFilter::None
	};
...
pub fn apply_filter(filter: &CompiledFilter, stdout: &str) -> String {
	...
	if let Some(max_chars) = filter.truncate_lines_at {
		lines = lines.into_iter().map(|l| crate::core::utils::truncate(&l, max_chars)).collect();
	}
...
pub fn run_filter_tests(filter_name_opt: Option<&str>) -> VerifyResults {
	...
	let mut tested_filter_names: std::collections::HashSet<String> = std::collections::HashSet::new();
...
fn collect_test_outcomes(
	...
	for (filter_name, tests) in file.tests {
		...
		let compiled = match compiled_filters.get(&filter_name) {
			Some(f) => f,
			None => {
				eprintln!("[rtk] warning: [[tests.{}]] references unknown filter", filter_name);
				continue;
			},
		};
...
"#;
		...
		assert!(outcomes[0].passed, "test should pass: {:?}", outcomes[0].actual);
	}
...
"#;
	...
	fn test_builtin_all_expected_filters_present() {
		let filters = make_filters(BUILTIN_TOML);
		let names: std::collections::HashSet<&str> = filters.iter().map(|f| f.name.as_str()).collect();
	...
	fn test_builtin_all_filters_have_inline_tests() {
		...
		let mut outcomes = Vec::new();
		collect_test_outcomes(BUILTIN_TOML, None, &mut outcomes, &mut all_names, &mut tested);
...
"#;
		...
		assert_eq!(filters.len(), 59, "Expected 59 filters after concat (58 built-in + 1 new)");

		// New filter is discoverable
		let found = find_filter_in("my-new-tool --verbose", &filters);
		assert!(found.is_some(), "Newly added filter must be discoverable via find_filter_in");
