...
pub fn print_period_table<T: PeriodStats>(data: &[T]) {
	...
	let avg_time = if total_cmds > 0 { total_time / total_cmds as u64 } else { 0 };
...
impl PeriodStats for WeekStats {
	...
	fn period(&self) -> String {
		let start = if self.week_start.len() > 5 { &self.week_start[5..] } else { &self.week_start };
		let end = if self.week_end.len() > 5 { &self.week_end[5..] } else { &self.week_end };
		format!("{} → {}", start, end)
	}
