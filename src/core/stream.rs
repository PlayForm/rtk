...
impl<H: BlockHandler> BlockStreamFilter<H> {
	pub fn new(handler: H) -> Self {
		Self { handler, in_block: false, current_block: Vec::new(), blocks_emitted: 0 }
	}
...
impl RegexBlockFilter {
	pub fn new(tool_name: &str, start_pattern: &str) -> Self {
		Self {
			start_re: Regex::new(start_pattern)
				.unwrap_or_else(|e| panic!("RegexBlockFilter: bad pattern '{}': {}", start_pattern, e)),
	...
	pub fn skip_prefixes(mut self, prefixes: &[&str]) -> Self {
		self.skip_prefixes.extend(prefixes.iter().map(|s| s.to_string()));
		self
	}
}
...
pub fn run_streaming(cmd: &mut Command, stdin_mode: StdinMode, stdout_mode: FilterMode<'_>) -> Result<StreamResult> {
	...
	let stdin_thread: Option<std::thread::JoinHandle<()>> = match stdin_mode {
		StdinMode::Filter(mut filter) => {
			...
			Some(std::thread::spawn(move || {
				...
				for line in BufReader::new(stdin_handle.lock()).lines().map_while(Result::ok) {
	...
	if is_streaming {
		...
		if let FilterMode::Streaming(mut filter) = stdout_mode {
			...
			let flush_dest: &mut dyn Write = if filter_fd_is_stderr { &mut err_out } else { &mut out };
...

		{
			...
			match stdout_mode {
				...
				FilterMode::Buffered(filter_fn) => {
					...
					filtered = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| filter_fn(&raw_stdout)))
						.unwrap_or_else(|_| {
	...
	Ok(StreamResult { exit_code, raw, raw_stdout, raw_stderr, filtered })
}
...
pub(crate) mod tests {
	...
	fn test_line_filter_drops_lines() {
		let mut f = LineFilter::new(|l| if l.starts_with('#') { None } else { Some(l.to_string()) });
	...
	fn test_run_streaming_streaming_filter_drops_lines() {
		...
		let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::Streaming(Box::new(filter))).unwrap();
	...
	fn test_run_streaming_raw_cap_at_10mb() {
		...
		assert!(result.raw.len() > 1_000_000, "Should have captured significant data");
	}
	...
	fn test_capture_result_combined_empty() {
		let r = CaptureResult { stdout: String::new(), stderr: String::new(), exit_code: 0 };
	...
	fn test_block_filter_emits_blocks() {
		...
		assert!(result.contains("ERROR second\n  detail2"), "got: {}", result);
	...
	fn test_regex_block_filter_emits_blocks() {
		...
		assert!(result.contains("error[E0599]: no method\n  help: try"), "got: {}", result);
		assert!(result.contains("test: 2 blocks in output"), "got: {}", result);
	}
	...
	fn test_regex_block_filter_multiple_skip_prefixes() {
		let handler = RegexBlockFilter::new("test", r"^error").skip_prefixes(&["note:", "warning:", "help:"]);
		let mut f = BlockStreamFilter::new(handler);
	...
	fn test_line_filter_summary_propagates_exit_code() {
		let handler = CountingLineHandler { observed: Vec::new(), skip_prefixes: Vec::new(), summary_tag: "demo" };
		let mut f = LineStreamFilter::new(handler);
