...
impl Tracker {
	...
	pub fn new() -> Result<Self> {
		...
		let _ = conn.execute("ALTER TABLE commands ADD COLUMN exec_time_ms INTEGER DEFAULT 0", []);
		// Migration: add project_path column with DEFAULT '' for new rows // changed: added DEFAULT
		let _ = conn.execute("ALTER TABLE commands ADD COLUMN project_path TEXT DEFAULT ''", []);
		// One-time migration: normalize NULLs from pre-default schema // changed: guarded with EXISTS
		let has_nulls: bool = conn
			.query_row("SELECT EXISTS(SELECT 1 FROM commands WHERE project_path IS NULL)", [], |row| {
				row.get(0)
			})
			.unwrap_or(false);
		if has_nulls {
			let _ = conn.execute("UPDATE commands SET project_path = '' WHERE project_path IS NULL", []);
		}
		...
		conn.execute("CREATE INDEX IF NOT EXISTS idx_pf_timestamp ON parse_failures(timestamp)", [])?;
	...
	fn cleanup_old(&self) -> Result<()> {
		...
		self.conn
			.execute("DELETE FROM commands WHERE timestamp < ?1", params![cutoff.to_rfc3339()])?;
		self.conn
			.execute("DELETE FROM parse_failures WHERE timestamp < ?1", params![cutoff.to_rfc3339()])?;
		Ok(())
	}
	...
	pub fn record_parse_failure(&self, raw_command: &str, error_message: &str, fallback_succeeded: bool) -> Result<()> {
		self.conn.execute(
			"INSERT INTO parse_failures (timestamp, raw_command, error_message, fallback_succeeded)
             VALUES (?1, ?2, ?3, ?4)",
			params![Utc::now().to_rfc3339(), raw_command, error_message, fallback_succeeded as i32,],
		)?;
		self.cleanup_old()?;
		Ok(())
	}
	...
	pub fn get_parse_failure_summary(&self) -> Result<ParseFailureSummary> {
		...
		let succeeded: i64 =
			self.conn
				.query_row("SELECT COUNT(*) FROM parse_failures WHERE fallback_succeeded = 1", [], |row| {
					row.get(0)
				})?;

		let recovery_rate = if total > 0 { (succeeded as f64 / total as f64) * 100.0 } else { 0.0 };
		...
		Ok(ParseFailureSummary { total: total as usize, recovery_rate, top_commands, recent })
	}
	...
	pub fn get_all_days_filtered(&self, project_path: Option<&str>) -> Result<Vec<DayStats>> {
		...
		let rows = stmt.query_map(params![project_exact, project_glob], |row| {
			...
			let avg_time_ms = if commands > 0 { total_time / commands as u64 } else { 0 };
	...
	pub fn count_commands_since(&self, since: chrono::DateTime<chrono::Utc>) -> Result<i64> {
		...
		let count: i64 =
			self.conn
				.query_row("SELECT COUNT(*) FROM commands WHERE timestamp >= ?1", params![ts], |row| {
					row.get(0)
				})?;
		Ok(count)
	}
	...
	pub fn total_tokens_saved(&self) -> Result<i64> {
		let saved: i64 = self
			.conn
			.query_row("SELECT COALESCE(SUM(saved_tokens), 0) FROM commands", [], |row| row.get(0))?;
		Ok(saved)
	}
	...
	pub fn first_seen_days(&self) -> Result<i64> {
		let oldest: Option<String> =
			match self.conn.query_row("SELECT MIN(timestamp) FROM commands", [], |row| row.get(0)) {
				Ok(v) => v,
...
fn categorize_command(rtk_cmd: &str) -> String {
	...
	match tool {
		"git" | "gh" | "gt" => "git",
		"cargo" => "cargo",
		"npm" | "npx" | "pnpm" | "vitest" | "tsc" | "lint" | "prettier" | "next" | "playwright" | "prisma" => "js",
		"pytest" | "ruff" | "mypy" | "pip" => "python",
		"go" | "golangci-lint" => "go",
		"docker" | "kubectl" => "cloud",
		"rspec" | "rubocop" | "rake" => "ruby",
		"dotnet" => "dotnet",
		"ls" | "tree" | "grep" | "find" | "wc" | "read" | "env" | "json" | "log" | "smart" | "diff" | "deps"
		| "summary" | "format" => "system",
		_ => "other",
	}
	.to_string()
}
...
mod tests {
	...
	fn test_tracker_record_and_recent() {
		...
		tracker.record("git status", &test_cmd, 100, 20, 50).expect("Failed to record");
	...
	fn test_track_passthrough_no_dilution() {
		...
		tracker.record("cmd1", &cmd1, 1000, 200, 10).expect("Failed to record cmd1");

		// Record passthrough (0, 0)
		tracker.record("cmd2", &cmd2, 0, 0, 5).expect("Failed to record passthrough");

		// Verify both records exist in recent history
		let recent = tracker.get_recent(20).expect("Failed to get recent");

		let record1 = recent.iter().find(|r| r.rtk_cmd == cmd1).expect("cmd1 record not found");
		let record2 = recent.iter().find(|r| r.rtk_cmd == cmd2).expect("passthrough record not found");
	...
	fn test_project_filter_params_glob_pattern() {
		...
		assert_eq!(glob_val, format!("/home/user/project{}*", std::path::MAIN_SEPARATOR));
	}
	...
	fn test_project_filter_params_underscore_safe() {
		...
		assert_eq!(glob_val, format!("/home/user/my_project{}*", std::path::MAIN_SEPARATOR));
	}
	...
	fn test_parse_failure_roundtrip() {
		...
		let summary = tracker.get_parse_failure_summary().expect("Failed to get summary");
	...
	fn test_parse_failure_recovery_rate() {
		...
		tracker.record_parse_failure(&format!("cmd_ok1_{}", pid), "err", true).unwrap();
		tracker.record_parse_failure(&format!("cmd_ok2_{}", pid), "err", true).unwrap();
		tracker
	...
	fn test_reset_all_clears_both_tables() {
		...
		tracker
			.record("git status", &format!("rtk git status reset_test_{}", pid), 100, 20, 50)
		...
		assert_eq!(summary.total_commands, 0, "commands table should be empty after reset");

		let failures = tracker.get_parse_failure_summary().expect("Failed to get failure summary");
		assert_eq!(failures.total, 0, "parse_failures table should be empty after reset");
	}
}
