...
impl Language {
	pub fn from_extension(ext: &str) -> Self {
		match ext.to_lowercase().as_str() {
			...
			"json" | "jsonc" | "json5" | "yaml" | "yml" | "toml" | "xml" | "csv" | "tsv" | "graphql" | "gql"
			| "sql" | "md" | "markdown" | "txt" | "env" | "lock" => Language::Data,
			_ => Language::Unknown,
		}
	}
...
lazy_static! {
	...
	static ref FUNC_SIGNATURE: Regex =
  ...
  "lint-staged": {
    "**/package.json": [
      "sort-package-json"
		...
		assert!(result.contains("scripts"), "scripts section must not be stripped");
		assert!(result.contains("lint-staged"), "lint-staged section must not be stripped");
		assert!(
