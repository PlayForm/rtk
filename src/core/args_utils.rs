...
mod tests {
	...
	fn test_args_before_dash() {
		...
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["name", "--", "--nocapture"]);
	}
	...
	fn test_dashes_with_args_between() {
		...
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["--", "arg1", "--", "arg2"]);
	}
	...
	fn test_dash_already_preserved() {
		...
		let raw = vec!["rtk", "cargo", "clippy", "-p", "pkg", "--", "-D", "warnings"];
		let parsed = vec!["-p", "pkg", "--", "-D", "warnings"];
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["-p", "pkg", "--", "-D", "warnings"]);
	}
	...
	fn test_no_dash_in_original() {
		...
		let cases = vec![
			(vec!["rtk", "git", "diff", "feature/auth"], vec!["feature/auth"]),
			(vec!["rtk", "git", "diff", "main...feature"], vec!["main...feature"]),
			(vec!["rtk", "git", "diff", "main"], vec!["main"]),
			(vec!["rtk", "git", "diff", "--stat", "--cached"], vec!["--stat", "--cached"]),
		];
	...
	fn test_duplicate_args_both_sides() {
		...
		let raw = vec!["rtk", "cargo", "clippy", "-p", "p1", "-p", "p2", "--", "-p", "p3"];
		let parsed = vec!["-p", "p1", "-p", "p2", "-p", "p3"];
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["-p", "p1", "-p", "p2", "--", "-p", "p3"]);
	}
	...
	fn test_cargo_test_named_cargo() {
		...
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["cargo", "--", "--nocapture"]);
	}
	...
	fn test_git_diff_flags_before_path() {
		...
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["--cached", "--", "file"]);
	}
	...
	fn test_git_diff_multiple_files() {
		...
		assert_eq!(restore_with_raw(&parsed, &raw), vec!["--", "file1", "file2", "file3"]);
	}
}
