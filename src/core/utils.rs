...
pub fn fallback_tail(output: &str, label: &str, n: usize) -> String {
	eprintln!("[rtk] {}: output format not recognized, showing last {} lines", label, n);
	...
	match resolve_binary(name) {
		...
		Err(e) => {
			...
			if cfg!(any(target_os = "windows", debug_assertions)) {
				eprintln!("rtk: Failed to resolve '{}' via PATH, falling back to direct exec: {}", name, e);
			}
...
mod tests {
	...
	fn test_resolve_binary_fails_for_unknown() {
		...
		assert!(result.is_err(), "resolve_binary should fail for nonexistent binary");
	}

	#[test]
	fn test_resolve_binary_path_contains_binary_name() {
		let path = resolve_binary("cargo").expect("cargo should be resolvable");
		let filename = path.file_name().expect("should have filename").to_string_lossy();
	...
	fn test_resolved_command_executes_known_command() {
		...
		assert!(output.status.success(), "cargo --version should succeed via resolved_command");
	}
	...
	fn test_tool_exists_finds_cargo() {
		assert!(tool_exists("cargo"), "tool_exists('cargo') should return true");
	}
	...
	mod windows_tests {
		...
		fn test_resolve_binary_finds_cmd_wrapper() {
			...
			let result = which::which_in("fake-tool-test", Some(search_path), std::env::current_dir().unwrap());
		...
		fn test_resolve_binary_finds_bat_wrapper() {
			...
			let search_path = path_with_dir(temp_dir.path());
			let result = which::which_in("fake-bat-tool", Some(search_path), std::env::current_dir().unwrap());
		...
		fn test_resolved_command_executes_cmd_wrapper() {
			...
			let search_path = path_with_dir(temp_dir.path());
			let resolved = which::which_in("fake-exec-test", Some(search_path), std::env::current_dir().unwrap())
		...
		fn test_tool_exists_finds_cmd_wrapper() {
			...
			let search_path = path_with_dir(temp_dir.path());
			let result = which::which_in("fake-exists-test", Some(search_path), std::env::current_dir().unwrap());

			assert!(result.is_ok(), "which_in should find .cmd wrapper on Windows");
		}
	}
	...
	fn test_shorten_arn_lambda() {
		assert_eq!(shorten_arn("arn:aws:lambda:us-west-2:123:function:my-function"), "my-function");
	}
