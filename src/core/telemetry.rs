...
fn send_ping() -> Result<(), Box<dyn std::error::Error>> {
	...
	let (commands_24h, top_commands, savings_pct, tokens_saved_24h, tokens_saved_total) = match &tracker {
		Some(t) => get_stats(t),
		None => (0, vec![], None, 0, 0),
	};
...
fn get_stats(tracker: &tracking::Tracker) -> (i64, Vec<String>, Option<f64>, i64, i64) {
	...
	(commands_24h, top_commands, savings_pct, tokens_saved_24h, tokens_saved_total)
}
...
fn detect_has_config() -> bool {
	dirs::config_dir().map(|d| d.join("rtk/config.toml").exists()).unwrap_or(false)
}
...
fn detect_install_method() -> &'static str {
	...
	let real_path = std::fs::canonicalize(&exe).unwrap_or(exe).to_string_lossy().to_string();
...
mod tests {
	...
	fn test_install_method_unix_paths() {
		assert_eq!(install_method_from_path("/opt/homebrew/Cellar/rtk/0.28.0/bin/rtk"), "homebrew");
		assert_eq!(install_method_from_path("/usr/local/homebrew/bin/rtk"), "homebrew");
	...
	fn test_install_method_windows_paths() {
		assert_eq!(install_method_from_path("C:\\Users\\user\\.cargo\\bin\\rtk.exe"), "cargo");
		assert_eq!(install_method_from_path("C:\\Users\\user\\.local\\bin\\rtk.exe"), "script");
		assert_eq!(install_method_from_path("C:\\Program Files\\rtk\\rtk.exe"), "other");
	}
	...
	fn test_detect_hook_type_returns_known() {
		...
		assert!(
			["claude", "gemini", "codex", "cursor", "copilot", "none", "unknown"].contains(&ht.as_str()),
			"Unexpected hook type: {}",
			ht
		);
	}
