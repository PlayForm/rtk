...
fn run_status() -> Result<()> {
	...
	let enabled_str = if config.telemetry.enabled { "yes" } else { "no" };
...
fn run_enable() -> Result<()> {
	...
	if !io::stdin().is_terminal() {
		anyhow::bail!("consent requires interactive terminal — cannot enable telemetry in piped mode");
	}
...
fn run_forget() -> Result<()> {
	...
	if salt_path.exists() {
		std::fs::remove_file(&salt_path).with_context(|| format!("Failed to delete {}", salt_path.display()))?;
	}
