...
pub(crate) fn check_command_with_rules(
	...
	for segment in &segments {
		...
		if all_segments_allowed {
			let matched = allow_rules.iter().any(|pattern| command_matches_pattern(segment, pattern));
			if !matched {
...
fn gemini_settings() -> Option<Value> {
	...
	let trusted = std::env::var("GEMINI_CLI_TRUST_WORKSPACE").as_deref() == Ok("true")
		|| !global
			.as_ref()
			.and_then(|j| j.pointer("/security/folderTrust/enabled").and_then(Value::as_bool))
			.unwrap_or(false);
	if trusted {
		if let Some(root) = find_project_root() {
...
mod tests {
	...
	fn test_parse_bash_pattern() {
		assert_eq!(extract_bash_pattern("Bash(git push --force)"), "git push --force");
	...
	#[test]
	fn test_exact_match() {
		assert!(command_matches_pattern("git push --force", "git push --force"));
	}
	...
	fn test_non_bash_rules_ignored() {
		...
		assert_eq!(check_command_with_rules("cat .env", &[], &[], &[]), PermissionVerdict::Default);
	}
	...
	fn test_prefix_match() {
		assert!(command_matches_pattern("git push --force origin main", "git push --force"));
	}
	...
	fn test_no_partial_word_match() {
		...
		assert!(!command_matches_pattern("git push --forceful", "git push --force"));
	}
	...
	fn test_middle_wildcard_at_end_of_command() {
		...
		assert!(command_matches_pattern("git -C /path diff --stat", "git -C * diff:*"));
		// Must NOT match a different subcommand
		assert!(!command_matches_pattern("git -C /path status", "git -C * diff:*"));
	}

	// Bug 3: multiple wildcards
	#[test]
	fn test_multiple_wildcards() {
		assert!(command_matches_pattern("git push --force origin main", "git * --force *"));
		assert!(!command_matches_pattern("git pull origin main", "git * --force *"));
	}
	...
	fn test_deny_star_colon_star() {
		...
		assert_eq!(check_command_with_rules("rm -rf /", &deny, &[], &[]), PermissionVerdict::Deny);
	}
	...
	fn test_compound_allow_requires_every_segment() {
		...
		let allow = vec!["git status *".to_string(), "git status".to_string(), "cargo *".to_string()];
		...
		assert_eq!(
			check_command_with_rules("cargo test && git add . && git commit -m foo", &[], &[], &allow,),
			PermissionVerdict::Default,
	...
	fn test_compound_allow_all_segments_matched() {
		...
		assert_eq!(
			check_command_with_rules("git log --oneline && cargo build && git status", &[], &[], &allow),
			PermissionVerdict::Allow
		);
	}
	...
	fn test_double_quoted_substitution_never_auto_allowed() {
		let allow = vec!["git *".to_string()];
		for cmd in [r#"git log --pretty="$(rm -rf ~)""#, r#"git log --pretty="`rm -rf ~`""#] {
			assert_ne!(
				check_command_with_rules(cmd, &[], &[], &allow),
				PermissionVerdict::Allow,
				"{cmd} must not auto-allow"
	...
	fn test_wrapped_rules_extracted_patterns_match() {
		let mut allow = Vec::new();
		append_wrapped_rules(Some(&serde_json::json!(["Shell(git)"])), &["Shell("], &mut allow);
		assert_eq!(
			check_command_with_rules("git status", &[], &[], &allow),
			PermissionVerdict::Allow
		);
