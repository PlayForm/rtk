...
use super::constants::{CLAUDE_HOOK_COMMAND, HOOKS_SUBDIR, PRE_TOOL_USE_KEY, REWRITE_HOOK_FILE, SETTINGS_JSON};
...
fn check_and_warn() -> Option<()> {
	let warning = match status() {
		...
		HookStatus::Missing => "[rtk] /!\\ No hook installed — run `rtk init -g` for automatic token savings",
...
mod tests {
	...
	use crate::hooks::constants::{
		...
		HERMES_PLUGIN_MANIFEST_FILE, HERMES_PLUGIN_NAME, OPENCODE_PLUGIN_FILE, OPENCODE_SUBDIR, PLUGIN_SUBDIR,
	};
	...
	fn other_integration_installed(home: &std::path::Path) -> bool {
		let paths = [
			...
			home.join(CURSOR_DIR).join(HOOKS_SUBDIR).join(REWRITE_HOOK_FILE),
			home.join(CODEX_DIR).join("AGENTS.md"),
			home.join(GEMINI_DIR).join(HOOKS_SUBDIR).join(GEMINI_HOOK_FILE),
	...
	fn test_other_integration_cursor() {
		let tmp = tempfile::tempdir().expect("tempdir");
		let path = tmp.path().join(CURSOR_DIR).join(HOOKS_SUBDIR).join(REWRITE_HOOK_FILE);
		std::fs::create_dir_all(path.parent().unwrap()).unwrap();
	...
	fn test_other_integration_gemini() {
		let tmp = tempfile::tempdir().expect("tempdir");
		let path = tmp.path().join(GEMINI_DIR).join(HOOKS_SUBDIR).join(GEMINI_HOOK_FILE);
		std::fs::create_dir_all(path.parent().unwrap()).unwrap();
		std::fs::write(&path, b"hook").unwrap();
		assert!(other_integration_installed(tmp.path()));
	}
	...
	fn test_other_integration_empty_dirs_not_enough() {
		...
		std::fs::create_dir_all(tmp.path().join(HERMES_DIR).join(HERMES_PLUGINS_SUBDIR).join(HERMES_PLUGIN_NAME))
			.unwrap();
		assert!(!other_integration_installed(tmp.path()));
	}
