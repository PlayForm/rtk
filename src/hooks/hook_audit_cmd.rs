...
fn default_log_path() -> PathBuf {
	if let Ok(dir) = std::env::var("RTK_AUDIT_DIR") {
		...
		PathBuf::from(home).join(".local/share/rtk").join("hook-audit.log")
	}
}
...
fn base_command(cmd: &str) -> String {
	...
	let stripped = cmd.split_whitespace().skip_while(|w| w.contains('=')).collect::<Vec<_>>();
...
fn filter_since_days(entries: &[AuditEntry], days: u64) -> Vec<&AuditEntry> {
	...
	entries.iter().filter(|e| e.timestamp >= cutoff_str).collect()
}
...
pub fn run(since_days: u64, verbose: u8) -> Result<()> {
	...
	let content = std::fs::read_to_string(&log_path).context(format!("Failed to read {}", log_path.display()))?;
	...
	for entry in &filtered {
		*action_counts.entry(&entry.action).or_insert(0) += 1;
		if entry.action == "rewrite" {
			*cmd_counts.entry(base_command(&entry.original_cmd)).or_insert(0) += 1;
		}
	}
	...
	let rewrite_pct = if total > 0 { rewrites as f64 / total as f64 * 100.0 } else { 0.0 };
	let skip_pct = if total > 0 { skips as f64 / total as f64 * 100.0 } else { 0.0 };
	...
	if !skip_actions.is_empty() {
		...
		for (action, count) in &sorted_skips {
			...
			println!("  {}:{}{}", reason, " ".repeat(14 - reason.len().min(13)), count);
		}
	}
...
mod tests {
	...
	fn test_filter_since_days_zero_returns_all() {
		let entries = vec![make_entry("rewrite", "git status"), make_entry("skip:no_match", "echo hi")];
...
2026-02-16T14:30:08Z | rewrite | cargo clippy --all-targets | rtk cargo clippy --all-targets"#;
		...
		let skips = entries.iter().filter(|e| e.action.starts_with("skip:")).count();
		assert_eq!(skips, 3);
		...
		assert!(savings >= 30.0, "Expected >=30% savings for audit summary, got {:.1}%", savings);
	}
}
