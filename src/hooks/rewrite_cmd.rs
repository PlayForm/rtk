...
mod tests {
	...
	fn test_run_already_rtk_returns_some() {
		assert_eq!(rewrite_command_no_prefixes("rtk git status"), Some("rtk git status".into()));
	}
	...
	mod unattestable_passthrough {
		...
		fn test_backtick_substitution_passthrough() {
			assert_eq!(evaluate("git status `rm -rf /tmp/x`", &[], &[]), RewriteOutcome::Passthrough);
		}
		...
		fn test_file_redirect_passthrough() {
			assert_eq!(evaluate("git log > /tmp/out.txt", &[], &[]), RewriteOutcome::Passthrough);
		}

		#[test]
		fn test_fd_dup_redirect_still_rewrites() {
			assert!(matches!(evaluate("git status 2>&1", &[], &[]), RewriteOutcome::Ask(_)));
		}
