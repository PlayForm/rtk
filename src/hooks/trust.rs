...
fn read_store() -> Result<TrustStore> {
	...
	let content =
		std::fs::read_to_string(&path).with_context(|| format!("Failed to read trust store: {}", path.display()))?;
	serde_json::from_str(&content).with_context(|| format!("Failed to parse trust store: {}", path.display()))
}
...
fn write_store(store: &TrustStore) -> Result<()> {
	...
	std::fs::write(&path, content).with_context(|| format!("Failed to write trust store: {}", path.display()))
}
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
	...
	if std::env::var("RTK_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
		...
		eprintln!("[rtk] WARNING: RTK_TRUST_PROJECT_FILTERS=1 ignored (CI environment not detected)");
	}
	...
	let actual_hash =
		integrity::compute_hash(filter_path).with_context(|| format!("Failed to hash: {}", filter_path.display()))?;

	if actual_hash == entry.sha256 {
		Ok(TrustStatus::Trusted)
	} else {
		Ok(TrustStatus::ContentChanged { expected: entry.sha256.clone(), actual: actual_hash })
	}
}
...
pub fn trust_filter_with_hash(filter_path: &Path, hash: &str) -> Result<()> {
	...
	store.trusted.insert(
		key,
		TrustEntry { sha256: hash.to_string(), trusted_at: chrono::Utc::now().to_rfc3339() },
	);
	write_store(&store)
}
...
pub fn run_trust(list: bool) -> Result<()> {
	...
	println!();
	println!("Trusted .rtk/filters.toml (sha256:{})", hash.get(..16).unwrap_or(&hash));
...
mod tests {
	...
	fn trust_with_store(filter_path: &Path, store_file: &Path) -> Result<()> {
		...
		store
			.trusted
			.insert(key, TrustEntry { sha256: hash, trusted_at: chrono::Utc::now().to_rfc3339() });
