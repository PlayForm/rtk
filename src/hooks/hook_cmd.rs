		...
		if tool_name == "bash" {
			if let Some(tool_args_str) = v.get("toolArgs").and_then(|t| t.as_str()) {
				if let Ok(tool_args) = serde_json::from_str::<Value>(tool_args_str) {
					if let Some(cmd) = tool_args.get("command").and_then(|c| c.as_str()).filter(|c| !c.is_empty()) {
						return HookFormat::CopilotCli { command: cmd.to_string(), args: tool_args };
					}
				}
			}
		}
...
fn copilot_cli_response(cmd: &str, args: &Value) -> Option<Value> {
	copilot_cli_response_from_decision(args, decide_hook_action(cmd, permissions::Host::Claude), cmd)
}

fn copilot_cli_response_from_decision(args: &Value, decision: HookDecision, cmd: &str) -> Option<Value> {
...
pub fn run_gemini() -> Result<()> {
	...
	let cmd = json.pointer("/tool_input/command").and_then(|v| v.as_str()).unwrap_or("");
...
enum PayloadAction {
	Rewrite { cmd: String, rewritten: String, output: Value },
	Skip { reason: &'static str, cmd: String },
	Ignore,
}
...
fn process_claude_payload(v: &Value) -> PayloadAction {
	...
	let (rewritten, allow) = match decide_hook_action(cmd, permissions::Host::Claude) {
		HookDecision::Deny => return PayloadAction::Skip { reason: "skip:deny_rule", cmd: cmd.to_string() },
		HookDecision::Defer => return PayloadAction::Skip { reason: "skip:defer", cmd: cmd.to_string() },
		HookDecision::AllowRewrite(r) => (r, true),
		HookDecision::AskRewrite(r) => (r, false),
	};
...
mod tests {
	...
	fn test_copilot_cli_deny_returns_none() {
		assert!(
			copilot_cli_response_from_decision(&cli_args("cargo test"), HookDecision::Deny, "cargo test",).is_none()
		);
	}
	...
	fn test_copilot_cli_preserves_env_prefix() {
		let r = copilot_cli_response("RUST_LOG=debug cargo test", &cli_args("RUST_LOG=debug cargo test")).unwrap();
		assert_eq!(r["modifiedArgs"]["command"], "RUST_LOG=debug rtk cargo test");
	}
	...
	fn end_to_end(cmd: &str) -> Option<Value> {
		let verdict = crate::hooks::permissions::check_command_with_rules(cmd, &[], &[], &["Bash(git:*)".to_string()]);
	...
	fn test_print_rewrite_format() {
		...
		assert_eq!(json["hookSpecificOutput"]["tool_input"]["command"], "rtk git status");
	}
	...
	fn test_gemini_hook_excluded_commands() {
		...
		assert_eq!(rewrite_command_no_prefixes("curl https://example.com", &excluded), None);
		assert_eq!(
	...
	fn test_cursor_substitution_defers_even_when_allowed() {
		assert_eq!(run_cursor_allowed(&cursor_input("git status `rm -rf /tmp/x`")), "{}");
		assert_eq!(run_cursor_allowed(&cursor_input("git status $(rm -rf /tmp/x)")), "{}");
	}
	...
	fn test_cursor_unallowed_segment_defers() {
		let out =
			run_cursor_inner_with_rules(&cursor_input("git status && rm -rf /tmp/x"), &[], &[], &["git *".to_string()]);
		assert_eq!(out, "{}");
	}
	...
	fn test_cursor_compound_rewrite_includes_continue() {
		...
		assert_eq!(v["continue"], true);
		assert_eq!(v["permission"], "allow");
		assert_eq!(v["updated_input"]["command"], "cd \"/tmp/proj\" && rtk git status");
	}
	...
	fn test_audit_log_format_four_fields() {
		...
		assert_eq!(parts.len(), 4, "Expected 4 pipe-delimited fields, got: {:?}", parts);
	...
	fn test_cursor_deny_blocks_rewrite() {
		...
		assert_eq!(check_command_with_rules("git status", &deny, &[], &[]), PermissionVerdict::Deny);
	}
	...
	fn test_gemini_deny_blocks_rewrite() {
		...
		assert_eq!(check_command_with_rules("cargo test", &deny, &[], &[]), PermissionVerdict::Deny);
	...
	fn decide_with_rules(cmd: &str, deny: &[String], ask: &[String], allow: &[String]) -> HookDecision {
	...
	fn test_decide_deny() {
		assert!(matches!(
			decide_with_rules("rm -rf /tmp/x", &["rm -rf".to_string()], &[], &all_allowed()),
			HookDecision::Deny
		));
	}
	...
	fn test_decide_defer_for_substitution_even_when_allowed() {
		for cmd in [
			...
			assert!(
				matches!(decide_with_rules(cmd, &[], &[], &all_allowed()), HookDecision::Defer),
	...
	fn gemini_render(cmd: &str, deny: &[String], ask: &[String], allow: &[String]) -> String {
		match decide_with_rules(cmd, deny, ask, allow) {
			HookDecision::Deny => r#"{"decision":"deny","reason":"Blocked by RTK permission rule"}"#.to_string(),
	...
	fn test_gemini_allow_emits_rewrite() {
		let v: Value = serde_json::from_str(&gemini_render("git status", &[], &[], &all_allowed())).unwrap();
		assert_eq!(v["decision"], "allow");
		assert_eq!(v["hookSpecificOutput"]["tool_input"]["command"], "rtk git status");
	}
	...
	fn test_gemini_substitution_asks_user_without_rewrite() {
		let v: Value =
			serde_json::from_str(&gemini_render("git status `rm -rf /tmp/x`", &[], &[], &all_allowed())).unwrap();
		assert_eq!(v["decision"], "ask_user");
		assert!(v.get("hookSpecificOutput").is_none());
	}
	...
	fn test_gemini_deny_decision() {
		let v: Value =
			serde_json::from_str(&gemini_render("rm -rf /tmp/x", &["rm -rf".to_string()], &[], &[])).unwrap();
