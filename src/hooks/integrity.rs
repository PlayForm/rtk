...
fn hash_path(hook_path: &Path) -> PathBuf {
	hook_path.parent().unwrap_or(Path::new(".")).join(HASH_FILENAME)
}
...
pub fn store_hash(hook_path: &Path) -> Result<()> {
	...
	let filename = hook_path.file_name().and_then(|n| n.to_str()).unwrap_or(REWRITE_HOOK_FILE);
	...
	fs::write(&hash_file, &content).with_context(|| format!("Failed to write hash to {}", hash_file.display()))?;
...
pub fn remove_hash(hook_path: &Path) -> Result<bool> {
	...
	fs::remove_file(&hash_file).with_context(|| format!("Failed to remove hash file: {}", hash_file.display()))?;
	...
	match (hook_path.exists(), hash_file.exists()) {
		...
		(true, true) => {
			...
			if stored == actual {
				...
				Ok(IntegrityStatus::Tampered { expected: stored, actual })
			}
		},
	}
}
...
fn read_stored_hash(path: &Path) -> Result<String> {
	let content = fs::read_to_string(path).with_context(|| format!("Failed to read hash file: {}", path.display()))?;
...
mod tests {
	...
	fn test_hash_only_no_filename_rejected() {
		...
		fs::write(&hash_file, "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2\n").unwrap();
