...
use super::constants::{
	BEFORE_TOOL_KEY, CLAUDE_DIR, CLAUDE_HOOK_COMMAND, CODEX_DIR, CURSOR_HOOK_COMMAND, GEMINI_HOOK_FILE, HERMES_DIR,
	HERMES_PLUGINS_SUBDIR, HERMES_PLUGIN_INIT_FILE, HERMES_PLUGIN_MANIFEST_FILE, HERMES_PLUGIN_NAME, HOOKS_JSON,
	HOOKS_SUBDIR, PI_CODING_AGENT_DIR_ENV, PI_DIR, PI_EXTENSIONS_SUBDIR, PI_LOCAL_DIR, PI_PLUGIN_FILE,
	PRE_TOOL_USE_KEY, REWRITE_HOOK_FILE, SETTINGS_JSON,
};
use super::integrity;
use crate::hooks::constants::{
	CONFIG_DIR, COPILOT_HOME_ENV, COPILOT_HOOK_FILE, COPILOT_INSTRUCTIONS_FILE, COPILOT_USER_DIR, CURSOR_DIR,
	GEMINI_DIR, GITHUB_DIR, OPENCODE_PLUGIN_FILE, OPENCODE_SUBDIR, PLUGIN_SUBDIR,
};
	...
	if codex {
		...
		if install_windsurf {
			...
			match (install_claude, install_opencode, claude_md, hook_only) {
				...
				(true, opencode, false, true) => run_hook_only_mode(global, patch_mode, opencode, ctx)?,
				(true, opencode, false, false) => run_default_mode(global, patch_mode, opencode, ctx)?,
				(false, false, _, _) => {
					if !install_cursor {
...
fn write_if_changed(path: &Path, content: &str, name: &str, ctx: InitContext) -> Result<bool> {
	...
	if path.exists() {
		let existing =
			fs::read_to_string(path).with_context(|| format!("Failed to read {}: {}", name, path.display()))?;
		...
		if existing == content {
			...
			if dry_run {
				...
				atomic_write(path, content).with_context(|| format!("Failed to write {}: {}", name, path.display()))?;
				if verbose > 0 {
...
fn atomic_write(path: &Path, content: &str) -> Result<()> {
	...
	let parent = target
		.parent()
		.with_context(|| format!("Cannot write to {}: path has no parent directory", target.display()))?;
	...
	temp_file
		.persist(&target)
		.with_context(|| format!("Failed to atomically replace {} (disk full?)", target.display()))?;
...
fn prompt_user_consent(settings_path: &Path) -> Result<bool> {
	...
	stdin.lock().read_line(&mut line).context("Failed to read user input")?;
...
pub fn save_telemetry_consent(accepted: bool) -> Result<()> {
	...
	config.save().context("Failed to save telemetry consent to config.toml")
}
...
fn remove_hook_from_json(root: &mut serde_json::Value) -> bool {
	let hooks = match root.get_mut("hooks").and_then(|h| h.get_mut(PRE_TOOL_USE_KEY)) {
...
fn remove_hook_from_settings(ctx: InitContext) -> Result<bool> {
	...
	let content =
		fs::read_to_string(&settings_path).with_context(|| format!("Failed to read {}", settings_path.display()))?;
	...
	if removed {
		if dry_run {
			println!("[dry-run] would remove RTK hook entry from {}", settings_path.display());
			if verbose > 0 {
				let serialized = serde_json::to_string_pretty(&root).context("Failed to serialize settings.json")?;
				println!("[dry-run] content:\n{}", serialized);
			}
			return Ok(true);
		}
...
pub fn uninstall(global: bool, gemini: bool, codex: bool, cursor: bool, pi: bool, ctx: InitContext) -> Result<()> {
	let InitContext { verbose, dry_run } = ctx;
	if codex {
	...
	if hook_path.exists() {
		if dry_run {
			println!("[dry-run] would remove hook script: {}", hook_path.display());
		} else {
			fs::remove_file(&hook_path).with_context(|| format!("Failed to remove hook: {}", hook_path.display()))?;
		}
			...
			if trimmed.is_empty() {
				if dry_run {
					...
					fs::remove_file(&claude_md_path)
						.with_context(|| format!("Failed to remove empty CLAUDE.md: {}", claude_md_path.display()))?;
				}
				...
				fs::write(&claude_md_path, &working_content)
					.with_context(|| format!("Failed to write CLAUDE.md: {}", claude_md_path.display()))?;
			}
		}
	}
	...
	if removed.is_empty() {
		...
		let header = if dry_run { "[dry-run] would uninstall RTK:" } else { "RTK uninstalled:" };
		println!("{}", header);
		for item in removed {
			println!("  - {}", item);
		}
...
fn uninstall_codex_at(codex_dir: &Path, ctx: InitContext) -> Result<Vec<String>> {
	...
	if agents_md_path.exists() {
		...
		if agents_changed {
			atomic_write(&agents_md_path, &working_content)
				.with_context(|| format!("Failed to write AGENTS.md: {}", agents_md_path.display()))?;
		}
	}

	if remove_rtk_reference_from_agents(&agents_md_path, &[RTK_MD_REF, absolute_rtk_md_ref.as_str()], ctx)? {
...
fn patch_settings_json_command(
	...
	match mode {
		...
		PatchMode::Ask => {
			...
			if dry_run {
				println!("[dry-run] would prompt before patching {}", settings_path.display());
	...
	if dry_run {
		println!("[dry-run] would patch settings.json: {}", settings_path.display());
		if verbose > 0 {
			println!("[dry-run] content:\n{}", serialized);
		}
	...
	if settings_path.with_extension("json.bak").exists() {
		println!("  Backup: {}", settings_path.with_extension("json.bak").display());
	}
	if include_opencode {
...
fn hook_already_present(root: &serde_json::Value, hook_command: &str) -> bool {
	...
	pre_tool_use_array
		...
		.any(|cmd| cmd == hook_command || cmd == CLAUDE_HOOK_COMMAND || cmd.contains(REWRITE_HOOK_FILE))
}
...
fn run_default_mode(global: bool, patch_mode: PatchMode, install_opencode: bool, ctx: InitContext) -> Result<()> {
	...
	let patch_result = patch_settings_json_command(CLAUDE_HOOK_COMMAND, patch_mode, install_opencode, ctx)?;
...
fn migrate_old_hook_script(ctx: InitContext) {
	...
	if let Some(home) = dirs::home_dir() {
		let old_hook = home.join(CLAUDE_DIR).join(HOOKS_SUBDIR).join(REWRITE_HOOK_FILE);
		if old_hook.exists() {
			if dry_run {
				println!("[dry-run] would migrate legacy hook script: {}", old_hook.display());
			// nosemgrep: filesystem-deletion
		...
		let hash_file = home.join(CLAUDE_DIR).join(HOOKS_SUBDIR).join(".rtk-hook.sha256");
		if hash_file.exists() {
			if dry_run {
				println!("[dry-run] would remove legacy hash file: {}", hash_file.display());
			} else {
		...
		if cursor_hook.exists() {
			if dry_run {
				println!("[dry-run] would remove legacy Cursor hook: {}", cursor_hook.display());
			} else {
...
fn remove_legacy_settings_entries(ctx: InitContext) -> Result<()> {
	...
	let mut root: serde_json::Value =
		serde_json::from_str(&content).with_context(|| format!("Failed to parse {}", settings_path.display()))?;
	...
	let backup_path = settings_path.with_extension("json.bak");
	fs::copy(&settings_path, &backup_path).with_context(|| format!("Failed to backup to {}", backup_path.display()))?;
...
fn generate_project_filters_template(ctx: InitContext) -> Result<()> {
	...
	if dry_run {
		println!("[dry-run] would create .rtk/filters.toml template: {}", path.display());
		return Ok(());
	}

	fs::create_dir_all(rtk_dir).with_context(|| format!("Failed to create directory: {}", rtk_dir.display()))?;
	fs::write(&path, FILTERS_TEMPLATE).with_context(|| format!("Failed to write {}", path.display()))?;

	println!("  filters:   {} (template, edit to add project filters)", path.display());
	Ok(())
}
...
fn generate_global_filters_template(ctx: InitContext) -> Result<()> {
	...
	if dry_run {
		println!("[dry-run] would create global filters template: {}", path.display());
		return Ok(());
	}

	fs::create_dir_all(&rtk_dir).with_context(|| format!("Failed to create directory: {}", rtk_dir.display()))?;
	fs::write(&path, FILTERS_GLOBAL_TEMPLATE).with_context(|| format!("Failed to write {}", path.display()))?;

	println!("  filters:   {} (template, edit to add user-global filters)", path.display());
	Ok(())
}

/// Hook-only mode: just the hook, no RTK.md
fn run_hook_only_mode(global: bool, patch_mode: PatchMode, install_opencode: bool, ctx: InitContext) -> Result<()> {
	let InitContext { dry_run, .. } = ctx;
	if !global {
...
fn run_claude_md_mode(global: bool, install_opencode: bool, ctx: InitContext) -> Result<()> {
	...
	let recovery_cmd = if global { "rtk init -g --claude-md" } else { "rtk init --claude-md" };

	let action = write_rtk_block(&path, RTK_INSTRUCTIONS, "rtk instructions", recovery_cmd, ctx)?;
	...
	if global {
		if install_opencode {
			...
			if !dry_run {
				println!("[ok] OpenCode plugin installed: {}", opencode_plugin_path.display());
			}
		}
		if !dry_run {
...
fn run_cline_mode(ctx: InitContext) -> Result<()> {
	...
	if existing.contains("RTK") || existing.contains("rtk") {
		...
		if dry_run {
			println!("[dry-run] would write .clinerules: {}", rules_path.display());
			if verbose > 0 {
				println!("[dry-run] content:\n{}", new_content);
			}
		} else {
...
fn run_windsurf_mode(ctx: InitContext) -> Result<()> {
	...
	if existing.contains("RTK") || existing.contains("rtk") {
		...
		if dry_run {
			println!("[dry-run] would write .windsurfrules: {}", rules_path.display());
			if verbose > 0 {
				println!("[dry-run] content:\n{}", new_content);
			}
		} else {
...
fn run_kilocode_mode_at(base_dir: &Path, ctx: InitContext) -> Result<()> {
	...
	if existing.contains("RTK") || existing.contains("rtk") {
		...
		if dry_run {
			...
			fs::write(&rules_path, &new_content).context("Failed to write .kilocode/rules/rtk-rules.md")?;
...
fn run_antigravity_mode_at(base_dir: &Path, ctx: InitContext) -> Result<()> {
	...
	if existing.contains("RTK") || existing.contains("rtk") {
		...
		if dry_run {
			...
			fs::write(&rules_path, &new_content).context("Failed to write .agents/rules/antigravity-rtk-rules.md")?;
...
fn hermes_plugin_dir(hermes_home: &Path) -> PathBuf {
	hermes_home.join(HERMES_PLUGINS_SUBDIR).join(HERMES_PLUGIN_NAME)
}
...
fn run_hermes_mode_at(hermes_home: &Path, ctx: InitContext) -> Result<()> {
	...
	if !dry_run {
		fs::create_dir_all(&plugin_dir)
			.with_context(|| format!("Failed to create Hermes plugin directory: {}", plugin_dir.display()))?;
	}
	...
	write_if_changed(&manifest_path, HERMES_PLUGIN_YAML, "Hermes plugin manifest", ctx)?;
...
fn uninstall_hermes_at(hermes_home: &Path, ctx: InitContext) -> Result<Vec<String>> {
	...
	if plugin_dir.exists() {
		if dry_run {
			println!("[dry-run] would remove Hermes plugin directory: {}", plugin_dir.display());
		} else {
			// nosemgrep: filesystem-deletion -- uninstall intentionally removes only RTK's Hermes plugin directory.
			fs::remove_dir_all(&plugin_dir)
				.with_context(|| format!("Failed to remove Hermes plugin directory: {}", plugin_dir.display()))?;
			if verbose > 0 {
	...
	if config_path.exists() {
		...
		if patched_config != existing_config {
			if dry_run {
				println!("[dry-run] would update Hermes config: {}", config_path.display());
				if verbose > 0 {
					println!("[dry-run] content:\n{}", patched_config);
				}
			} else {
				atomic_write(&config_path, &patched_config)
...
fn rewrite_hermes_config(existing: &str, add_rtk: bool) -> String {
	if existing.trim().is_empty() {
		return if add_rtk { hermes_plugins_block() } else { String::new() };
	}
	...
	let Some(enabled_idx) = find_yaml_key_line(&lines, "enabled", plugins_idx + 1, Some((plugins_end, plugins_indent)))
	else {
		if add_rtk {
...
fn find_yaml_key_line(lines: &[String], key: &str, start: usize, block: Option<(usize, usize)>) -> Option<usize> {
	let end = block.map_or(lines.len(), |(end, _)| end);
	let min_indent = block.map(|(_, indent)| indent);

	lines[start..end].iter().enumerate().find_map(|(offset, line)| {
...
fn rewrite_block_hermes_enabled(lines: &mut Vec<String>, enabled_idx: usize, add_rtk: bool) {
	...
	if add_rtk && !saw_rtk {
		...
		kept.push(format!("{}- {}\n", " ".repeat(item_indent), HERMES_PLUGIN_NAME));
	}
	...
	if add_rtk
		&& kept.iter().any(|line| is_yaml_list_item_named(line, HERMES_PLUGIN_NAME))
...
fn hermes_enabled_list_end(lines: &[String], enabled_idx: usize) -> usize {
	...
	lines[enabled_idx + 1..]
		...
		.find_map(|(offset, line)| {
			...
			if indent < enabled_indent || (indent == enabled_indent && !is_yaml_list_item_line(line)) {
...
fn hermes_enabled_list_item_indent(lines: &[String], enabled_idx: usize, enabled_end: usize) -> usize {
...
fn hermes_missing_enabled_indents(
	...
	let item_indent = if uses_indentationless_sequences { child_indent } else { child_indent + 2 };
...
fn run_codex_mode_with_paths(
	...
	let rtk_md_ref = if global {
		codex_rtk_md_ref(rtk_md_path.parent().context("RTK.md path missing parent directory")?)
	} else {
	...
	if !dry_run {
		...
		if global {
			println!("\n  Codex global instructions path: {}", agents_md_path.display());
		} else {
			println!("\n  Codex project instructions path: {}", agents_md_path.display());
		}
	}
	...
	match action {
		RtkBlockUpsert::Added => {
			if dry_run {
				...
				atomic_write(path, &new_content).with_context(|| format!("Failed to write {}", path.display()))?;
		...
		RtkBlockUpsert::Malformed => {
			...
			if let Some((line_num, _)) = existing.lines().enumerate().find(|(_, line)| line.contains(RTK_BLOCK_START)) {
...
fn patch_claude_md(path: &Path, ctx: InitContext) -> Result<bool> {
	...
	if content.contains(RTK_MD_REF) {
		...
		if migrated {
			if dry_run {
				println!("[dry-run] would migrate old RTK block in CLAUDE.md: {}", path.display());
			} else {
	...
	if dry_run {
		println!("[dry-run] would add @RTK.md reference to CLAUDE.md: {}", path.display());
		if verbose > 0 {
			println!("[dry-run] content:\n{}", new_content);
		}
	} else {
...
fn patch_agents_md(path: &Path, rtk_md_ref: &str, ctx: InitContext) -> Result<bool> {
	...
	let mut content = if path.exists() {
		fs::read_to_string(path).with_context(|| format!("Failed to read AGENTS.md: {}", path.display()))?
	} else {
		String::new()
	};
	...
	if content.contains(RTK_MD_REF) || content.contains(rtk_md_ref) {
		...
		if rtk_md_ref != RTK_MD_REF && content.contains(RTK_MD_REF) && !content.contains(rtk_md_ref) {
			...
			if dry_run {
				println!("[dry-run] would migrate {} to {} in {}", RTK_MD_REF, rtk_md_ref, path.display());
			} else {
		...
		if migrated {
			if dry_run {
				println!("[dry-run] would write migrated AGENTS.md: {}", path.display());
			} else {
				atomic_write(path, &content)
					.with_context(|| format!("Failed to write AGENTS.md: {}", path.display()))?;
			}
	...
	if dry_run {
		println!("[dry-run] would add {} reference to AGENTS.md: {}", rtk_md_ref, path.display());
		if verbose > 0 {
			println!("[dry-run] content:\n{}", new_content);
		}
	} else {
		atomic_write(path, &new_content).with_context(|| format!("Failed to write AGENTS.md: {}", path.display()))?;
		if verbose > 0 {
...
fn has_rtk_reference(content: &str, refs: &[&str]) -> bool {
	content.lines().map(str::trim).any(|line| refs.contains(&line))
}
...
fn remove_rtk_reference_from_agents(path: &Path, refs: &[&str], ctx: InitContext) -> Result<bool> {
	...
	let content = fs::read_to_string(path).with_context(|| format!("Failed to read AGENTS.md: {}", path.display()))?;
	...
	if dry_run {
		println!("[dry-run] would remove RTK.md reference from AGENTS.md: {}", path.display());
		if verbose > 0 {
			println!("[dry-run] content:\n{}", cleaned);
		}
		return Ok(true);
	...
	atomic_write(path, &cleaned).with_context(|| format!("Failed to write AGENTS.md: {}", path.display()))?;

	if verbose > 0 {
		eprintln!("Removed RTK.md reference from AGENTS.md: {}", path.display());
	}
...
fn remove_rtk_block(content: &str) -> (String, bool) {
	if let (Some(start), Some(end)) = (content.find(RTK_BLOCK_START), content.find(RTK_BLOCK_END)) {
		...
		eprintln!("[warn] Warning: Found '{}' without closing marker.", RTK_BLOCK_START);
		eprintln!("    This can happen if CLAUDE.md was manually edited.");

		if let Some((line_num, _)) = content.lines().enumerate().find(|(_, line)| line.contains(RTK_BLOCK_START)) {
			eprintln!("    Location: line {}", line_num + 1);
		}
...
fn resolve_home_subdir(subdir: &str) -> Result<PathBuf> {
	dirs::home_dir().map(|h| h.join(subdir)).context(if cfg!(windows) {
...
pub fn resolve_claude_dir() -> Result<PathBuf> {
	resolve_claude_dir_from(std::env::var_os("CLAUDE_CONFIG_DIR").map(PathBuf::from), dirs::home_dir())
}

fn resolve_claude_dir_from(claude_dir: Option<PathBuf>, home_dir: Option<PathBuf>) -> Result<PathBuf> {
...
fn resolve_codex_dir() -> Result<PathBuf> {
	resolve_codex_dir_from(std::env::var_os("CODEX_HOME").map(PathBuf::from), dirs::home_dir())
}

fn resolve_codex_dir_from(codex_home: Option<PathBuf>, home_dir: Option<PathBuf>) -> Result<PathBuf> {
...
fn resolve_hermes_home_from_env(home_dir: Option<PathBuf>, hermes_home: Option<OsString>) -> Result<PathBuf> {
...
fn pi_plugin_path_for_scope(global: bool) -> Result<PathBuf> {
	if global {
		...
		Ok(PathBuf::from(PI_LOCAL_DIR).join(PI_EXTENSIONS_SUBDIR).join(PI_PLUGIN_FILE))
	}
}
...
fn ensure_pi_extensions_dir(parent: &Path, name: &str, ctx: InitContext) -> Result<()> {
	...
	if dry_run {
		...
		fs::create_dir_all(parent).with_context(|| format!("Failed to create {}: {}", name, parent.display()))?;
	}
	Ok(())
}
...
fn uninstall_pi(global: bool, ctx: InitContext) -> Result<()> {
	...
	if plugin_path.exists() {
		if dry_run {
			println!("[dry-run] would remove Pi extension: {}", plugin_path.display());
		} else {
...
fn print_pi_result(plugin_path: &Path, installed: bool) {
	let status = if installed { "installed" } else { "already up to date" };
...
fn ensure_opencode_plugin_installed(path: &Path, ctx: InitContext) -> Result<bool> {
	...
	if !dry_run {
		if let Some(parent) = path.parent() {
			fs::create_dir_all(parent)
				.with_context(|| format!("Failed to create OpenCode plugin directory: {}", parent.display()))?;
		}
	}
...
fn remove_opencode_plugin(ctx: InitContext) -> Result<Vec<PathBuf>> {
	...
	if path.exists() {
		if dry_run {
			...
			fs::remove_file(&path).with_context(|| format!("Failed to remove OpenCode plugin: {}", path.display()))?;
			if verbose > 0 {
...
fn install_cursor_hooks(ctx: InitContext) -> Result<()> {
	...
	if old_hook.exists() {
		if dry_run {
			println!("[dry-run] would remove old Cursor hook script: {}", old_hook.display());
		} else {
			let _ = fs::remove_file(&old_hook);
			if verbose > 0 {
				eprintln!("  [ok] Removed old Cursor hook script: {}", old_hook.display());
			}
		}
...
fn patch_cursor_hooks_json(path: &Path, ctx: InitContext) -> Result<bool> {
	...
	let serialized = serde_json::to_string_pretty(&root).context("Failed to serialize hooks.json")?;

	if dry_run {
		println!("[dry-run] would patch Cursor hooks.json: {}", path.display());
		if verbose > 0 {
			println!("[dry-run] content:\n{}", serialized);
		}
		return Ok(true);
	...
	if path.exists() {
		...
		fs::copy(path, &backup_path).with_context(|| format!("Failed to backup to {}", backup_path.display()))?;
		if verbose > 0 {
			eprintln!("Backup: {}", backup_path.display());
		}
	}
...
fn remove_cursor_hooks(ctx: InitContext) -> Result<Vec<String>> {
	...
	if hook_path.exists() {
		if dry_run {
			println!("[dry-run] would remove Cursor hook: {}", hook_path.display());
		} else {
			// nosemgrep: filesystem-deletion
...
fn show_claude_config() -> Result<()> {
	...
	if binary_hook_registered {
		...
		#[cfg(unix)]
		{
			...
			let has_guards = hook_content.contains("command -v rtk") && hook_content.contains("command -v jq");
			let is_thin_delegator = hook_content.contains("rtk rewrite");
			let hook_version = super::hook_check::parse_hook_version(&hook_content);

			if !is_executable {
				println!("[warn] Hook: {} (NOT executable - run: chmod +x)", hook_path.display());
				...
				println!("[warn] Hook: {} (no guards - outdated)", hook_path.display());
			}
		}
	...
	if global_claude_md.exists() {
		...
		if content.contains(RTK_MD_REF) {
			...
			println!("[warn] Global (~/.claude/CLAUDE.md): old RTK block (run: rtk init -g to migrate)");
		} else {
	...
	if let Ok(cursor_dir) = resolve_cursor_dir() {
		...
		if cursor_binary_registered {
			...
			#[cfg(unix)]
			{
				...
				if !is_executable {
					println!("[warn] Cursor hook: {} (legacy script, NOT executable)", cursor_hook.display());
				} else {
					println!(
...
pub fn run_gemini(global: bool, hook_only: bool, patch_mode: PatchMode, ctx: InitContext) -> Result<()> {
	let InitContext { dry_run, .. } = ctx;
	if !global {
	...
	if !dry_run {
		fs::create_dir_all(&gemini_dir)
			.with_context(|| format!("Failed to create Gemini config dir: {}", gemini_dir.display()))?;
	}

	// 1. Install hook script
	let hook_dir = gemini_dir.join("hooks");
	if !dry_run {
		fs::create_dir_all(&hook_dir).with_context(|| format!("Failed to create hook dir: {}", hook_dir.display()))?;
	}
...
fn patch_gemini_settings(gemini_dir: &Path, hook_path: &Path, patch_mode: PatchMode, ctx: InitContext) -> Result<()> {
	let InitContext { verbose, dry_run } = ctx;
	...
	if dry_run {
		println!("[dry-run] would patch Gemini settings.json: {}", settings_path.display());
		if verbose > 0 {
			println!("[dry-run] content:\n{}", content);
		}
		return Ok(());
...
fn uninstall_gemini(ctx: InitContext) -> Result<Vec<String>> {
	...
	if hook_path.exists() {
		if dry_run {
			println!("[dry-run] would remove Gemini hook: {}", hook_path.display());
		} else {
	...
	if gemini_md.exists() {
		if dry_run {
			...
			fs::remove_file(&gemini_md).with_context(|| format!("Failed to remove {}", gemini_md.display()))?;
		}
	...
	if settings_path.exists() {
		...
		if let Ok(mut settings) = serde_json::from_str::<serde_json::Value>(&content) {
			...
			if let Some(arr) = settings.pointer_mut(&bt_pointer).and_then(|v| v.as_array_mut()) {
				let before = arr.len();
				arr.retain(|h| {
					!h.pointer("/hooks/0/command")
						.and_then(|v| v.as_str())
...
fn uninstall_copilot_at(base: &Path, ctx: InitContext) -> Result<Vec<String>> {
	...
	if hook_path.exists() {
		if dry_run {
			println!("[dry-run] would remove hook config: {}", hook_path.display());
		} else {
		...
		if content.contains(RTK_BLOCK_START) {
			...
			if did_remove {
				if dry_run {
					...
					atomic_write(&instructions_path, &cleaned)
						.with_context(|| format!("Failed to write {}", instructions_path.display()))?;
				}
				removed.push(format!("{}: removed rtk-instructions block", COPILOT_INSTRUCTIONS_FILE));
			}
		}
	}
...
fn run_copilot_global_at(copilot_dir: &Path, ctx: InitContext) -> Result<()> {
	...
	let hook_path = hooks_dir.join(COPILOT_HOOK_FILE);
	write_if_changed(&hook_path, COPILOT_HOOK_JSON, "Copilot global hook config", ctx)?;
...
mod tests {
	...
	fn test_init_mentions_all_top_level_commands() {
		for cmd in [
			...
			assert!(RTK_INSTRUCTIONS.contains(cmd), "Missing {cmd} in RTK_INSTRUCTIONS");
		}
	}
	...
	fn test_opencode_plugin_install_and_update() {
		...
		let changed = ensure_opencode_plugin_installed(&plugin_path, InitContext::default()).unwrap();
		assert!(changed);
		let content = fs::read_to_string(&plugin_path).unwrap();
		assert_eq!(content, OPENCODE_PLUGIN);

		fs::write(&plugin_path, "// old").unwrap();
		let changed_again = ensure_opencode_plugin_installed(&plugin_path, InitContext::default()).unwrap();
	...
	fn test_codex_mode_rejects_auto_patch() {
		...
		.unwrap_err();
		assert_eq!(err.to_string(), "--codex cannot be combined with --auto-patch");
	}
	...
	fn test_codex_mode_rejects_no_patch() {
		...
		.unwrap_err();
		assert_eq!(err.to_string(), "--codex cannot be combined with --no-patch");
	}
	...
	fn test_hermes_mode_preserves_pyyaml_same_indent_config_and_is_idempotent() {
		...
		assert_eq!(second, expected, "Hermes PyYAML config patch should be idempotent");
		assert_eq!(first.matches("rtk-rewrite").count(), 1);
	}
	...
	fn test_run_codex_mode_global_writes_absolute_reference_to_codex_dir() {
		...
		run_codex_mode_with_paths(agents_md.clone(), rtk_md.clone(), true, InitContext::default()).unwrap();
	...
	fn test_resolve_codex_dir_prefers_codex_home_and_ignores_empty_value() {
		...
		let preferred = resolve_codex_dir_from(Some(codex_home.clone()), Some(home_dir.clone())).unwrap();
		let empty_falls_back = resolve_codex_dir_from(Some(PathBuf::new()), Some(home_dir.clone())).unwrap();
	...
	fn test_resolve_claude_dir_prefers_rtk_override() {
		let result =
			resolve_claude_dir_from(Some(PathBuf::from("/custom/rtk-claude")), Some(PathBuf::from("/home/user")))
				.unwrap();
	...
	fn test_resolve_claude_dir_uses_claude_config_dir() {
		let result =
			resolve_claude_dir_from(Some(PathBuf::from("/custom/claude-config")), Some(PathBuf::from("/home/user")))
				.unwrap();
	...
	fn test_resolve_claude_dir_ignores_empty_overrides() {
		let empty = resolve_claude_dir_from(Some(PathBuf::new()), Some(PathBuf::from("/home/user"))).unwrap();
	...
	fn test_resolve_hermes_home_prefers_hermes_home() {
		...
		let resolved = resolve_hermes_home_from_env(Some(home_dir), Some(hermes_home.clone())).unwrap();
	...
	fn test_resolve_hermes_home_empty_env_falls_back_to_home() {
		...
		let empty_falls_back = resolve_hermes_home_from_env(Some(home_dir.clone()), Some(OsString::new())).unwrap();
		let missing_falls_back = resolve_hermes_home_from_env(Some(home_dir.clone()), None).unwrap();
	...
	fn test_write_if_changed_dry_run_does_not_create_file() {
		...
		let changed = write_if_changed(
			...
			"some content",
			"test file",
			InitContext { dry_run: true, ..Default::default() },
		)
		.unwrap();

		assert!(changed, "dry-run should report would-change for missing file");
		assert!(!target.exists(), "dry-run must not create file: {}", target.display());
	}
	...
	fn test_run_codex_mode_dry_run_writes_nothing() {
		...
		assert!(!rtk_md.exists(), "dry-run must not create RTK.md: {}", rtk_md.display());
		assert!(
			!agents_md.exists(),
	...
	fn test_insert_hook_entry_empty_root() {
		...
		assert!(json_content.get("hooks").unwrap().get("PreToolUse").is_some());
	...
	fn test_preserve_order_round_trip() {
		let original =
			r#"{"env": {"PATH": "/usr/bin"}, "permissions": {"allowAll": true}, "model": "claude-sonnet-4"}"#;
		...
		let _serialized_keys: Vec<&str> = serialized.split("\"").filter(|s| s.contains(":")).collect();
	...
	fn test_global_default_mode_creates_artifacts() {
		...
		with_claude_dir_override(&tmp, |claude_dir| {
			...
			assert!(claude_dir.join(CLAUDE_MD).exists(), "CLAUDE.md must be created");

			let settings = claude_dir.join(SETTINGS_JSON);
			assert!(settings.exists(), "settings.json must be created");
			let content = fs::read_to_string(&settings).unwrap();
			assert!(content.contains(CLAUDE_HOOK_COMMAND), "settings.json must contain hook command");
		});
	}
	...
	fn test_global_uninstall_removes_artifacts() {
		...
		with_claude_dir_override(&tmp, |claude_dir| {
			...
			let settings_content = fs::read_to_string(claude_dir.join(SETTINGS_JSON)).unwrap_or_default();
			assert!(
	...
	fn test_local_init_no_hook() {
		...
		result.unwrap();
		assert!(tmp.path().join(CLAUDE_MD).exists(), "local CLAUDE.md must be created");
		assert!(
	...
	fn test_run_default_mode_dry_run_writes_nothing() {
		let tmp = TempDir::new().unwrap();
		with_claude_dir_override(&tmp, |claude_dir| {
			let dry = InitContext { dry_run: true, ..Default::default() };
			run_default_mode(true, PatchMode::Auto, false, dry).unwrap();

			assert!(!claude_dir.join(RTK_MD).exists(), "dry-run must not create RTK.md");
			assert!(!claude_dir.join(CLAUDE_MD).exists(), "dry-run must not create CLAUDE.md");
			assert!(
	...
	fn test_uninstall_dry_run_preserves_artifacts() {
		...
		with_claude_dir_override(&tmp, |claude_dir| {
			...
			assert!(claude_dir.join(RTK_MD).exists(), "dry-run uninstall must not remove RTK.md");
			assert!(
				claude_dir.join(SETTINGS_JSON).exists(),
	...
	fn test_uninstall_integration_preserves_user_content() {
		...
		assert!(cleaned.contains("My Project Rules"), "user content must be preserved");
		assert!(cleaned.contains("snake_case"), "user content must be preserved");
		assert!(!cleaned.contains(RTK_BLOCK_START), "RTK block must be fully removed");
		assert!(!cleaned.contains(RTK_BLOCK_END), "RTK end marker must be removed");
	}
	...
	fn test_claude_md_mode_refuses_malformed_block() {
		...
		with_claude_dir_override(&tmp, |claude_dir| {
			...
			let malformed = format!("# Existing notes\n\n{}\nincomplete RTK block\n", RTK_BLOCK_START);
			fs::write(&claude_md, &malformed).unwrap();

			let result = run_claude_md_mode(true, false, InitContext::default());

			assert!(result.is_err(), "Malformed CLAUDE.md must cause a hard error, not silent skip");
	...
	fn test_run_pi_mode_global_installs_plugin() {
		...
		with_pi_dir_override(&tmp, |pi_dir| {
			...
			assert!(content.contains("rtk rewrite"), "extension must delegate to rtk rewrite");
		});
	}
	...
	fn test_run_pi_mode_local_installs_plugin() {
		...
		let plugin = tmp.path().join(".pi").join(PI_EXTENSIONS_SUBDIR).join(PI_PLUGIN_FILE);
	...
	fn test_run_pi_mode_global_creates_plugin_when_dir_absent() {
		...
		assert!(plugin.exists(), "plugin must be written even when dir was absent");
	...
	fn test_pi_plugin_path_for_scope_local() {
		...
		assert_eq!(
			path,
			PathBuf::from(PI_LOCAL_DIR).join(PI_EXTENSIONS_SUBDIR).join(PI_PLUGIN_FILE)
		);
	}
	...
	fn test_run_pi_mode_global_dry_run_writes_nothing() {
		let tmp = TempDir::new().unwrap();
		with_pi_dir_override(&tmp, |pi_dir| {
			run_pi_mode(true, InitContext { verbose: 0, dry_run: true }).unwrap();
			...
			assert!(
				!pi_dir.join(PI_EXTENSIONS_SUBDIR).join(PI_PLUGIN_FILE).exists(),
	...
	fn test_run_pi_mode_local_dry_run_writes_nothing() {
		...
		let result = run_pi_mode(false, InitContext { verbose: 0, dry_run: true });
		std::env::set_current_dir(&cwd).unwrap();
		result.unwrap();
	...
	fn test_pi_global_uninstall_dry_run_keeps_plugin() {
		...
		with_pi_dir_override(&tmp, |pi_dir| {
			run_pi_mode(true, InitContext::default()).unwrap();
			let plugin = pi_dir.join(PI_EXTENSIONS_SUBDIR).join(PI_PLUGIN_FILE);
			assert!(plugin.exists(), "plugin must exist before uninstall dry-run");

			uninstall(true, false, false, false, true, InitContext { verbose: 0, dry_run: true }).unwrap();

			assert!(plugin.exists(), "dry-run uninstall must not remove the Pi extension");
		});
	}
	...
	fn test_pi_local_uninstall_dry_run_keeps_plugin() {
		...
		let result = uninstall(false, false, false, false, true, InitContext { verbose: 0, dry_run: true });
		std::env::set_current_dir(&cwd).unwrap();
		result.unwrap();

		assert!(plugin.exists(), "dry-run uninstall must not remove the local Pi extension");
	}
	...
	fn test_copilot_init_preserves_existing_instructions() {
		...
		assert!(final_content.contains(RTK_BLOCK_START), "RTK block start marker missing");
		assert!(final_content.contains(RTK_BLOCK_END), "RTK block end marker missing");
	}
	...
	fn test_copilot_init_idempotent_repeats() {
		...
		assert_eq!(after_first, after_second, "Second init must be a no-op (idempotent)");

		let count_start = after_first.matches(RTK_BLOCK_START).count();
		let count_end = after_first.matches(RTK_BLOCK_END).count();
		assert_eq!(count_start, 1, "RTK_BLOCK_START must appear once, got {count_start}");
	...
	fn test_copilot_init_updates_stale_block() {
		...
		assert!(updated.contains("Use rg."), "User content outside the block must be preserved");
		assert!(
	...
	fn test_copilot_init_dry_run_no_write() {
		...
		let ctx = InitContext { dry_run: true, ..InitContext::default() };
		run_copilot_at(temp.path(), ctx).unwrap();

		assert!(!instructions_path.exists(), "Dry-run must not create copilot-instructions.md");
	}
	...
	fn test_copilot_init_fresh_install_creates_file() {
		...
		assert!(instructions_path.exists(), "Fresh install must create copilot-instructions.md");
	...
	fn test_copilot_hook_json_serves_both_vscode_and_cli_schemas() {
		...
		assert!(cli["timeoutSec"].is_number(), "Copilot CLI uses `timeoutSec`");
	}
	...
	fn test_copilot_init_writes_dual_schema_to_disk() {
		...
		let hook_path = temp.path().join(".github").join("hooks").join("rtk-rewrite.json");
		let v: serde_json::Value = serde_json::from_str(&fs::read_to_string(&hook_path).unwrap()).unwrap();
	...
	fn test_copilot_uninstall_removes_hook_and_block() {
		...
		assert!(!instructions.contains(RTK_BLOCK_START), "RTK block must be removed");
	}
	...
	fn test_copilot_uninstall_dry_run_keeps_files() {
		...
		let ctx = InitContext { verbose: 0, dry_run: true };
	...
	fn test_copilot_install_does_not_touch_other_hooks() {
		...
		let other_content = r#"{"hooks":{"sessionStart":[{"type":"command","command":"echo hi"}]}}"#;
	...
	fn test_copilot_uninstall_does_not_touch_other_hooks() {
		...
		assert!(other_hook.exists(), "third-party hook file must survive rtk uninstall");
		assert_eq!(
			fs::read_to_string(&other_hook).unwrap(),
			other_content,
	...
	fn test_copilot_init_refuses_malformed_block() {
		...
		assert!(result.is_err(), "Malformed file must cause an error, not silent rewrite");
