...
use cmds::js::{lint_cmd, next_cmd, npm_cmd, playwright_cmd, pnpm_cmd, prettier_cmd, prisma_cmd, tsc_cmd, vitest_cmd};
use cmds::jvm::{gradlew_cmd, mvn_cmd};
use cmds::python::{mypy_cmd, pip_cmd, pytest_cmd, ruff_cmd};
use cmds::ruby::{rake_cmd, rspec_cmd, rubocop_cmd};
use cmds::rust::{cargo_cmd, runner};
use cmds::system::{
	deps, env_cmd, find_cmd, format_cmd, grep_cmd, json_cmd, local_llm, log_cmd, ls, pipe_cmd, read, summary, tree,
	wc_cmd,
};
	...
	if let Some(filter) = toml_match {
		...
		match result {
			Ok(output) => {
				...
				timer.track(&raw_command, &format!("rtk:toml {}", raw_command), &combined_raw, &filtered);
...
fn run_cli() -> Result<i32> {
	...
	let code = match cli.command {
		...
		Commands::Read { files, level, max_lines, tail_lines, line_numbers } => {
			...
			for file in &files {
				let result = if file == Path::new("-") {
					...
					read::run(file, level, max_lines, tail_lines, line_numbers, cli.verbose)
				};
				if let Err(e) = result {
		...
		Commands::Git {
			...
			match command {
				GitCommands::Diff { args } => git::run(git::GitCommand::Diff, &args, None, cli.verbose, &global_args)?,
				GitCommands::Log { args } => git::run(git::GitCommand::Log, &args, None, cli.verbose, &global_args)?,
				GitCommands::Status { args } => {
					git::run(git::GitCommand::Status, &args, None, cli.verbose, &global_args)?
				},
				GitCommands::Show { args } => git::run(git::GitCommand::Show, &args, None, cli.verbose, &global_args)?,
				GitCommands::Add { args } => git::run(git::GitCommand::Add, &args, None, cli.verbose, &global_args)?,
				GitCommands::Commit { args } => {
					git::run(git::GitCommand::Commit, &args, None, cli.verbose, &global_args)?
				},
				GitCommands::Push { args } => git::run(git::GitCommand::Push, &args, None, cli.verbose, &global_args)?,
				GitCommands::Pull { args } => git::run(git::GitCommand::Pull, &args, None, cli.verbose, &global_args)?,
				...
				GitCommands::Stash { subcommand, args } => {
					git::run(git::GitCommand::Stash { subcommand }, &args, None, cli.verbose, &global_args)?
				},
		...
		Commands::Glab { repo, group, subcommand, mut args } => {
		...
		Commands::Pnpm { filter, command } => {
			...
			match command {
				...
				PnpmCommands::Outdated { args } => {
					pnpm_cmd::run(pnpm_cmd::PnpmCommand::Outdated, &merge_pnpm_args(&filter, &args), cli.verbose)?
				},
				PnpmCommands::Install { args } => {
					pnpm_cmd::run(pnpm_cmd::PnpmCommand::Install, &merge_pnpm_args(&filter, &args), cli.verbose)?
				},
		...
		Commands::Json { file, depth, keys_only } => {
			if file == Path::new("-") {
		...
		Commands::Docker { command } => match command {
			...
			DockerCommands::Images => container::run(container::ContainerCmd::DockerImages, &[], cli.verbose)?,
			...
			DockerCommands::Compose { command: compose } => match compose {
				...
				ComposeCommands::Build { service } => container::run_compose_build(service.as_deref(), cli.verbose)?,
				ComposeCommands::Other(args) => container::run_compose_passthrough(&args, cli.verbose)?,
			},
		...
		Commands::Grep { max_len, max, context_only, file_type, extra_args } => {
			grep_cmd::run(max_len, max, context_only, file_type.as_deref(), &extra_args, cli.verbose)?
		},
		...
		Commands::Init {
			...
			let ctx = hooks::init::InitContext { verbose: cli.verbose, dry_run };
			if show {
				...
				if global {
					anyhow::bail!("Antigravity is project-scoped. Use: rtk init --agent antigravity");
				}
		...
		Commands::CcEconomics { daily, weekly, monthly, all, format } => {
		...
		Commands::Prisma { command } => match command {
			...
			PrismaCommands::Migrate { command } => match command {
				PrismaMigrateCommands::Dev { name, args } => prisma_cmd::run(
					prisma_cmd::PrismaCommand::Migrate { subcommand: prisma_cmd::MigrateSubcommand::Dev { name } },
					&args,
					cli.verbose,
				)?,
				PrismaMigrateCommands::Status { args } => prisma_cmd::run(
					prisma_cmd::PrismaCommand::Migrate { subcommand: prisma_cmd::MigrateSubcommand::Status },
					&args,
					cli.verbose,
				)?,
				PrismaMigrateCommands::Deploy { args } => prisma_cmd::run(
					prisma_cmd::PrismaCommand::Migrate { subcommand: prisma_cmd::MigrateSubcommand::Deploy },
					&args,
					cli.verbose,
				)?,
			},
			PrismaCommands::DbPush { args } => prisma_cmd::run(prisma_cmd::PrismaCommand::DbPush, &args, cli.verbose)?,
		},
		...
		Commands::Cargo { command } => match command {
			CargoCommands::Build { args } => cargo_cmd::run(cargo_cmd::CargoCommand::Build, &args, cli.verbose)?,
			CargoCommands::Test { args } => cargo_cmd::run(cargo_cmd::CargoCommand::Test, &args, cli.verbose)?,
			CargoCommands::Clippy { args } => cargo_cmd::run(cargo_cmd::CargoCommand::Clippy, &args, cli.verbose)?,
			CargoCommands::Check { args } => cargo_cmd::run(cargo_cmd::CargoCommand::Check, &args, cli.verbose)?,
			CargoCommands::Install { args } => cargo_cmd::run(cargo_cmd::CargoCommand::Install, &args, cli.verbose)?,
			CargoCommands::Nextest { args } => cargo_cmd::run(cargo_cmd::CargoCommand::Nextest, &args, cli.verbose)?,
		...
		Commands::Discover { project, limit, all, since, format } => {
		...
		Commands::Learn { project, all, since, format, write_rules, min_confidence, min_occurrences } => {
			learn::run(project, all, since, format, write_rules, min_confidence, min_occurrences)?;
			0
		},
			...
			match args[0].as_str() {
				...
				"prisma" => {
					...
					if args.len() > 1 {
						...
						match args[1].as_str() {
							"generate" => {
								prisma_cmd::run(prisma_cmd::PrismaCommand::Generate, &prisma_args, cli.verbose)?
							},
		...
		Commands::Proxy { args } => {
			...
			let (cmd_name, cmd_args): (String, Vec<String>) = if args.len() == 1 {
				...
				(
					...
					args[1..].iter().map(|s| s.to_string_lossy().into_owned()).collect(),
				)
			};
			...
			let stdout_pipe = inner.stdout.take().context("Failed to capture child stdout")?;
			let stderr_pipe = inner.stderr.take().context("Failed to capture child stderr")?;
		...
		Commands::Verify { filter, require_all } => {
			if filter.is_some() {
...
mod tests {
	...
	fn test_git_commit_multiple_messages() {
		let cli =
			Cli::try_parse_from(["rtk", "git", "commit", "-m", "feat: add support", "-m", "Body paragraph here."])
				.unwrap();
		match cli.command {
			Commands::Git { command: GitCommands::Commit { args }, .. } => {
				assert_eq!(args, vec!["-m", "feat: add support", "-m", "Body paragraph here."]);
			},
			_ => panic!("Expected Git Commit command"),
		}
	}
	...
	fn test_git_commit_amend() {
		let cli = Cli::try_parse_from(["rtk", "git", "commit", "--amend", "-m", "new msg"]).unwrap();
		match cli.command {
			Commands::Git { command: GitCommands::Commit { args }, .. } => {
	...
	fn test_git_global_options_parsing() {
		let cli = Cli::try_parse_from(["rtk", "git", "--no-pager", "--no-optional-locks", "status"]).unwrap();
		match cli.command {
			Commands::Git { no_pager, no_optional_locks, bare, literal_pathspecs, .. } => {
				assert!(no_pager);
	...
	fn test_git_commit_long_flag_multiple() {
		...
		match cli.command {
			Commands::Git { command: GitCommands::Commit { args }, .. } => {
				assert_eq!(args, vec!["--message", "title", "--message", "body", "--message", "footer"]);
			},
			_ => panic!("Expected Git Commit command"),
		}
	}
	...
	fn test_try_parse_kubectl_get_alias() {
		...
		match cli.command {
			Commands::Kubectl { command: KubectlCommands::Get { args } } => {
	...
	fn test_init_uninstall_dispatch_routes_hermes_to_hermes_cleanup() {
		...
		let ctx = hooks::init::InitContext { verbose: 2, dry_run: true };
	...
	fn test_try_parse_unknown_subcommand_is_error() {
		match Cli::try_parse_from(["rtk", "nonexistent-command"]) {
			Err(e) => assert!(!matches!(e.kind(), ErrorKind::DisplayHelp | ErrorKind::DisplayVersion)),
	...
	fn test_try_parse_git_with_dash_c_succeeds() {
		...
		assert!(result.is_ok(), "git -C /path status should parse successfully");
	...
	fn test_meta_commands_reject_bad_flags() {
		...
		for cmd in RTK_META_COMMANDS {
			...
			assert!(result.is_err(), "Meta-command '{}' with bad flag should fail to parse", cmd);
		}
	}
	...
	fn test_hook_claude_parses() {
		...
		assert!(matches!(cli.command, Commands::Hook { command: HookCommands::Claude }));
	}
	...
	fn test_hook_check_parses() {
		...
		match cli.command {
			Commands::Hook { command: HookCommands::Check { agent, command } } => {
	...
	fn test_hook_check_with_agent() {
		let cli = Cli::try_parse_from(["rtk", "hook", "check", "--agent", "gemini", "cargo", "test"]).unwrap();
		match cli.command {
			Commands::Hook { command: HookCommands::Check { agent, command } } => {
	...
	fn test_hook_check_preserves_double_dash_in_command() {
		let cli = Cli::try_parse_from(["rtk", "hook", "check", "shadowenv", "exec", "--", "git", "status"]).unwrap();
		match cli.command {
			Commands::Hook { command: HookCommands::Check { agent, command } } => {
				assert_eq!(agent, "claude");
	...
	fn test_meta_command_list_is_complete() {
		...
		for args in &meta_cmds_that_parse {
			...
			assert!(result.is_ok(), "Meta-command {:?} should parse successfully", args);
		}
	}
	...
	fn test_shell_split_single_quotes() {
		assert_eq!(shell_split("grep -r 'hello world' ."), vec!["grep", "-r", "hello world", "."]);
	}
	...
	fn test_merge_filters_with_args_os() {
		...
		let args = vec![OsString::from("--depth=0")];
		let expected_args = vec![OsString::from("--filter=@app1"), OsString::from("--depth=0")];
		assert_eq!(merge_pnpm_args_os(&filters, &args), expected_args);
	}
	...
	fn test_pnpm_subcommand_with_filter() {
		let cli = Cli::try_parse_from([
			"rtk", "pnpm", "--filter", "@app1", "--filter", "@app2", "list", "--filter", "@app3", "--filter", "@app4",
			"--prod",
		])
		.unwrap();
		match cli.command {
			Commands::Pnpm { filter, command: PnpmCommands::List { depth, args } } => {
				assert_eq!(depth, 0);
				assert_eq!(filter, vec!["@app1", "@app2"]);
				assert_eq!(args, vec!["--filter", "@app3", "--filter", "@app4", "--prod"]);
			},
	...
	fn test_git_push_u_flag_passes_through() {
		...
		assert!(!cli.ultra_compact, "-u on git push must NOT be consumed as --ultra-compact");
		match cli.command {
			Commands::Git { command: GitCommands::Push { args }, .. } => {
				assert!(
	...
	fn test_pnpm_subcommand_with_short_filter() {
		...
		let cli = Cli::try_parse_from(["rtk", "pnpm", "-F", "@app1", "-F", "@app2", "list"]).unwrap();
		match cli.command {
	...
	fn test_pnpm_typecheck_without_filters() {
		let cli = Cli::try_parse_from(["rtk", "pnpm", "typecheck", "--filter", "@app3", "--filter", "@app4"]).unwrap();
		match cli.command {
			Commands::Pnpm { filter, command } => {
	...
	fn test_init_uninstall_agent_pi_parses() {
		let cli = Cli::try_parse_from(["rtk", "init", "--uninstall", "--agent", "pi", "--global"]).unwrap();
		match cli.command {
			Commands::Init { uninstall, agent, global, .. } => {
				assert!(uninstall);
