...
pub fn format_console_report(rules: &[CorrectionRule], total_corrections: usize, sessions: usize, days: u64) -> String {
...
pub fn write_rules_file(rules: &[CorrectionRule], path: &str) -> Result<()> {
	...
	for rule in rules {
		grouped.entry(rule.base_command.clone()).or_default().push(rule);
	}
