...
pub fn run(
	...
	for session_path in &sessions {
		...
		for ext_cmd in extracted {
			...
			if let Some(output) = ext_cmd.output_content {
				all_commands.push(CommandExecution { command: ext_cmd.command, is_error: ext_cmd.is_error, output });
			}
		}
	}
	...
	if corrections.is_empty() {
		println!("No CLI corrections detected in {} sessions.", sessions.len());
		return Ok(());
	}

	// Filter by confidence
	let filtered: Vec<_> = corrections.into_iter().filter(|c| c.confidence >= min_confidence).collect();
